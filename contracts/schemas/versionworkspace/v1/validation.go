...
func ValidateChangesResult(result ChangesResult) error {
	if err := ValidateSession(result.Session); err != nil || len(result.ChangedPaths) > MaxChangedPaths || result.Summary.Total != result.Summary.Added+result.Summary.Modified+result.Summary.Removed || result.Summary.Total < 0 {
		return errors.New("版本工作区变更结果无效")
	}
	paths := append([]string(nil), result.ChangedPaths...)
	if !sort.StringsAreSorted(paths) || result.Dirty != (result.Summary.Total > 0) {
		return errors.New("版本工作区变更列表未排序或状态不一致")
	}
	for index, value := range paths {
		if strings.TrimSpace(value) == "" || index > 0 && paths[index-1] == value {
...
func ValidateCommitRequest(request CommitRequest) error {
	if !sessionPattern.MatchString(request.SessionID) || request.ExpectedRevision == 0 || len(request.Message) > 1024 || len(request.Labels) > 16 {
