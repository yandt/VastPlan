...
func TestParse_NormalizesStableChannelAndMatchesNode(t *testing.T) {
  ...
  "units":[{
    ...
    "plugins":[{"id":"com.example.demo","version":"1.2.3","sha256":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"}]
  }]
}`))
	if err != nil {
...
func TestParse_RejectsReplicasAboveOne(t *testing.T) {
	_, err := Parse([]byte(`{"version":1,"revision":1,"metadata":{"name":"local"},"units":[{"id":"x","kind":"service","enabled":true,"service_role":"backend","replicas":2,"plugins":[{"id":"com.example.demo","version":"1.0.0","sha256":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"}]}]}`))
	if err == nil {
		t.Fatal("本地 v1 不具备跨节点调度，replicas>1 必须 fail-closed")
	}
}

func TestParse_RequiresResolvedArtifactDigest(t *testing.T) {
	_, err := Parse([]byte(`{"version":1,"revision":1,"metadata":{"name":"local"},"units":[{"id":"x","kind":"service","enabled":true,"service_role":"backend","replicas":1,"plugins":[{"id":"com.example.demo","version":"1.0.0"}]}]}`))
	if err == nil {
		t.Fatal("Assignment 必须携带 Resolver 锁定的制品 SHA-256")
	}
}
...
func TestParse_RejectsDuplicateUnitAndPluginIDs(t *testing.T) {
	cases := []struct {
		...
		{"unit 重复", `{"version":1,"revision":1,"metadata":{"name":"local"},"units":[{"id":"x","kind":"service","enabled":true,"service_role":"backend","replicas":1,"plugins":[{"id":"com.example.a","version":"1.0.0","sha256":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"}]},{"id":"x","kind":"service","enabled":true,"service_role":"backend","replicas":1,"plugins":[{"id":"com.example.b","version":"1.0.0","sha256":"bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"}]}]}`},
		{"插件重复", `{"version":1,"revision":1,"metadata":{"name":"local"},"units":[{"id":"x","kind":"service","enabled":true,"service_role":"backend","replicas":1,"plugins":[{"id":"com.example.a","version":"1.0.0","sha256":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"},{"id":"com.example.a","version":"1.1.0","sha256":"bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"}]}]}`},
	}
...
func TestParse_ValidatesPluginScopedConfigurationEnvelope(t *testing.T) {
	valid := `{"version":1,"revision":1,"metadata":{"name":"local"},"units":[{"id":"x","kind":"service","enabled":true,"service_role":"backend","replicas":1,"config":{"plugins":{"com.example.a":{"retries":3}},"environment_allowlist":{"com.example.a":["EXAMPLE_TOKEN"]}},"plugins":[{"id":"com.example.a","version":"1.0.0","sha256":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"}]}]}`
	if _, err := Parse([]byte(valid)); err != nil {
		t.Fatalf("插件隔离配置信封应通过: %v", err)
	}
	invalid := `{"version":1,"revision":1,"metadata":{"name":"local"},"units":[{"id":"x","kind":"service","enabled":true,"service_role":"backend","replicas":1,"config":{"plugins":{"com.example.other":{"token":"secret"}}},"plugins":[{"id":"com.example.a","version":"1.0.0","sha256":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"}]}]}`
