...
func TestAuthorizationGuardAcceptsInjectedApprovalPolicy(t *testing.T) {
...
func TestBuildPermissionCatalogFromSystemManagementPlugins(t *testing.T) {
	...
	if catalog.SchemaVersion != PermissionCatalogSchemaVersion || len(catalog.Permissions) != 26 || len(catalog.Operations) != 99 || len(catalog.Digest) != 64 {
...
func TestPortalComposerPermissionCatalogUsesSignedOperations(t *testing.T) {
	...
	if len(catalog.Permissions) != 4 || len(catalog.Operations) != 21 {
