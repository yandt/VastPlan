...
func TestManifestDataModels(t *testing.T) {
	...
	if len(references) != 1 || references[0].ID != "example.record" || references[0].ContractVersion != "1.1.0" {
		t.Fatalf("DataModel 引用解析错误: %+v", references)
	}
}

func TestManifestDataModelsRejectsDuplicates(t *testing.T) {
	raw := strings.Replace(string(dataModelManifest()), `]}}`, `,{"id":"example.record","contractVersion":"1.1.0","path":"data-models/other.json","sha256":"bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"}]}}`, 1)
...
func dataModelManifest() []byte {
  ...
  "contributes":{"backend":{"dataModels":[{"id":"example.record","contractVersion":"1.1.0","path":"data-models/record.json","sha256":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"}]}}
}`)
}
