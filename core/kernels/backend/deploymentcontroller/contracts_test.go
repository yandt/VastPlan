...
func TestValidateDeploymentContractsBuildsRemoteDAGAndChecksVersion(t *testing.T) {
	databaseManifest := []byte(`{
		...
		"version":"7.4.9","publisher":"example","engines":{"backend":"^1.0"},
		"runtime":{"instancePolicy":"active-active","stateModel":"external-shared","visibility":"cluster","routing":"queue","provides":[{"extensionPoint":"tool.package","capability":"platform.database","contractVersion":"1.2.0","visibility":"cluster","routing":"queue"}]},
	...
	consumerManifest := []byte(`{
		...
		"runtime":{"instancePolicy":"active-active","stateModel":"external-shared","visibility":"cluster","routing":"queue","provides":[{"extensionPoint":"tool.package","capability":"consumer.tool","contractVersion":"1.0.0","visibility":"cluster","routing":"queue"}],
			"requires":[{"capability":"platform.database","contractRange":"^1.0.0","scope":"remote","kind":"strong","ready":"readiness","failurePolicy":"retry","logicalService":"platform.database"}]},
		"activation":["onStartup"],"entry":{"backend":"backend/main"},
		"contributes":{"backend":{"tools":[{"id":"consumer.tool","service_role":"backend","title":"consumer","subcommands":[]}]}}
	}`)
	reader := contractArtifactReader{
		"com.example.database@7.4.9": {PluginID: "com.example.database", Version: "7.4.9", Channel: "stable", Manifest: databaseManifest},
	...
	deployment := deploymentv2.Deployment{Version: 2, Revision: 1, Metadata: deploymentv1.Metadata{Name: "prod"}, Units: []deploymentv2.ServiceUnit{
		{ID: "database", Kind: "service", Enabled: true, ServiceRole: "backend", LogicalService: "platform.database", InstancePolicy: "active-active", StateModel: "external-shared", Visibility: "cluster", Routing: "queue", Replicas: 1, Plugins: []deploymentv1.PluginRef{lockedRef("com.example.database", "7.4.9")}},
	...
	if err := validateDeploymentContracts(deployment, map[string][]string{"database": nil, "api": nil}, reader); err == nil || !strings.Contains(err.Error(), "契约范围") {
...
func TestValidateDeploymentContractsDefersExplicitExternalRemoteDependencyToReadiness(t *testing.T) {
	consumerManifest := []byte(`{
		"id":"com.example.consumer","name":"consumer","description":"consumer",
		"version":"1.0.0","publisher":"example","engines":{"backend":"^1.0"},
		"runtime":{"instancePolicy":"active-active","stateModel":"external-shared","visibility":"cluster","routing":"queue","routingDomain":"application","provides":[{"extensionPoint":"tool.package","capability":"consumer.tool","contractVersion":"1.0.0","visibility":"cluster","routing":"queue","routingDomain":"application"}],
			"requires":[{"capability":"platform.settings","contractRange":"^1.0.0","scope":"remote","kind":"strong","ready":"readiness","failurePolicy":"fail","logicalService":"platform.settings","routingDomain":"platform"}]},
		"activation":["onStartup"],"entry":{"backend":"backend/main"},
		"contributes":{"backend":{"tools":[{"id":"consumer.tool","service_role":"backend","title":"consumer","subcommands":[]}]}}
	}`)
