...
func TestTrustedCatalogRequiresVerifiedFrontendRenderAdapterContribution(t *testing.T) {
	...
	rendererArtifact, rendererPackage := packageFrontendFixture(t, `{"id":"cn.vastplan.foundation.frontend.render.adapter.test-renderer","name":"renderer","description":"test","version":"1.0.0","publisher":"vastplan","engines":{"frontend":"^1.0"},"activation":["onPortalStartup"],"entry":{"frontend":"frontend/main.js"},"contributes":{"frontend":{"rendererModules":[{"id":"test","adapter":"ui.render.adapter","uiContract":"^4.0.0","engineFamily":"react","framework":"test"}]}}}`, []byte(`export const renderer = { id: "test" };`))
	...
	source[rendererArtifact.PluginID+"@"+rendererArtifact.Version] = artifacttrust.Envelope{Artifact: rendererArtifact, PackageBytes: rendererPackage}
	...
	rendererRef := portalapi.PluginRef{ID: rendererArtifact.PluginID, Version: rendererArtifact.Version}
	workbenchRef := portalapi.PluginRef{ID: workbenchArtifact.PluginID, Version: workbenchArtifact.Version}
	spec := portalapi.PortalSpec{Revision: 1, ID: "admin", TenantID: "tenant-a", Route: "/", RuntimeEngine: portalapi.RuntimeEngine{PluginRef: engineRef, EngineContract: "^1.0.0", Family: "react"}, RenderAdapter: portalapi.RenderAdapter{PluginRef: ref, UIContract: "^4.0.0", Config: frontendcompositionv1.RenderAdapterConfig{DefaultRenderer: "test", AllowedRenderers: []string{"test"}}}, Shell: portalapi.Shell{PluginRef: shellRef, UIContract: "^4.0.0", Config: frontendcompositionv1.ShellConfig{DefaultTemplate: "standard", AllowedTemplates: []string{"standard"}}}, Workbench: portalapi.Workbench{PluginRef: workbenchRef, UIContract: "^4.0.0"}, Plugins: []portalapi.PluginRef{engineRef, ref, rendererRef, shellRef, standardRef, topRef, workbenchRef}, Resolution: portalapi.Resolution{PlatformProfile: compositioncommonv1.Ref{ID: "default", Revision: 1, Digest: strings.Repeat("a", 64)}, ApplicationComposition: compositioncommonv1.Ref{ID: "admin", Revision: 1, Digest: strings.Repeat("b", 64)}, PluginOrigins: map[string]string{engineRef.ID: compositioncommonv1.OriginPlatformProfile, ref.ID: compositioncommonv1.OriginPlatformProfile, rendererRef.ID: compositioncommonv1.OriginPlatformProfile, shellRef.ID: compositioncommonv1.OriginPlatformProfile, standardRef.ID: compositioncommonv1.OriginPlatformProfile, topRef.ID: compositioncommonv1.OriginPlatformProfile, workbenchRef.ID: compositioncommonv1.OriginPlatformProfile}}}
	...
	if len(runtime.Modules) != len(spec.Plugins) || runtime.Modules[1].SHA256 != hex.EncodeToString(wantDigest[:]) || runtime.Modules[1].PackageSHA256 != artifact.SHA256 || len(runtime.Contributions.Contributions) == 0 {
