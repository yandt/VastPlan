...
export class SealedCookieCodec {
  ...
  public static async open(keyFile: string, audience: string, now: () => number = Date.now): Promise<SealedCookieCodec> {
    ...
    if (!info.isFile() || info.isSymbolicLink() || (info.mode & 0o077) !== 0) throw new Error("OIDC session key 必须是仅属主可读写的普通文件");
    ...
    if (key.byteLength !== 32) throw new Error("OIDC session key 必须是 32 字节或 64 位十六进制");
  ...
  public seal(value: Readonly<Record<string, unknown>>): string {
    ...
    if (plaintext.byteLength > 4096) throw new Error("OIDC session 内容超过上限");
  ...
  public unseal(value: string): Readonly<Record<string, unknown>> {
    ...
    catch { throw new Error("OIDC session cookie 无效"); }
    if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) throw new Error("OIDC session cookie 内容无效");
