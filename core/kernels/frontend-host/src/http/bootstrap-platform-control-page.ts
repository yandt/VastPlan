...
export const bootstrapPlatformControlPageContract = "3";
...
const json=async(url,options={})=>{const response=await fetch(url,{credentials:'same-origin',...options,headers:{'Content-Type':'application/json',...(options.headers||{})}});let body={};try{body=await response.json()}catch{}if(response.status===401){location.href='/auth/login?returnTo='+encodeURIComponent(location.pathname);throw new Error('会话已失效，请重新登录')}if(!response.ok){const detail=friendlyValidation(body.validation)||friendly(body.error||'request_failed'),trace=typeof body.traceId==='string'&&/^[a-f0-9]{32}$/.test(body.traceId)?'（跟踪编号：'+body.traceId+'）':'';throw new Error(detail+trace)}return body};
const friendly=code=>({database_connection_invalid:'数据库连接配置无效，请检查地址、端口、用户名和传输加密设置',database_connection_failed:'数据库连接未能建立，请检查网络、账户密码和服务端状态',database_credential_unavailable:'数据库密码不可用或已经失效，请重新输入密码后重试',database_credential_service_unavailable:'数据库凭证服务暂时不可用，请稍后重试',database_tls_policy_forbidden:'当前部署策略不允许关闭数据库传输加密校验',database_name_resolution_failed:'数据库地址无法解析，请检查主机名或 DNS 配置',database_connection_refused:'数据库服务器拒绝了连接，请检查地址、端口和服务监听状态',database_connection_timeout:'连接数据库超时，请检查网络和连接超时设置',database_tls_verification_failed:'数据库传输加密或证书校验失败，请检查证书与服务器名称',database_authentication_failed:'数据库用户名或密码验证失败',database_not_found:'指定的数据库不存在',database_permission_denied:'数据库账户没有所需权限',database_pool_exhausted:'数据库连接资源暂时不足，请稍后重试',database_runtime_unavailable:'数据库运行服务暂时不可用，请稍后重试',platform_control_invalid:'配置格式无效，请检查各字段',platform_control_secret_unavailable:'无法读取密码来源，请检查文件权限或 Credential 名称',platform_control_database_unavailable:'数据库不可连接，请检查地址、TLS、账号和密码',platform_control_initialization_failed:'数据库初始化失败，请检查 Schema 权限',platform_control_conflict:'配置已被其他节点更新，请刷新后重试',csrf_rejected:'安全令牌已失效，请重试',platform_control_ready:'平台控制数据库已经启用，请进入平台管理中心',platform_service_unavailable:'数据库 Bootstrap 服务暂时不可用'})[code]||code;
...
test.addEventListener('click',async()=>{if(!form.reportValidity()||!await ensureCurrentPage())return;const request=payload();busy(true);message('正在测试数据库连接…');try{await mutate('/v1/bootstrap/platform-control/test','POST',request);message('连接测试成功，尚未写入配置。')}catch(error){csrf='';message(error.message,true)}finally{busy(false)}});
form.addEventListener('submit',async event=>{event.preventDefault();if(!form.reportValidity()||!await ensureCurrentPage())return;const request=payload();busy(true);message('正在初始化 Schema 并提交配置，请勿关闭页面…');try{status=await mutate('/v1/bootstrap/platform-control','PUT',request);message('配置已提交，正在启动完整平台服务…');setTimeout(refresh,600)}catch(error){csrf='';message(error.message,true);busy(false)}});
refresh();
</script></body></html>`;
}
