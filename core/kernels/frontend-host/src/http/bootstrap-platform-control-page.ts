...
<label><span class="required">密码来源</span><select name="secretKind"><option value="owner-file">Owner-only 文件</option><option value="systemd-credential">systemd Credential</option></select></label>
<label class="secret-file"><span class="required">密码文件</span><input name="secretPath" placeholder="/absolute/path/database-password" spellcheck="false"></label>
<label class="secret-systemd hidden"><span class="required">Credential 名称</span><input name="secretName" placeholder="vastplan-platform-database" spellcheck="false"></label>
<p class="hint">密码不会写入 Platform Control Profile、数据库、日志或错误信息。开发文件必须为 0600；生产使用 systemd Credential。</p>
</div></fieldset>
...
const payload=()=>{const data=new FormData(form),kind=data.get('secretKind'),tls=data.get('tlsMode');return{profile:{schemaVersion:1,generation:Number(status.generation||0)+1,providerId:data.get('providerId'),endpoint:String(data.get('endpoint')||'').trim(),database:String(data.get('database')||'').trim(),schema:String(data.get('schema')||'').trim(),tls:{mode:tls,...(tls==='disable'||!String(data.get('serverName')||'').trim()?{}:{serverName:String(data.get('serverName')).trim()})},username:String(data.get('username')||'').trim(),secretRef:kind==='systemd-credential'?{kind,name:String(data.get('secretName')||'').trim()}:{kind,path:String(data.get('secretPath')||'').trim()},contractRange:'^1.0.0'},expectedGeneration:Number(status.generation||0)}};
...
form.secretKind.addEventListener('change',()=>{const systemd=form.secretKind.value==='systemd-credential';document.querySelector('.secret-file').classList.toggle('hidden',systemd);document.querySelector('.secret-systemd').classList.toggle('hidden',!systemd)});
