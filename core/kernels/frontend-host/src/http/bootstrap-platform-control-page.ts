...
export const bootstrapPlatformControlPageContract = "4";
...
<label><span class="required">数据库</span><input name="database" maxlength="63" required spellcheck="false"></label>
<label><span class="required">Schema</span><input name="schema" value="vastplan_platform" maxlength="63" required spellcheck="false"></label>
...
test.addEventListener('click',async()=>{if(!form.reportValidity()||!await ensureCurrentPage())return;const request=payload();busy(true);message('正在测试数据库连接…');try{await mutate('/v1/bootstrap/platform-control/test','POST',request);message('连接测试成功，尚未写入配置。')}catch(error){csrf='';if(error.code==='database_not_found'&&request.createDatabaseIfMissing)message('目标数据库尚不存在；初始化并启用时将自动创建。');else message(error.message,true)}finally{busy(false)}});
