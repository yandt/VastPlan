...
describe("Platform deployment service revision routes", () => {
  it("maps draft, separation, publication, rollback and audit operations", async () => {
    ...
    expect((await fetch(`${base}/service-revisions/7`, { method: "PUT", headers: server.writeHeaders, body: '{"revisionId":99,"intent":{"id":"intent-b"}}' })).status).toBe(200);
...
async function start(calls: PlatformInvocation[]) {
  ...
  const write = ["createServiceDraft", "updateServiceDraft", "createIntentDraft", "updateIntentDraft", "refreshIntentDraft", "submitServiceDraft", "approveServiceRevision", "publishServiceRevision", "rollbackServiceRevision"];
