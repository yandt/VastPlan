...
describe("Platform deployment service revision routes", () => {
  it("maps draft, separation, publication, rollback and audit operations", async () => {
    ...
    expect((await fetch(`${base}/service-revisions`, { method: "POST", headers: server.writeHeaders, body: '{"composition":{"id":"service-a"}}' })).status).toBe(400);
    expect((await fetch(`${base}/service-revisions`, { method: "POST", headers: server.writeHeaders, body: '{"intent":{"id":"intent-a"}}' })).status).toBe(200);
    expect((await fetch(`${base}/service-revisions/7`, { method: "PUT", headers: server.writeHeaders, body: '{"composition":{"id":"service-b"}}' })).status).toBe(400);
    expect((await fetch(`${base}/service-revisions/7`, { method: "PUT", headers: server.writeHeaders, body: '{"intent":{"id":"intent-b"}}' })).status).toBe(200);
    for (const action of ["refresh-plan", "submit", "approve", "publish", "rollback"]) expect((await fetch(`${base}/service-revisions/7/${action}`, { method: "POST", headers: server.writeHeaders, body: "{}" })).status, action).toBe(200);
    ...
    expect(calls.map(({ operation, payload }) => ({ operation, payload }))).toEqual([
      ...
      { operation: "createIntentDraft", payload: { intent: { id: "intent-a" } } },
      { operation: "updateIntentDraft", payload: { revisionId: 7, intent: { id: "intent-b" } } },
      { operation: "refreshIntentDraft", payload: { revisionId: 7 } },
...
async function start(calls: PlatformInvocation[]) {
  ...
  const write = ["createIntentDraft", "updateIntentDraft", "refreshIntentDraft", "submitServiceDraft", "approveServiceRevision", "publishServiceRevision", "rollbackServiceRevision"];
