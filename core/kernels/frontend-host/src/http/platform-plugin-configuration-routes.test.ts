...
describe("Platform plugin configuration routes", () => {
  it("uses opaque configuration resources and server-owned management targets", async () => {
    ...
    const server = await startPlatformManagementTestServer(
      ...
      ["platform.plugin-configuration.read", "platform.plugin-configuration.write"],
      managementBinding([{ capability: "platform.plugin-configuration", read: ["listDefinitions", "getDefinition", "listCandidates"], write: ["createDraft", "discardDraft"] }]),
    );
    ...
    expect((await fetch(`${base}/candidates`, { method: "POST", headers: server.writeHeaders, body: JSON.stringify({ configurationId: "cfg_aaaaaaaaaaaaaaaaaaaaaaaa", catalogDigest: "b".repeat(64), values: { region: "cn-east" } }) })).status).toBe(200);
    ...
    expect(calls.map(({ capability, operation, payload }) => ({ capability, operation, payload }))).toEqual([
      ...
      { capability: "platform.plugin-configuration", operation: "createDraft", payload: { configurationId: "cfg_aaaaaaaaaaaaaaaaaaaaaaaa", catalogDigest: "b".repeat(64), values: { region: "cn-east" } } },
