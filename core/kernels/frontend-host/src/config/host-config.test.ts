...
describe("parseHostArguments", () => {
  ...
  it("accepts a signed-artifact API Contract Catalog for management routes", () => {
    const config = parseHostArguments([
      "--portal-assets", "bin/portal", "--session-file", "sessions.json", "--allow-insecure-http",
      "--api-contract-catalog", "config/api-contracts.json",
    ], "/srv/vastplan");
  ...
  it("requires a complete, mTLS-protected Addressing configuration", () => {
    ...
    const config = parseHostArguments([
      ...
      "--interaction-logical-service", "platform.interaction-broker", "--platform-control-bootstrap-logical-service", "platform.database",
    ], "/srv/vastplan");
    expect(config.addressing).toMatchObject({
      ...
      platformControlBootstrapLogicalService: "platform.database",
    });
  });
