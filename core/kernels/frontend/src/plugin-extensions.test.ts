...
const graph = parsePortalExtensionGraph({
  points: [{ id: pointID, ownerPluginId: owner, surface: "frontend", contract: "1.0.0", kind: "frontend.page", dispatch: "mount", targets: ["preferences"] }],
  contributions: [{ point: pointID, id: `${contributor}.page`, pluginId: contributor, contract: "^1.0.0", order: 20, descriptor: { pageId: "account.security", groupId: "preferences" } }],
});

const extensionPage: PortalRegisteredPage = {
  id: "account.security", pluginID: contributor, path: "/account/security", title: "Security",
  navigation: { id: "account.security", label: "Security", parentMenuRef: { pluginID: owner, nodeID: "preferences" } },
...
describe("Portal plugin extension graph", () => {
  ...
  it("rejects undeclared or mismatched pages in an owned navigation target", () => {
    ...
    expect(() => validateFrontendPageExtensions([{ ...extensionPage, navigation: { ...extensionPage.navigation!, parentMenuRef: { pluginID: owner, nodeID: "other" } } }], graph)).toThrow(/未按签名 descriptor/);
  });
