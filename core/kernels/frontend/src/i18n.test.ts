...
import { localizeJSONSchema, message, translate } from "@vastplan/portal-ui";
...
describe("Portal locale runtime", () => {
  it("resolves plugin-scoped messages and safely interpolates text values", () => {
    const descriptor = message("com.vastplan.test", "greeting", "你好", { name: "Ada" });
    expect(translate(descriptor, "en-GB", catalogs)).toBe("Hello, Ada");
    expect(translate(message("com.vastplan.test", "missing", "Fallback {name}", { name: "<b>" }), "en-US", catalogs)).toBe("Fallback <b>");
  });
  ...
  it("localizes a cloned JSON Schema through governed JSON pointers", () => {
    ...
    const localized = localizeJSONSchema(source, { "/properties/name/title": message("com.vastplan.test", "greeting", "名称", { name: "field" }) }, (value) => translate(value, "en-US", catalogs));
