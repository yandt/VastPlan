...
const portal = {
  ...
  renderAdapter: { config: { defaultRenderer: "antd", allowedRenderers: ["antd", "arco"], userSelectable: true, rendererOptions: {
    ...
    arco: { allowedThemeTemplates: ["light", "dark"], allowedIconThemes: ["canonical"] },
  } } },
...
describe("PortalAppearanceSession", () => {
  it("persists all appearance choices locally without a network port", () => {
    ...
    session.setRenderer("arco");
    session.setShellTemplate("top-navigation");
    session.setAppearance("arco", { mode: "dark", light: { templateID: "light" }, dark: { templateID: "dark", colors: { primary: "#112233" } } });
    expect(PortalAppearanceSession.open(portal).resolve()).toMatchObject({ rendererID: "arco", shellTemplateID: "top-navigation", themeTemplateID: "dark" });
    PortalAppearanceSession.open(portal).commitPendingRenderer();
    expect(PortalAppearanceSession.open(portal).appearance("arco").dark.colors?.primary).toBe("#112233");
  });

  it("isolates values by user", () => {
    const alice = PortalAppearanceSession.open(portal);
    alice.setRenderer("arco");
