...
func TestPythonPlugin_CrossLanguageInvokeAndFeatureNegotiation(t *testing.T) {
	...
	probe.Env = []string{"PYTHONPATH=" + filepath.Join(root, "sdk/python")}
	...
	manifestRaw, err := os.ReadFile(filepath.Join(root, "plugins/com.vastplan.python-hello/vastplan.plugin.json"))
	if err != nil {
		t.Fatal(err)
	}
	...
	process, err := host.LaunchSpecWithPolicy(ctx, protocolbus.LaunchSpec{
		Command: python,
		Args:    []string{filepath.Join(root, "plugins/com.vastplan.python-hello/backend/main.py")},
		Dir:     root,
		ExtraEnv: []string{
			"PYTHONPATH=" + filepath.Join(root, "sdk/python"),
		},
	}, protocolbus.LaunchPolicy{
		PluginID: "com.vastplan.python-hello", Version: "0.1.0", Contributions: contributions,
