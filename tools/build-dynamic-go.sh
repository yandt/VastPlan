...
fingerprint="$(go run ./tools/dynamicgofingerprint -root .)"
...
go build -trimpath -buildvcs=false \
  ...
  -o "$OUT_DIR/backend-kernel" ./kernels/backend
...
go build -trimpath -buildvcs=false -buildmode=plugin \
  ...
  ./plugins/com.vastplan.foundation.security.bootstrap-policy/dynamic
