...
func TestDeploySampleIsResolverOutput(t *testing.T) {
	...
	for _, id := range []string{"com.vastplan.demo-permission", "com.vastplan.hello-world"} {
		raw, readErr := os.ReadFile(filepath.Join(root, "plugins", id, "vastplan.plugin.json"))
		if readErr != nil {
			t.Fatal(readErr)
		}
	...
	want, err := deploymentv2.ParseFile(filepath.Join(root, "deploy", "cluster.deployment.json"))
	if err != nil {
		t.Fatal(err)
	}
	if !reflect.DeepEqual(resolved, want) {
		t.Fatalf("deploy/cluster.deployment.json 必须由当前双输入精确生成\nresolved=%+v\nwant=%+v", resolved, want)
	}
}
