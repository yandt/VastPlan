...
- 用户信息页面；
- 用户设置分组；
- 仅保存在当前浏览器、修改后即时生效的外观设置页面；页面声明受治理的 `small` Page Body，在两种 Shell 中以 720px 上限居中，不额外创建卡片边框或重复标题，并按“偏好设置 → 浅色/深色主题 → 颜色微调”组织。主题模式使用“跟随系统 / 浅色 / 深色”的可访问视觉预览选择器，模板通过可替换的专用 Select 选择，其他字段采用标签与控件同行的单行式布局。
