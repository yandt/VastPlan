...
插件不拥有 Shell 布局。它被归类为 `foundation.frontend`，因为外观页面需要可信 Host 的本地个性化窄端口和语义 UI 原语；它不作为普通应用插件暴露。Shell 只把统一组合模型中的 `account` 根分组渲染成专用账户入口：标准布局位于左下角，顶部布局位于右上角。本插件在该根分组下默认声明“用户信息”和“偏好设置”二级菜单，再将页面挂为叶子；其他已启用、签名可信插件也可声明各自的平级账户菜单。
...
可信 Catalog 会在发布前校验所有者版本、扩展契约和 descriptor，Portal Runtime 会在同一 Generation 中核对实际页面、贡献插件和导航分组。未声明扩展关系的插件不能直接把页面挂入个人中心拥有的菜单；需要独立账户功能时，应自行在签名 Manifest 中声明账户根菜单的子项。插件之间不得导入源码、共享 React 节点或借用主插件权限。
