...
export function createAPIExposurePage(
  ...
  return defineCollectionPage<APIExposureRow>({
    ...
    collection: {
      ...
      selection: "single",
      actions: [
        { id: "create", label: text("action.create", "新建草稿"), placement: "page.primary", tone: "primary", form: "create" },
        { id: "submit", label: text("action.submit", "提交审批"), placement: "page.secondary", requiresSelection: true, visibleWhen: { pointer: "/status", equals: "Draft" } },
        { id: "approve", label: text("action.approve", "审批"), placement: "page.secondary", requiresSelection: true, visibleWhen: { pointer: "/status", equals: "PendingApproval" } },
        { id: "publish", label: text("action.publish", "发布"), placement: "page.secondary", tone: "primary", requiresSelection: true, visibleWhen: { pointer: "/status", equals: "Approved" } },
        { id: "retire", label: text("action.retire", "退役"), placement: "page.secondary", tone: "danger", requiresSelection: true, confirm: text("confirm.retire", "退役后公开 Route Key 永久墓碑化，不会重新分配。"), visibleWhen: { pointer: "/status", equals: "Published" } },
      ],
