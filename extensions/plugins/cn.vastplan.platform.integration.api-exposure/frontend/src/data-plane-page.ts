...
export function createDataPlaneExposurePage(client: PlatformAdminClient, serviceID: string): CollectionPageDefinition<DataPlaneExposureRow> {
  return defineCollectionPage<DataPlaneExposureRow>({
    ...
    navigation: { id: `platform.data-plane-exposure.${serviceID}`, label: text("dataPlane.navigation", "数据面暴露"), parentMenuRef: { pluginID: "cn.vastplan.platform.integration.api-exposure", nodeID: "integration" }, order: 56 },
    collection: {
