...
func main() {
	...
	// Vault endpoint/key 属于部署配置；token 只可从受控挂载文件读取。
	service, err := credentials.NewWithOptions(credentials.VaultTransit{Address: os.Getenv("VASTPLAN_VAULT_ADDR"), Key: os.Getenv("VASTPLAN_VAULT_TRANSIT_KEY"), TokenFile: os.Getenv("VASTPLAN_VAULT_TOKEN_FILE")}, credentials.ServiceOptions{Maintenance: maintenance})
	if err != nil {
