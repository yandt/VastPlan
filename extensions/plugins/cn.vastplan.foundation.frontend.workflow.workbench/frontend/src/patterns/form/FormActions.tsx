...
import { formValidationReady } from "./form-validation.js";
...
export function FormActions({ form }: { form: FormWorkflowController }) {
  ...
  const validationReady = formValidationReady(form.validation);
  const renderActions = (placement: "footer.start" | "footer.end") => actions.filter((action) => action.placement === placement).map((action) =>
    <ui.Button key={action.id} kind={action.tone === "danger" ? "danger" : action.tone === "primary" ? "primary" : "secondary"} loading={form.runningActionID === action.id} disabled={running || form.loading || action.requiresValid !== false && !validationReady} onClick={() => void form.runAction(action)}>
  ...
  return <ui.Stack direction="row" gap="sm" justify="between" align="center" wrap>
    ...
    {steps && sectionIndex < sections.length - 1
      ...
      : <ui.Button kind="primary" loading={form.submitting} disabled={running || form.loading || !validationReady} onClick={() => void form.submit()}>{i18n.text(form.definition?.workflow.submitLabel ?? message(namespace, "action.submit", "提交"))}</ui.Button>}
    </ui.Stack>
  </ui.Stack>;
}
