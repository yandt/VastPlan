...
export function CollectionOverlayWorkflow({ definition, selected, open, onClose }: { definition?: WorkbenchOverlayDefinition; selected: readonly CollectionRow[]; open: boolean; onClose(): void }) {
  ...
  return definition.surface === "drawer" ? <ui.Drawer open={open} title={i18n.text(definition.title)} size={definition.size} width={definition.width} onClose={onClose}>{body}</ui.Drawer> : <ui.Dialog open={open} title={i18n.text(definition.title)} size={definition.size} width={definition.width} onClose={onClose}>{body}</ui.Dialog>;
}
