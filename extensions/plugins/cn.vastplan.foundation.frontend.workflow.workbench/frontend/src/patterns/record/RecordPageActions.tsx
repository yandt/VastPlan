...
export function RecordPageActions({ page }: { page: RecordPageDefinition }) {
  const ui = usePortalUI();
  const i18n = usePortalI18n();
  const controller = useMemo(() => pageActionController(page), [page]);
  const snapshot = useSyncExternalStore(controller.subscribe, controller.getSnapshot, controller.getSnapshot);
  const [overflowOpen, setOverflowOpen] = useState(false);
  const actions = (page.actions ?? []).filter((action) => (action.placement === "page.primary" || action.placement === "page.secondary") && snapshot.visibleActionIDs.has(action.id));
