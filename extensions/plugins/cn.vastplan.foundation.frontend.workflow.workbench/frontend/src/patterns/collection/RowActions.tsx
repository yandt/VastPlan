...
export function RowActions({ actions, row, onRunAction }: {
  ...
  return <ui.Stack direction="row" gap="sm" align="center" justify="center">
    ...
    {overflow.length === 0 ? null : <ui.Popover
      ...
      trigger={(props) => <span ref={props.ref} aria-expanded={props["aria-expanded"]} aria-controls={props["aria-controls"]} onClick={props.onClick} onKeyDown={props.onKeyDown} style={{ display: "inline-flex", lineHeight: 0 }}><ui.IconButton icon="more" label={moreLabel} size="sm" /></span>}
    ><ui.Menu size="sm" variant="action" items={overflow.map((action) => ({ id: action.id, label: i18n.text(action.label), icon: <ui.Icon name={action.icon} />, disabled: false }))} onSelect={(id) => {
