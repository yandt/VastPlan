...
export function RowActions({ actions, row, onRunAction }: {
  ...
  return <ui.Stack direction="row" gap="xs" align="center" justify="center">
    {direct.map((action) => <ui.IconButton key={action.id} icon={action.icon} label={i18n.text(action.label)} tone={action.tone === "danger" ? "danger" : action.tone === "primary" ? "primary" : "normal"} onClick={() => onRunAction(action)} />)}
    ...
    ><ui.Stack direction="row" gap="xs" align="center" justify="center" wrap>{overflow.map((action) => <ui.IconButton key={action.id} icon={action.icon} label={i18n.text(action.label)} tone={action.tone === "danger" ? "danger" : action.tone === "primary" ? "primary" : "normal"} onClick={() => { onRunAction(action); setOverflowOpen(false); }} />)}</ui.Stack></ui.Popover>}
  </ui.Stack>;
}
