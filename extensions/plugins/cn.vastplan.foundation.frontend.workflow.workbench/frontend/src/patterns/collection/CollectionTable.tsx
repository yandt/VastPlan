import type { ActionSpec, CollectionDensity, CollectionSpec, ColumnSpec } from "@vastplan/ui-contract";
...
export function CollectionTable({ collection, columns, rows, selectedKeys, loading, density, keyOf, onSelectionChange, onRunAction }: {
  ...
  const renderValue = (column: ColumnSpec, value: unknown) => {
  ...
  const tableColumns = [
    ...visibleColumns.map((column) => ({ key: column.key, title: i18n.text(column.label), width: column.minWidth, render: (value: unknown) => renderValue(column, value) })),
