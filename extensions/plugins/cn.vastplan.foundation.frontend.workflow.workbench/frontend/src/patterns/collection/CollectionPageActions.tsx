...
export function CollectionPageActions({ page }: { page: CollectionPageDefinition }) {
  ...
  return <ui.Stack direction="row" gap="xs" align="center">
    {direct.map((action) => <ui.IconButton key={action.id} icon={action.icon} label={i18n.text(action.label)} disabled={disabled(action)} tone={action.tone === "danger" ? "danger" : action.tone === "primary" ? "primary" : "normal"} onClick={() => run(action)} />)}
    ...
    ><ui.Menu items={overflow.map((action) => ({ id: action.id, label: i18n.text(action.label), icon: <ui.Icon name={action.icon} />, disabled: disabled(action) }))} onSelect={(id) => {
