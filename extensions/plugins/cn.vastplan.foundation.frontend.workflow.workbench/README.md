...
功能插件只能通过 `@vastplan/workbench-sdk` 提交 Collection、Workspace、Record 或 Form 页面定义；数据加载和动作处理仍由功能插件提供，视觉与状态机由本插件统一处理。
...
这里的 `CollectionTable` / `CollectionCards` 是集合工作流的受控呈现区，不是具体 UI 框架的基础组件。基础表格和卡片分别通过 `ui.Table` / `ui.DataCard` 由渲染适配器提供。`patterns/record/` 负责 RecordDetail、MasterDetail、TreeDetail 的详情加载、主从选择、树边界、URL 恢复、窄屏与页内编辑保护；`patterns/action/` 是 Collection/Record 共用的 Page Header 动作桥，并由 `ActionExecutionFeedback` 统一管理一次动作的进行中状态：立即状态提示、650ms 后的等待 Dialog、同页去重和卸载取消。功能插件只实现 `runAction`，不得各自实现等待遮罩。

`patterns/form/` 把动态表单拆为 `useFormWorkflow`（加载、脏状态、校验和提交生命周期）、`FormContent`（纯 Schema 表单）、`FormActions`（步骤与提交操作）、默认 `FormDialog` 组合和显式 `FormPagePanel`。功能插件省略 `workflow.surface` 时统一打开 Dialog；只有独立表单页和记录页内编辑器显式使用 `surface: "page"`，表单不再支持 Drawer。调用方只提交数据定义：`preset` 选择 compact/standard/comfortable/guided；所有业务表单的 Label 与输入控件默认同行，控件默认 `controlAlignment: "end"`，特殊场景可显式选择 `start`；`layout: "vertical"` 只控制区块排列，不再隐式切换为上下 Label。只有明确需要时才可显式声明 `labelPlacement: "stacked"`，筛选组件继续显式使用 `inside-inline`。根表单和 Section 均支持 1–4 列及合计 100 的 `columnWidths` 百分比权重。`form-validation.ts` 统一处理提交与 `requiresValid` 辅助动作：按钮接收点击意图后基于当前值执行完整校验，失焦校验的瞬时状态不能吞掉第一次点击。`beforeSubmit → submit → afterSubmit` 顺序由 Workbench 统一执行，字段错误规范化后显示在对应输入框下方，表单级错误才进入全局错误区。`secret-material.ts` 统一识别和丢弃一次性秘密，禁止材料进入 baseline 或在提交/关闭后滞留。Render Adapter 只负责把同一语义映射为各框架的表单、分栏、列表和树，不允许功能插件直接组合基础 UI 组件。
...
const form: WorkbenchFormDefinition = {
  ...
  async afterSubmit() { await auditRefresh(); },
};
```
