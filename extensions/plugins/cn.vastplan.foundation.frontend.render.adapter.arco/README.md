...
`cn.vastplan.foundation.frontend.render.adapter.arco` 是 Portal 的首个第一方设计系统插件。它以 Arco Design 实现 `@vastplan/ui-primitives` 的 `1.x` 契约，提供 PortalShell、布局、菜单/面包屑/标签/命令面板、弹窗/抽屉、动态表单、表格/描述/分页、状态反馈、语义图标与主题 token。

它不是业务页面，也不是门户内核的一部分。每个 Portal 发布版本只能选择一个 `ui.render.adapter`；可在另一个 Portal 中选择未来的 MUI 等适配器，但不能在同一 Portal 中混用。
