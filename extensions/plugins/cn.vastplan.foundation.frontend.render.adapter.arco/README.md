...
`cn.vastplan.foundation.frontend.render.adapter.arco` 是统一 Render Adapter 的内部 Arco Renderer 源码包。它实现 `@vastplan/ui-primitives` 的语义组件、动态表单与主题映射，但不再单独贡献 `ui.render.adapter` 或作为 Portal 基础插件被选择。
