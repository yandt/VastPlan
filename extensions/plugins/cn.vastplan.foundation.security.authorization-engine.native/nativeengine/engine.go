// Package nativeengine implements the first-party Authorization Engine
// Provider. It belongs to the plugin that owns its state and lifecycle; it is
// deliberately not part of the generic plugin SDK.
...
func (e *Engine) Evaluate(request authorizationv1.EngineEvaluateRequest) (authorizationv1.EngineEvaluateResult, error) {
	...
	evaluation := authorizationeval.Evaluate(prepared.policy, request.Input, now)
