...
export function migrationPage(client: PlatformAdminClient, id: string, path: string): CollectionPageDefinition<Row> {
  return defineCollectionPage<Row>({
    ...
    navigation: { id, label: text("page.migration.navigation", "存储迁移"), parentMenuRef: { pluginID: "cn.vastplan.platform.artifacts.repository", nodeID: "artifacts" }, order: 54 },
    ...
    collection: {
      ...
      actions: [
        ...
        { id: "finalize", label: text("action.migration.finalize", "确认迁移"), icon: "success", placement: "record.row", confirm: text("confirm.migration.finalize", "确认结束双写观察期？完成后不能再回滚。"), requiredPermissions: ["platform.artifacts.migrate"], visibleWhen: { pointer: "/canFinalize", equals: true } },
        { id: "release", label: text("action.migration.release", "释放旧 Volume"), icon: "remove", placement: "record.row", tone: "danger", confirm: text("confirm.migration.release", "确认配置已指向目标 Volume，并隔离释放旧 Volume？"), requiredPermissions: ["platform.artifacts.migrate"], visibleWhen: { pointer: "/canRelease", equals: true } },
      ],
