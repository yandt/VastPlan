...
function capacityPage(client: PlatformAdminClient, id: string, path: string): CollectionPageDefinition<Row> {
  return defineCollectionPage<Row>({
    ...
    navigation: { id, label: text("page.capacity.navigation", "容量与配额"), parentMenuRef: { pluginID: "cn.vastplan.platform.artifacts.repository", nodeID: "artifacts" }, order: 51 },
    collection: {
...
function garbageCollectionPage(client: PlatformAdminClient, id: string, path: string): CollectionPageDefinition<Row> {
  return defineCollectionPage<Row>({
    ...
    navigation: { id, label: text("page.gc.navigation", "垃圾回收"), parentMenuRef: { pluginID: "cn.vastplan.platform.artifacts.repository", nodeID: "artifacts" }, order: 53 },
    pageActions: [
