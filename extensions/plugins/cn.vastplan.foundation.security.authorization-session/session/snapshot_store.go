...
func (s FileSnapshotStore) Load() (authorizationv1.SignedPolicySnapshot, error) {
	if err := securePath(s.SnapshotPath, false); err != nil {
		return authorizationv1.SignedPolicySnapshot{}, err
	}
	if err := securePath(s.TrustPath, false); err != nil {
		return authorizationv1.SignedPolicySnapshot{}, err
	}
...
func securePath(path string, ownerOnly bool) error {
	if !filepath.IsAbs(path) || filepath.Clean(path) != path {
		return errors.New("Authorization Session 文件必须是规范绝对路径")
	}
	info, err := os.Lstat(path)
	if err != nil || !info.Mode().IsRegular() || info.Mode()&os.ModeSymlink != 0 || info.Mode().Perm()&0o022 != 0 || (ownerOnly && info.Mode().Perm()&0o077 != 0) {
		return errors.New("Authorization Session 文件权限不安全")
	}
	return nil
}
