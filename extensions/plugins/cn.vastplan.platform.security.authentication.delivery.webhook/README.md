...
- 插件同时实现 `configuration.resource.v1`，以租户隔离、Active CAS 和 prepare/commit/abort/status 管理 Profile；每个租户使用独立的 fenced Shared State 值，生产不接受 `stateFile`，Store 不可用或 revision 冲突时 fail-closed。
- 当前仍按 `leader + external-shared` 运行，以保留凭证探测、Profile 提交和旧引用退休的单一工作流所有者；Shared State 解决耐久与接管，不等于可以无审计地改成 active-active。
