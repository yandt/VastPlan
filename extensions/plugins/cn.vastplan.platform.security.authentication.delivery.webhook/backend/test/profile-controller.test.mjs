...
class MemoryProfileStateStore {
  constructor() { this.values = new Map(); }
  async load(context) { const value = this.values.get(context.tenant_id); return value === undefined ? undefined : structuredClone(value); }
  async save(state, context) { this.values.set(context.tenant_id, structuredClone(state)); }
}

test("Webhook Profile Controller persists tenant-isolated resources and retires replaced refs", async () => {
  const store = new MemoryProfileStateStore(), profiles = new Map();
  ...
  let controller = new WebhookProfileController({collectionId,store,materialLease,profiles});
  ...
  controller = new WebhookProfileController({collectionId,store,materialLease,profiles:new Map()});
...
test("Webhook Profile Controller rejects stale CAS and invalid secret material", async () => {
  const controller=new WebhookProfileController({collectionId,store:new MemoryProfileStateStore(),profiles:new Map(),materialLease:{async withMaterial(_ref,_tenant,_signal,use){return use(Buffer.from("short"));}}});
