...
func TestBootstrapPolicyProtectsSettingsWrites(t *testing.T) {
	...
	if got := evaluateWriteGuard(pluginContext("cn.vastplan.platform.security.credentials", true), write); got.Decision != extpoint.DecisionDeny {
		t.Fatalf("插件不能继承管理员 principal 写权限: %+v", got)
	}
	if got := evaluateWriteGuard(pluginContext("cn.vastplan.foundation.security.reader", false), read); got.Decision != extpoint.DecisionAbstain {
		t.Fatalf("只读操作应交给基线: %+v", got)
	}
	if got := evaluateWriteGuard(pluginContext("cn.vastplan.foundation.security.reader", false), extpoint.PermissionRequest{
...
func TestBootstrapBaselineUsesVerifiedNamespaceClassification(t *testing.T) {
	...
	for _, id := range []string{"cn.vastplan.foundation.security.credentials-bootstrap", "cn.vastplan.platform.data.database"} {
		if got := evaluateBaseline(pluginContext(id, false), read); got.Decision != extpoint.DecisionAllow {
			t.Fatalf("基础层首方插件 %s 应允许只读: %+v", id, got)
		}
	}
	for _, id := range []string{"cn.vastplan.product.agent.studio", "cn.vastplan.example.security.demo", "com.example.foundation.security.fake"} {
