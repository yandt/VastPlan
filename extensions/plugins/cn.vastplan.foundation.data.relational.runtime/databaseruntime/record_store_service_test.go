...
func TestRecordStoreSynchronizesModelsOnlyFromTrustedSystem(t *testing.T) {
	...
	request := recordstorev1.SyncModelsRequest{Generation: 1, Models: []recordstorev1.SignedModel{
		recordstore.EncodeSignedModel("cn.vastplan.application.orders", ref, document),
	}}
	...
	system := &contractv1.CallContext{Caller: &contractv1.Caller{Kind: contractv1.CallerKind_CALLER_KIND_SYSTEM, Id: "composition-controller"}}
	...
	if err != nil || accepted.GetStatus() != contractv1.CallResult_STATUS_OK || string(payload) != `{"generation":1,"models":1}` {
...
func TestRecordStoreRejectsCrossPluginModelUseBeforeDatabaseAccess(t *testing.T) {
	...
	_, _ = service.recordModels.Replace(recordstorev1.SyncModelsRequest{Generation: 1, Models: []recordstorev1.SignedModel{recordstore.EncodeSignedModel("cn.vastplan.application.orders", ref, document)}})
