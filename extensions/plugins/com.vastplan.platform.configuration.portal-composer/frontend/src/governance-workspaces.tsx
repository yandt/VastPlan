...
const namespace = "com.vastplan.platform.configuration.portal-composer";
...
export const governanceMessages = {
  "zh-CN": {
    ...
    "governance.column.revision": "Revision", "governance.column.profile": "Profile", "governance.column.portal": "Portal", "governance.column.status": "状态", "governance.column.activation": "Activation", "governance.column.time": "时间", "governance.column.action": "操作", "governance.column.inputs": "输入",
  ...
  "en-US": {
    ...
    "governance.column.revision": "Revision", "governance.column.profile": "Profile", "governance.column.portal": "Portal", "governance.column.status": "Status", "governance.column.activation": "Activation", "governance.column.time": "Time", "governance.column.action": "Action", "governance.column.inputs": "Inputs",
...
const profileSchema: FormSchema = { id: "portal-profile-editor.v1", schema: { $schema: jsonSchemaDialect, type: "object", additionalProperties: false, required: ["name", "layoutID", "pageBodyWidth", "navigationGroups"], properties: {
  ...
  layoutID: { type: "string", title: "布局插件", oneOf: [{ const: "com.vastplan.foundation.frontend.layout.standard", title: "标准侧栏布局" }, { const: "com.vastplan.foundation.frontend.layout.top-navigation", title: "顶部导航布局" }] },
...
function profileValue(profile: PortalPlatformProfile): ProfileValue { return { name: profile.id, layoutID: profile.layout.id, pageBodyWidth: profile.layout.config?.pageBodyWidth === "contained" ? "contained" : "fluid", navigationGroups: Array.isArray(profile.composition.config?.navigationGroups) ? profile.composition.config.navigationGroups : [] }; }
function buildProfile(base: PortalPlatformProfile, value: Record<string, unknown>, newRevision: boolean): PortalPlatformProfile {
  const oldLayoutID = base.layout.id; const layoutID = String(value.layoutID); const layout = { ...base.layout, id: layoutID, config: { ...(base.layout.config ?? {}), pageBodyWidth: value.pageBodyWidth as JSONValue } };
  const plugins = base.plugins.filter((ref) => ref.id !== oldLayoutID && ref.id !== layoutID).concat([{ id: layoutID, version: base.layout.version, channel: base.layout.channel }]);
  return { ...base, id: String(value.name), revision: newRevision ? base.revision + 1 : base.revision, composition: { ...base.composition, config: { ...(base.composition.config ?? {}), navigationGroups: value.navigationGroups as JSONValue } }, layout, plugins };
}
