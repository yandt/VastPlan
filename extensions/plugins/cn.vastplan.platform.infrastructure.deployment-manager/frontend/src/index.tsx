...
export function createDeploymentPage(client: PlatformAdminClient, serviceID: string, path: string, title: ReturnType<typeof localizedMessage>): CollectionPageDefinition<DeploymentRow> {
  ...
  return defineCollectionPage<DeploymentRow>({
    ...
    collection: { id: `platform.deployment.${serviceID}`, title, view: "table", query: { mode: "page", defaultPageSize: 20, pageSizeOptions: [20, 50, 100] },
      ...
      actions: [
        { id: "create", label: localizedMessage(namespace,"action.new","新建服务草稿"), icon: "add", placement: "page.primary", tone: "primary", form: "create" },
        { id: "edit", label: localizedMessage(namespace,"action.edit","编辑"), icon: "edit", placement: "page.secondary", requiresSelection: true, form: "edit", visibleWhen: { pointer: "/status", equals: "Draft" } },
        { id: "submit", label: localizedMessage(namespace,"action.submit","提交审批"), icon: "upload", placement: "page.secondary", requiresSelection: true, confirm: localizedMessage(namespace,"confirm.submit","提交后将重新执行可信解析，且不能继续编辑。"), visibleWhen: { pointer: "/status", equals: "Draft" } },
        { id: "approve", label: localizedMessage(namespace,"action.approve","批准"), icon: "success", placement: "page.secondary", requiresSelection: true, tone: "primary", confirm: localizedMessage(namespace,"confirm.approve","审批人与提交人必须不同。"), visibleWhen: { pointer: "/status", equals: "PendingApproval" } },
        { id: "publish", label: localizedMessage(namespace,"action.publish","发布"), icon: "publish", placement: "page.secondary", requiresSelection: true, tone: "primary", confirm: localizedMessage(namespace,"confirm.publish","发布后 Controller 会把副本调度到符合条件的 Node Agent。"), visibleWhen: { all: [{ pointer: "/status", in: ["Approved", "Publishing"] }, { pointer: "/configurationCandidateId", exists: false }] } },
        { id: "rollback", label: localizedMessage(namespace,"action.rollback","回滚到此版本"), icon: "refresh", placement: "page.secondary", requiresSelection: true, tone: "danger", confirm: localizedMessage(namespace,"confirm.rollback","回滚会用历史应用组合创建并发布一个新的单调 revision。"), visibleWhen: { all: [{ pointer: "/status", equals: "Published" }, { pointer: "/active", equals: false }] } },
        { id: "preview", label: localizedMessage(namespace,"action.preview","最终部署预览"), icon: "info", placement: "page.secondary", requiresSelection: true, overlay: "preview" },
        { id: "audit", label: localizedMessage(namespace,"action.audit","审计记录"), icon: "search", placement: "page.secondary", requiresSelection: true, overlay: "audit" },
      ] },
