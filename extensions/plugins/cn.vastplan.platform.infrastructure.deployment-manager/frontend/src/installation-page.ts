...
export function createPluginInstallationPage(deployment: PlatformAdminClient, repository: PlatformAdminClient | undefined, portalID: string, serviceID: string, path: string, title: LocalizedText): CollectionPageDefinition<InstallationRow> {
  ...
  return defineCollectionPage<InstallationRow>({
    ...
    navigation: { id: `platform.plugin-installations.${serviceID}`, label: title, parentMenuRef: { pluginID: "cn.vastplan.platform.infrastructure.deployment-manager", nodeID: "operations" }, managementServiceID: serviceID, order: 12 },
