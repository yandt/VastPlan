...
func allowedKernelCallback(c *v1.CallContext, request extpoint.PermissionRequest) bool {
	...
	switch c.Caller.Id {
	case "com.vastplan.platform.configuration.global-settings", "com.vastplan.platform.security.credentials":
...
func pluginMetadataReadAllowed(id, capability, operation string) bool {
	if len(id) < len("com.vastplan.platform.") || id[:len("com.vastplan.platform.")] != "com.vastplan.platform." {
		return false
	}
