# Artifact Assessment Provider Runtime

该插件内部 Go 包实现 ADR-0138/0139 的扫描 Provider 能力，不属于 SDK 或内核。它负责：
...
本包只由该插件、受控 CI 工具和 E2E 使用。制品正文不进入协议总线，签名私钥只通过面向精确首方 Provider 的 Material Lease 在同步回调内使用。
