...
- `VASTPLAN_SEED_ACCESS_ALLOW_INITIAL_SETUP=true` 只能由可信组合根在首次部署时注入。它允许 `Uninitialized` 状态下发一次性 `enrollment` 表单；写入成功后立即失效，不能作为密码重置能力。生产默认不设置该变量。

## 当前实现

- Seed 状态机、Provider 配置/真实身份验证/交接 Seal、企业启用和恢复租约；
- 0600、无符号链接、跨进程锁、CAS generation、临时写入、fsync 与原子 rename 的 File Store；
- Argon2id Seed Operator verifier；初始化和认证统一裁剪密码首尾 Unicode 空白，内部空白仍属于密码内容；
- Seed Provider 对 Portal/Authorization 只投影固定非 PII 主体 `seed.operator`。真实管理员账号仅用于 Seed Authority 校验，不写入授权快照、Assertion Session 或业务审计主体；
...
常规首次安装使用受限的 `enrollment` 认证 Step，而不是脚本传递密码。本机初始化和恢复仍可使用无网络监听的 `engineering/tools/seedaccessctl` 作为紧急修复手段。密码文件是只包含密码原文的普通文本文件，不是 JSON/YAML；密码与恢复证明只从 owner-only 普通文件读取，不能放入命令行参数。密码首尾的空格、Tab、CR/LF 等 Unicode 空白由统一密码协议裁剪，内部空白保留；生产不得以直接编辑 JSON 替代这些入口。
