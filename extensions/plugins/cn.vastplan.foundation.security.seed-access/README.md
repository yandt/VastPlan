...
本机初始化和恢复使用无网络监听的 `engineering/tools/seedaccessctl`。密码文件是只包含密码原文的普通文本文件，不是 JSON/YAML；密码与恢复证明只从 owner-only 普通文件读取，不能放入命令行参数。密码首尾的空格、Tab、CR/LF 等 Unicode 空白由统一密码协议裁剪，内部空白保留；管理 Workbench 在后续控制面阶段接入。生产不得以直接编辑 JSON 替代这些入口。
