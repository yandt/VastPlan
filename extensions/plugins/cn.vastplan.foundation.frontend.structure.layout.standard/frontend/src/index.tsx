...
export function StandardShell(props: UIShellProps) {
  ...
  const brand = <PortalBrand {...branding} className="vp-brand vp-brand-compact" markClassName="vp-brand-mark" logoClassName="vp-brand-logo" fullName focusable />;
...
function RailButton({ collection, selected, controls, buttonRef, onSelect, onOpen }: {
  ...
  return <ui.Tooltip title={label} placement="right"><button ref={buttonRef} type="button" className="vp-rail-button" data-selected={selected || undefined} aria-label={label} aria-pressed={selected} aria-controls={controls} onClick={onSelect} onKeyDown={(event) => {
...
function SecondLevelMenu({ collection, composition, onNavigate, onLogout }: { collection: PortalNavigationCollection; composition: UIShellProps["composition"]; onNavigate(id: string): void; onLogout?(): Promise<void> }) {
  ...
  const updateExpandedGroups = (ids: readonly string[]) => setOpenGroups(() => {
    ...
    for (const id of ids) {
      ...
      const childID = id.slice("group:".length);
      if (childIDs.has(childID)) next.add(childID);
    }
...
.vp-shell-frame{display:flex;flex:1;min-height:0;min-width:0}.vp-desktop-navigation{display:flex;flex:0 0 auto;min-height:0}.vp-navigation-rail{width:var(--vp-shell-rail-width);flex:0 0 var(--vp-shell-rail-width);min-height:0;display:grid;grid-template-rows:auto minmax(0,1fr) auto;background:var(--vp-shell-surface);border-right:1px solid var(--vp-shell-border)}.vp-navigation-start,.vp-navigation-center,.vp-navigation-end{display:flex;flex-direction:column;align-items:center;gap:8px;padding:8px}.vp-navigation-center{overflow-y:auto;overscroll-behavior:contain;scrollbar-width:thin}.vp-navigation-start{box-sizing:border-box;height:var(--vp-shell-bar-height);min-height:var(--vp-shell-bar-height);justify-content:center;overflow:visible;border-bottom:1px solid var(--vp-shell-border);position:relative;z-index:2}.vp-navigation-end{border-top:1px solid var(--vp-shell-border);max-height:40vh;overflow-y:auto}
.vp-rail-button{width:var(--vp-shell-touch-minimum);height:var(--vp-shell-touch-minimum);flex:0 0 var(--vp-shell-touch-minimum);display:grid;place-items:center;border:0;border-radius:10px;background:transparent;color:var(--vp-shell-muted);cursor:pointer}.vp-rail-button:hover{background:var(--vp-shell-hover);color:var(--vp-shell-primary)}.vp-rail-button[data-selected]{background:var(--vp-shell-selected);color:var(--vp-shell-primary)}.vp-rail-button:focus-visible,.vp-mobile-menu-button:focus-visible{outline:var(--vp-shell-focus-width) solid var(--vp-shell-focus);outline-offset:2px}
...
@media (max-width:1199px){.vp-navigation-panel{width:var(--vp-shell-navigation-compact-width);flex-basis:var(--vp-shell-navigation-compact-width)}.vp-brand-compact strong{width:calc(var(--vp-shell-rail-width) + var(--vp-shell-navigation-compact-width))}.vp-page{padding:var(--vp-page-content-start) 20px 20px}.vp-page-header{padding-left:20px;padding-right:20px}}
