...
func TestResolveInjectsPlatformRenderAdapterAndLocksInputs(t *testing.T) {
	...
	if resolved.Revision != 7 || len(resolved.Plugins) != 6 || resolved.RuntimeEngine.ID != profile.RuntimeEngine.ID || resolved.RenderAdapter.ID != profile.RenderAdapter.ID || resolved.Shell.ID != profile.Shell.ID || resolved.Workbench.ID != profile.Workbench.ID {
	...
	if len(resolved.Shell.Config.NavigationOverrides) == 0 || resolved.Shell.Config.DefaultTemplate != "standard" {
...
func testProfile() frontendcompositionv1.PlatformProfile {
	...
	accountCenter := frontendcompositionv1.PluginRef{ID: "cn.vastplan.foundation.frontend.identity.account-center", Version: "1.0.0", Channel: "stable"}
	order := 10
	return frontendcompositionv1.PlatformProfile{Document: compositioncommonv1.Document{Version: 1, Revision: 1, ID: "portal-default"}, Target: compositioncommonv1.Target{Kernel: compositioncommonv1.KernelFrontend}, RuntimeEngine: frontendcompositionv1.RuntimeEngine{PluginRef: engine, EngineContract: "^1.0.0", Family: "react"}, RenderAdapter: frontendcompositionv1.RenderAdapter{PluginRef: design, UIContract: contractregistry.FrontendUIContractRange, Config: frontendcompositionv1.RenderAdapterConfig{DefaultRenderer: "antd", AllowedRenderers: []string{"antd"}, UserSelectable: false, RendererOptions: map[string]frontendcompositionv1.RendererOptions{"antd": {ThemeTemplate: "light", IconTheme: "canonical"}}}}, Shell: frontendcompositionv1.Shell{PluginRef: shell, UIContract: contractregistry.FrontendUIContractRange, Config: frontendcompositionv1.ShellConfig{NavigationConfig: frontendcompositionv1.NavigationConfig{NavigationOverrides: []frontendcompositionv1.NavigationOverride{{Target: "cn.vastplan.product.frontend.admin/main", Order: &order}}}, DefaultTemplate: "standard", AllowedTemplates: []string{"standard", "top-navigation"}, UserSelectable: true, TemplateOptions: map[string]map[string]any{"standard": {}}}}, Workbench: frontendcompositionv1.Workbench{PluginRef: workbench, UIContract: contractregistry.FrontendUIContractRange}, AccountCenter: &accountCenter, Plugins: []frontendcompositionv1.PluginRef{engine, design, shell, workbench, accountCenter}, Security: frontendcompositionv1.SecurityPolicy{FirstPartyOnly: true, RequireIntegrity: true}}
}
