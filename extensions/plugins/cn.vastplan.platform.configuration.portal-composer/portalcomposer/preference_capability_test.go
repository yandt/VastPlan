...
func TestPreferenceContributionRequiresPortalBFFAndDerivesSubject(t *testing.T) {
	service, err := New(filepath.Join(t.TempDir(), "composer.json"), acceptingCatalog{})
	if err != nil {
		t.Fatal(err)
	}
	if err := service.configurePreferenceStore(filepath.Join(t.TempDir(), "preferences.json")); err != nil {
		t.Fatal(err)
	}
	service.catalogConfigured = true
...
func TestPreferenceContributionReturnsStableConflict(t *testing.T) {
	...
	if result, _, err := handler(context.Background(), nil, callContext, first); err != nil || result.GetStatus() != contractv1.CallResult_STATUS_OK {
	...
	result, _, err := handler(context.Background(), nil, callContext, stale)
