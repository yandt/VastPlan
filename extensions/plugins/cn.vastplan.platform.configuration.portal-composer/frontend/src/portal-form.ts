...
export const portalConfigurationSchema: FormSchema = {
  ...
  schema: {
    ...
    properties: {
      ...
      navigationPlacements: {
        type: "array", title: "导航语义映射", items: {
          type: "object", additionalProperties: false, required: ["semanticID", "groupID"],
          properties: {
            semanticID: { type: "string", title: "功能语义 ID", pattern: "^[A-Za-z0-9][A-Za-z0-9._-]{0,159}$" },
            groupID: { type: "string", title: "目标菜单组 ID", pattern: "^[A-Za-z0-9][A-Za-z0-9._-]{0,159}$" },
          },
        },
      },
  ...
  uiSchema: {
    ...
    navigationGroups: { "ui:help": "菜单树属于当前 Portal 的 Platform Profile；同一 Portal 管理的多个服务共享，其他 Portal 可独立配置。" },
    navigationPlacements: { "ui:help": "将插件声明的 semanticID 映射到当前菜单分组；未映射时使用插件默认位置。" },
  ...
  localization: {
    ...
    "/properties/navigationGroups/title": message(namespace, "form.navigationGroups", "导航分组"),
    "/properties/navigationPlacements/title": message(namespace, "form.navigationPlacements", "导航语义映射"),
...
export function configurationToForm(portalId: string, configuration: PortalConfiguration): Record<string, unknown> {
  ...
  return {
    ...
    navigationPlacements: Array.isArray(configuration.platform.shell.config.navigationPlacements) ? configuration.platform.shell.config.navigationPlacements : [],
...
export function buildPortalConfiguration(base: PortalConfiguration, value: Readonly<Record<string, unknown>>, platformDefault: PortalPlatformProfile = base.platform): PortalConfiguration {
  ...
  return {
    platform: {
      ...
      shell: { ...base.platform.shell, config: { ...base.platform.shell.config, defaultTemplate, navigationGroups: jsonArray(value.navigationGroups), navigationPlacements: jsonArray(value.navigationPlacements), templateOptions } },
    },
    application: {
      ...base.application,
