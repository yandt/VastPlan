...
const revisionActions = (prefix: string) => [
  ...
  { id: `${prefix}.preview`, label: message(namespace, "action.preview", "查看内容"), icon: "info" as const, placement: "record.row" as const, overlay: "preview" },
];
...
export function createProfilePage(client: PortalControlClient): CollectionPageDefinition<ProfileRow> {
  const form = (id: "create" | "edit"): WorkbenchFormDefinition<ProfileRow> => ({
    ...
    async submit({ value, selected }, signal) {
      ...
      const profile = buildProfile(base.profile, value, id === "create");
      if (id === "create") await client.createProfile(profile); else await client.updateProfile(base.id, profile);
    },
  });
  return defineCollectionPage<ProfileRow>({
    ...
    collection: { id: "portal-profiles", title: message(namespace, "page.profiles.title", "Portal 平台基线"), view: "table", query: pageQuery(), filterPanel: { fields: revisionFilters("profile") }, columns: [{ key: "id", label: "Revision", format: "number", defaultVisible: true }, { key: "profileId", label: message(namespace, "column.profile", "Profile"), defaultVisible: true, minWidth: 180 }, statusColumn(), updatedColumn()], selection: "single", preferences: { allowedColumns: ["id", "profileId", "status", "updatedAt"], density: true }, actions: revisionActions("profile") },
...
export function createBindingPage(client: PortalControlClient): CollectionPageDefinition<BindingRow> {
  ...
  return defineCollectionPage<BindingRow>({
    ...
    collection: { id: "portal-bindings", title: message(namespace, "page.bindings.title", "Portal 服务绑定"), view: "table", query: pageQuery(), filterPanel: { fields: revisionFilters("portalId") }, columns: [{ key: "id", label: "Revision", format: "number", defaultVisible: true }, { key: "portalId", label: message(namespace, "column.portal", "Portal"), defaultVisible: true, minWidth: 180 }, { key: "profileRevisionId", label: message(namespace, "column.profile", "Profile"), format: "number", defaultVisible: true }, statusColumn(), updatedColumn()], selection: "single", preferences: { allowedColumns: ["id", "portalId", "profileRevisionId", "status", "updatedAt"], density: true }, actions: [{ id: "binding.create", label: message(namespace, "action.createBinding", "新建 Binding"), icon: "add", placement: "record.row", tone: "primary", form: "create" }, ...revisionActions("binding")] },
...
function isKnownRenderer(value: unknown): value is KnownRenderer {
  return value === "antd" || value === "arco" || value === "mui";
}
...
export const profileSchema: FormSchema = { id: "portal-profile-editor.v1", schema: { $schema: jsonSchemaDialect, type: "object", additionalProperties: false, required: ["name", "defaultRenderer", "allowedRenderers", "userSelectableRenderer", "defaultTemplate", "pageBodyWidth", "navigationGroups"], properties: {
  name: { type: "string", title: "Profile ID", minLength: 1 }, defaultRenderer: { type: "string", title: "默认 UI 框架", oneOf: rendererChoices }, allowedRenderers: { type: "array", title: "允许的 UI 框架", minItems: 1, uniqueItems: true, items: { type: "string", oneOf: rendererChoices } }, userSelectableRenderer: { type: "boolean", title: "允许用户切换 UI 框架" }, defaultTemplate: { type: "string", title: "默认布局", oneOf: [{ const: "standard", title: "标准侧栏布局" }, { const: "top-navigation", title: "顶部导航布局" }] }, pageBodyWidth: { type: "string", title: "页面正文宽度", oneOf: [{ const: "fluid", title: "自适应" }, { const: "contained", title: "最大 1280px" }] }, navigationGroups: { type: "array", title: "导航分组", items: { type: "object", additionalProperties: true } },
