...
export function createApplicationPage(client: PortalControlClient): CollectionPageDefinition<ApplicationRow> {
  ...
  return defineCollectionPage<ApplicationRow>({
    ...
    collection: { id: "portal-applications", title: message(namespace, "panel.revisions", "Portal Application Revisions"), view: "table", query: { mode: "page", defaultPageSize: 20, pageSizeOptions: [10, 20, 50] }, filters: [{ id: "portal", label: message(namespace, "filter.portal", "Portal"), kind: "text" }, { id: "status", label: message(namespace, "filter.status", "状态"), kind: "select", options: Object.entries(statusLabels).map(([value, label]) => ({ value, label })) }], columns: [{ key: "id", label: "Revision", format: "number", defaultVisible: true }, { key: "portalId", label: "Portal", defaultVisible: true, minWidth: 180 }, { key: "status", label: message(namespace, "column.status", "状态"), format: "status", valueLabels: statusLabels, statusTones: { Draft: "neutral", PendingApproval: "warning", Approved: "info", Published: "success" }, defaultVisible: true }, { key: "updatedAt", label: message(namespace, "column.updated", "更新时间"), format: "datetime", defaultVisible: true, minWidth: 180 }], selection: "single", preferences: { allowedColumns: ["id", "portalId", "status", "updatedAt"], density: true }, actions: [
      { id: "application.create", label: message(namespace, "action.new", "新建 Portal 草稿"), placement: "page.primary", tone: "primary", form: "create" },
      { id: "application.edit", label: message(namespace, "action.edit", "编辑草稿"), placement: "page.secondary", requiresSelection: true, form: "edit", visibleWhen: { pointer: "/status", equals: "Draft" } },
      { id: "application.submit", label: message(namespace, "action.submit", "提交审批"), placement: "page.secondary", requiresSelection: true, confirm: message(namespace, "confirm.submit", "提交后不能继续编辑，需要由另一位审批人处理。"), visibleWhen: { pointer: "/status", equals: "Draft" } },
      { id: "application.approve", label: message(namespace, "action.approve", "批准"), placement: "page.secondary", tone: "primary", requiresSelection: true, visibleWhen: { pointer: "/status", equals: "PendingApproval" } },
      { id: "application.publish", label: message(namespace, "action.publishInput", "发布为可选输入"), placement: "page.secondary", tone: "primary", requiresSelection: true, confirm: message(namespace, "confirm.inputPublish", "发布只会使该 Application 可被 Activation 引用，不会直接改变线上 Portal。"), visibleWhen: { pointer: "/status", equals: "Approved" } },
      { id: "application.diff", label: message(namespace, "action.diff", "查看差异"), placement: "page.secondary", requiresSelection: true, overlay: "diff" }, { id: "application.audit", label: message(namespace, "action.audit", "审计记录"), placement: "page.secondary", requiresSelection: true, overlay: "audit" },
    ] },
