...
const id, version, capability = "cn.vastplan.platform.data.relational.connection-manager", "0.16.1", "platform.database"
...
func descriptor() []byte {
	return []byte(`{"title":"数据库连接","subcommands":[
		{"name":"define","description":"定义连接、数据库类型参数和连接池策略，并发布到数据库运行服务","paramsSchema":{"type":"object","additionalProperties":false,"properties":{"name":{"type":"string"},"connection":{"type":"object"},"credentialValue":{"type":"string"}},"required":["name","connection"]}},
		...
		{"name":"test","description":"使用当前表单值和短期托管凭证测试连接，不保存连接定义","paramsSchema":{"type":"object","additionalProperties":false,"properties":{"name":{"type":"string"},"connection":{"type":"object"},"credentialValue":{"type":"string"}},"required":["name","connection"]}}
