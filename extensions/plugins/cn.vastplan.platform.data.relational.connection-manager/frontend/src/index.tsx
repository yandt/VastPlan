...
const schema: FormSchema = {
  ...
  schema: {
    ...
    properties: {
      ...
      options: { type: "object", additionalProperties: false, required: ["user"], properties: {
        ...
        serverName: { type: "string", title: "证书校验服务器名称" }, connectTimeoutMs: { type: "integer", title: "连接超时（毫秒）", minimum: 100, maximum: 300000 },
        ...
        readTimeoutMs: { type: "integer", title: "读取超时（毫秒）", minimum: 0, maximum: 300000 }, writeTimeoutMs: { type: "integer", title: "写入超时（毫秒）", minimum: 0, maximum: 300000 }, rejectReadOnly: { type: "boolean", title: "拒绝只读实例" },
      } },
  ...
  localization: {
    "/properties/name/title": message(namespace,"form.name","连接名称"), "/properties/providerId/title": message(namespace,"form.provider","数据库类型"), "/properties/endpoint/title": message(namespace,"form.endpoint","地址"), "/properties/database/title": message(namespace,"form.database","数据库"), "/properties/credentialValue/title": message(namespace,"form.credential","数据库凭证"),
    "/properties/options/properties/user/title": message(namespace,"form.user","用户名"), "/properties/options/properties/tlsMode/title": message(namespace,"form.tlsMode","传输加密模式"), "/properties/options/properties/connectTimeoutMs/title": message(namespace,"form.connectTimeout","连接超时（毫秒）"),
    "/properties/options/properties/serverName/title": message(namespace,"form.serverName","证书校验服务器名称"), "/properties/options/properties/applicationName/title": message(namespace,"form.applicationName","客户端应用名称"), "/properties/options/properties/network/title": message(namespace,"form.network","网络类型"), "/properties/options/properties/readTimeoutMs/title": message(namespace,"form.readTimeout","读取超时（毫秒）"), "/properties/options/properties/writeTimeoutMs/title": message(namespace,"form.writeTimeout","写入超时（毫秒）"), "/properties/options/properties/rejectReadOnly/title": message(namespace,"form.rejectReadOnly","拒绝只读实例"),
...
export function createDatabaseConnectionsPage(client: PlatformAdminClient, serviceID: string, path: string, title: ReturnType<typeof message>): CollectionPageDefinition<DatabaseRow> {
  const form = (id: "create" | "edit"): WorkbenchFormDefinition<DatabaseRow> => ({
    ...
    presentation: {
      ...
      sections: [
        ...
        { id: "options", title: message(namespace, "section.options", "连接选项"), columns: 2, fields: ["/options", "/credentialValue"] },
      ...
      fields: [
        ...
        { pointer: "/credentialValue", span: 2, widget: "secretMaterial", help: message(namespace, id === "create" ? "form.credentialCreateHelp" : "form.credentialEditHelp", id === "create" ? "新建连接必须输入；提交后立即清除。" : "留空会保留现有托管凭证；输入新值会原子替换，提交后立即清除。") },
      ],
    },
    workflow: {
      dialogWidth: "lg",
      title: message(namespace, id === "create" ? "form.createTitle" : "form.editTitle", id === "create" ? "新增数据库连接" : "编辑数据库连接"),
      description: message(namespace, "form.description", "连接定义和池策略保存在数据库插件中；凭证明文仅用于本次连接请求。"),
  ...
  return defineCollectionPage<DatabaseRow>({
    ...
    description: message(namespace, "page.description", "在数据库插件内配置连接、数据库类型、连接池与托管凭证"),
...
export default {
  ...
  localization: { defaultLocale:"zh-CN", messages: {
    "zh-CN": { "form.name":"连接名称","form.provider":"数据库类型","form.endpoint":"地址","form.database":"数据库","form.options":"连接选项","form.pool":"连接池","form.credential":"数据库凭证","form.user":"用户名","form.tlsMode":"传输加密模式","form.connectTimeout":"连接超时（毫秒）","form.serverName":"证书校验服务器名称","form.applicationName":"客户端应用名称","form.network":"网络类型","form.readTimeout":"读取超时（毫秒）","form.writeTimeout":"写入超时（毫秒）","form.rejectReadOnly":"拒绝只读实例","option.verifyFull":"完整校验（推荐）","option.tlsDisable":"关闭（仅受控测试环境）","option.networkTcp":"TCP","option.networkUnix":"Unix 套接字","form.minIdle":"最小空闲连接","form.maxIdle":"最大空闲连接","form.maxOpen":"最大连接数","form.maxLifetime":"连接最长生命周期（毫秒）","form.maxIdleTime":"最长空闲时间（毫秒）","form.acquireTimeout":"获取连接超时（毫秒）","form.idlePoolTtl":"空池回收时间（毫秒）","section.identity":"连接标识","section.options":"连接选项","section.pool":"连接池策略","form.credentialCreateHelp":"新建连接必须输入；提交后立即清除。","form.credentialEditHelp":"留空会保留现有托管凭证；输入新值会原子替换，提交后立即清除。","form.createTitle":"新增数据库连接","form.editTitle":"编辑数据库连接","form.description":"连接定义和池策略保存在数据库插件中；凭证明文仅用于本次连接请求。","error.credentialRequired":"新建连接必须输入数据库凭证","error.nameRequired":"连接名称不能为空","error.endpointRequired":"连接地址不能为空","notice.saved":"数据库连接已保存","filter.name":"连接名称","filter.provider":"数据库类型","column.name":"名称","column.provider":"数据库类型","column.endpoint":"地址","column.runtime":"运行状态","column.credential":"凭证","column.credentialVersion":"凭证版本","runtime.ready":"已就绪","runtime.pending":"待发布","credential.managed":"已托管","credential.missing":"未配置","action.create":"新增连接","action.edit":"编辑","action.save":"保存","action.probe":"探测","action.delete":"删除","confirm.delete":"确认删除此连接并退役其托管凭证？","status.ready":"连接正常","status.unavailable":"连接不可用","page.title":"数据库连接","page.titleService":"数据库连接 · {service}","page.description":"在数据库插件内配置连接、数据库类型、连接池与托管凭证" },
    "en-US": { "form.name":"Connection name","form.provider":"Provider","form.endpoint":"Endpoint","form.database":"Database","form.options":"Connection options","form.pool":"Connection pool","form.credential":"Database credential","form.user":"User","form.tlsMode":"TLS mode","form.connectTimeout":"Connect timeout (ms)","form.serverName":"TLS server name","form.applicationName":"PostgreSQL application name","form.network":"MySQL network","form.readTimeout":"MySQL read timeout (ms)","form.writeTimeout":"MySQL write timeout (ms)","form.rejectReadOnly":"Reject read-only MySQL instances","option.verifyFull":"Verify fully (recommended)","option.tlsDisable":"Disabled (controlled test environments only)","option.networkTcp":"TCP","option.networkUnix":"Unix socket","form.minIdle":"Minimum idle","form.maxIdle":"Maximum idle","form.maxOpen":"Maximum open","form.maxLifetime":"Maximum lifetime (ms)","form.maxIdleTime":"Maximum idle time (ms)","form.acquireTimeout":"Acquire timeout (ms)","form.idlePoolTtl":"Idle-pool TTL (ms)","section.identity":"Connection identity","section.options":"Connection options","section.pool":"Pool policy","form.credentialCreateHelp":"Required for a new connection and cleared immediately after submission.","form.credentialEditHelp":"Leave blank to retain the managed credential; a new value replaces it atomically and is then cleared.","form.createTitle":"Create database connection","form.editTitle":"Edit database connection","form.description":"The database plugin stores connection and pool policy. Plaintext is used only for this connection request.","error.credentialRequired":"A database credential is required for a new connection","error.nameRequired":"Connection name is required","error.endpointRequired":"Endpoint is required","notice.saved":"Database connection saved","filter.name":"Connection name","filter.provider":"Provider","column.name":"Name","column.provider":"Provider","column.endpoint":"Endpoint","column.runtime":"Runtime","column.credential":"Credential","column.credentialVersion":"Credential version","runtime.ready":"Ready","runtime.pending":"Pending publication","credential.managed":"Managed","credential.missing":"Missing","action.create":"Create connection","action.edit":"Edit","action.save":"Save","action.probe":"Probe","action.delete":"Delete","confirm.delete":"Delete this connection and retire its managed credential?","status.ready":"Connection ready","status.unavailable":"Connection unavailable","page.title":"Database connections","page.titleService":"Database connections · {service}","page.description":"Configure providers, pools and managed credentials in the database plugin" }
  } },
};
