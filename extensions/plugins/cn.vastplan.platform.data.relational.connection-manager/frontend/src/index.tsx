...
const schema: FormSchema = {
  ...
  localization: {
    ...
    "/properties/options/properties/user/title": message(namespace,"form.user","用户名"), "/properties/options/properties/password/title": message(namespace,"form.credential","密码"), "/properties/options/properties/tlsMode/title": message(namespace,"form.tlsMode","传输加密模式"), "/properties/options/properties/connectTimeoutMs/title": message(namespace,"form.connectTimeout","连接超时（毫秒）"),
  ...
  const form = (id: "create" | "edit"): WorkbenchFormDefinition<DatabaseRow> => ({
    ...
    presentation: {
      ...
      fields: [
        ...
        { pointer: "/options/password", widget: "secretMaterial", help: message(namespace, id === "create" ? "form.credentialCreateHelp" : "form.credentialEditHelp", id === "create" ? "新建连接必须输入密码；提交后立即清除。" : "留空会保留现有密码；输入新值会原子替换，提交后立即清除。") },
      ],
    },
    workflow: {
      dialogWidth: "lg",
