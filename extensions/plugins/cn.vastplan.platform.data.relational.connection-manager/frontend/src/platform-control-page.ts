...
const schema: FormSchema = {
  ...
  schema: {
    ...
    required: ["providerId", "host", "port", "database", "schema", "username", "tlsMode", "secretKind", "contractRange"],
    properties: {
      ...
      secretKind: { type: "string", title: "密码来源", oneOf: [{ const: "systemd-credential", title: "systemd Credential（生产推荐）" }, { const: "owner-file", title: "受保护文件（开发）" }] },
      secretName: { type: "string", title: "Credential 名称", minLength: 1, maxLength: 128 },
      secretPath: { type: "string", title: "密码文件绝对路径", minLength: 1, maxLength: 4096 },
      contractRange: { type: "string", title: "Shared State 契约范围", minLength: 1, maxLength: 64 },
    },
  },
  uiSchema: {
    username: { "ui:options": { autocomplete: "new-password" } },
    secretName: { "ui:options": { autocomplete: "off" } },
    secretPath: { "ui:options": { autocomplete: "off" } },
  },
};
    ...
    form: {
      ...
      presentation: {
        ...
        sections: [
          ...
          { id: "security", title: message(namespace, "platformControl.section.security", "传输与密码"), columns: 2, fields: ["/tlsMode", "/serverName", "/secretKind", "/secretName", "/secretPath"] },
        ...
        fields: [
          ...
          { pointer: "/secretName", visibleWhen: { pointer: "/secretKind", equals: "systemd-credential" } },
          { pointer: "/secretPath", visibleWhen: { pointer: "/secretKind", equals: "owner-file" } },
        ],
      },
      workflow: {
        surface: "page",
        title: message(namespace, "platformControl.form.title", "平台控制数据库"),
        description: message(namespace, "platformControl.form.description", "页面只保存密码引用，不接收或返回密码明文。初始化成功后完整种子插件组合才会启动。"),
...
function statusToForm(status: PlatformControlStatus): PlatformControlForm {
  ...
  return {
    ...
    secretKind: profile.secretRef.kind,
    secretName: profile.secretRef.kind === "systemd-credential" ? profile.secretRef.name : undefined,
    secretPath: profile.secretRef.kind === "owner-file" ? profile.secretRef.path : undefined,
...
function toChangeRequest(value: PlatformControlForm): PlatformControlChangeRequest {
  ...
  const secretKind = text(value.secretKind);
  ...
  const secretRef: PlatformControlProfile["secretRef"] = secretKind === "systemd-credential"
    ? { kind: "systemd-credential", name: text(value.secretName) ?? "" }
    : { kind: "owner-file", path: text(value.secretPath) ?? "" };
  return {
    expectedGeneration,
    profile: {
      schemaVersion: 1,
...
function validatePlatformControlForm(value: PlatformControlForm): WorkbenchFormFieldErrors {
  ...
  return {
    ...
    ...(secretKind === "systemd-credential" && text(value.secretName) === undefined ? { "/secretName": message(namespace, "platformControl.error.secretNameRequired", "请输入 systemd Credential 名称") } : {}),
    ...(secretKind === "owner-file" && (text(value.secretPath)?.startsWith("/") !== true) ? { "/secretPath": message(namespace, "platformControl.error.secretPathRequired", "请输入受保护密码文件的绝对路径") } : {}),
  };
}
