...
function MegaGroup({ group, composition, onNavigate }: { group: PortalNavigationGroup; composition: UIShellProps["composition"]; onNavigate(id: string): void }) {
  ...
  if (group.pages.length === 0 && group.children.length === 0) {
    return <div className="vp-top-mega vp-top-mega-empty">{i18n.text(message(namespace, "navigation.accountUnavailable", "个人中心尚未装配"))}</div>;
  }
