  ...
  return defineCollectionPage<BindingRow>({
    ...
    collection: {
      ...
      actions: [
        ...
        { id: "revoke", label: message(namespace, "action.revoke", "即时撤权"), icon: "remove", placement: "record.row", tone: "danger", requiredPermissions: ["platform.authorization.revoke"], confirm: message(namespace, "confirm.revokeBinding", "立即撤销该 Binding？"), visibleWhen: { pointer: "/state", equals: "Published" } },
        { id: "retire", label: message(namespace, "action.retire", "退役"), icon: "remove", placement: "record.row", tone: "danger", requiredPermissions: ["platform.authorization.binding"], visibleWhen: { pointer: "/state", equals: "Published" } },
      ],
    },
