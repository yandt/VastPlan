...
export function permissionsPage(client: PlatformAdminClient, serviceID?: string, suffix = "", pathSuffix = ""): CollectionPageDefinition<PermissionRow> {
  return defineCollectionPage<PermissionRow>({
    id:`platform.authorization.permissions${suffix}`, path:`/settings/authorization/permissions${pathSuffix}`, title:message(namespace,"permissions.title","权限目录"),
    description:message(namespace,"permissions.description","权限由已验证插件清单声明，角色管理不能创建权限代码。"), requiredPermissions:["platform.authorization.catalog"], navigation:{id:`platform.authorization.permissions${suffix}`,label:message(namespace,"permissions.navigation","权限目录"),parentMenuRef:{pluginID:"cn.vastplan.platform.security.authorization-policy",nodeID:"security"},...(serviceID === undefined ? {} : { managementServiceID: serviceID }),order:10},
    collection:{id:"authorization-permissions",title:message(namespace,"permissions.title","权限目录"),view:"table",query:{mode:"page",defaultPageSize:20,pageSizeOptions:[20,50,100]},filterPanel: { fields: [{id:"search",label:message(namespace,"filter.permission","权限代码或插件"),kind:"text"}] },columns:[
