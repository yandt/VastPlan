...
export function permissionsPage(client: PlatformAdminClient): CollectionPageDefinition<PermissionRow> {
  return defineCollectionPage<PermissionRow>({
    ...
    description:message(namespace,"permissions.description","权限由已验证插件清单声明，角色管理不能创建权限代码。"), requiredPermissions:["platform.authorization.catalog"], navigation:{id:"platform.authorization.permissions",label:message(namespace,"permissions.navigation","权限目录"),parentMenuRef:{pluginID:"cn.vastplan.platform.security.authorization-policy",nodeID:"security"},order:10},
