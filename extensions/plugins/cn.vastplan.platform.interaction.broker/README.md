...
`cn.vastplan.platform.interaction.broker` 是平台级基础插件：它持久化并裁决跨 Portal、Mobile 与 Desktop 的人机交互任务。
...
当前实现提供 `open/list/get/watch/present/respond/cancel` 持久化闭环。`watch` 使用最后一次 `updatedAt` 作为游标，Desktop/后端来源可在断线重连后恢复观察；Node Portal Kernel 传输适配已接入，Mobile Gateway/Native Adapter 仍待实现。服务本身不依赖任何浏览器或原生 UI 运行时。
