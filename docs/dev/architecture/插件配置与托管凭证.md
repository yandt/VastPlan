...
本文是 VastPlan 插件配置、敏感输入和运行时配置投影的单一真相源。取舍原因见 [ADR-0090](../decisions/ADR-0090-插件配置与托管凭证闭环.md)。
...
| Portal Edge 配置协调器 | 鉴权、校验清单、CAS、驱动 candidate Saga、审计 | 解密凭证、把秘密写入日志或 URL |
...
```mermaid
stateDiagram-v2
    [*] --> Staged: 插件配置页提交 secrets
    Staged --> Preparing: 配置 CAS 成功
    Staged --> Aborted: 校验或 CAS 失败
    Preparing --> Active: 凭证激活且 candidate 发布
    Preparing --> Failed: 激活或发布失败
    Failed --> Collected: 按 candidate ID 清理不可达版本
    Active --> Staged: 替换或轮换
```

当前运行时只消费 `Active`。CredentialRef 的 `owner` 必须等于目标插件 ID，`purpose` 必须来自该版本的签名清单；托管器返回不匹配引用时协调器立即 fail-closed。

删除插件配置不应默认立即销毁外部身份。默认先撤销本插件的引用授权并进入保留期，再由明确的安全策略决定是否删除密文版本或调用目标系统吊销 API。

## 5. 用户体验

数据库连接页应直接出现“密码/访问令牌”字段，保存时同时更新连接定义与托管凭证；用户不再填写 `CredentialRef` 名称。编辑已存在连接时秘密字段为空代表“保持当前版本”，填写新值代表“创建新版本”。页面只显示托管状态和版本。
...
- 凭证插件的持久 `stage/activate/abort/GC` 适配器；
- 数据库页从手填引用迁移为内联秘密输入；
- 制品仓库 storage Provider 协议、file Provider 和在线迁移控制器；
- CredentialRef wire v2 与多语言 SDK 生成物。
