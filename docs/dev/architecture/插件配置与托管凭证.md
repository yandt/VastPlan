...
- Backend Resolver 已从本次解析的验签制品生成 `ConfigurationCatalog v1`；初始发布与 Deployment Manager 在线发布都会把目录作为同代 Deployment sidecar 写入可信控制面，内核只返回与当前 revision/digest 匹配的目录；plugin-settings 0.6.0 已提供目录读取、动态表单、Draft CAS、审计、重启恢复与 Application 激活，并明确不把 Draft 宣称为 Active。
- Backend Kernel 已实现默认 45 秒、单字段、一次性的 `ConfigurationAuthority`：控制面只保存 bearer 摘要并以 NATS KV CAS 原子消费；凭证 0.9.0 提供 delegated stage/prepare/activate/abort，plugin-settings 0.6.0 已接入 Workbench 只写秘密、`Preparing` 检查点和放弃回滚，公开候选不包含 handle、stage ID、authority、密文或明文。
- Application Deployment 激活路径已由 plugin-settings 0.6.0、deployment-manager 0.15.0 和 credentials 0.9.0 闭环：候选提交生成独立审批修订，普通发布入口不能绕过专用 Saga；候选凭证在 Candidate 窗口支持启动 readiness，成功后转 Active，失败后随单调 Deployment rollback 终止。ServiceUnit 以独立 `managed_credentials` 信封保存引用，宿主按认证插件通过 `kernel.config.credential-ref` 精确投影。
- Backend Platform Profile 激活已完成 [ADR-0116](../decisions/ADR-0116-Backend-Platform-Profile候选Catalog与配置激活.md) 的可信 Catalog 基座：`deploymentpublisher` 每次锁定并复核当前 Catalog digest 与 Profile ref；`VASTPLAN_BACKEND_PLATFORM_CATALOGS_V1` 和 `platformcatalog.Store` 已提供受限历史、create-only Seed、摘要/身份校验、候选 CAS 状态机和目标 binding 发布锁。Manager 连接只读，候选写入必须使用第二条、绑定精确 Catalog ID/key 的 `catalog-publisher` NATS 连接；回滚总是创建更高 Catalog revision。

待实现：

- Backend Platform Profile 的 Deployment Manager Activation Saga、精确内核候选端口与管理 UI，以及 `configuration.v1` hot 发布控制器；现有全局设置、数据库和凭证页面仍保留领域专用实现；
