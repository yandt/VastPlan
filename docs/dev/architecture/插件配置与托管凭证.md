...
- Application Deployment 激活路径已由 plugin-settings 0.6.0、deployment-manager 0.14.0 和 credentials 0.9.0 闭环：候选提交生成独立审批修订，普通发布入口不能绕过专用 Saga；候选凭证在 Candidate 窗口支持启动 readiness，成功后转 Active，失败后随单调 Deployment rollback 终止。ServiceUnit 以独立 `managed_credentials` 信封保存引用，宿主按认证插件通过 `kernel.config.credential-ref` 精确投影。
- Backend Platform Profile 激活已完成 [ADR-0116](../decisions/ADR-0116-Backend-Platform-Profile候选Catalog与配置激活.md) 与读取基座：`deploymentpublisher` 每次通过严格 `CatalogSource.Snapshot` 读取并复核当前 Catalog；`VASTPLAN_BACKEND_PLATFORM_CATALOGS_V1` 和 `platformcatalog.Store` 已提供受限历史、create-only Seed、摘要/身份校验和 Manager 只读 ACL。

待实现：

- Backend Platform Profile 的专用 Publisher 身份、候选锁、Deployment Manager Activation Saga，以及 `configuration.v1` hot 发布控制器；现有全局设置、数据库和凭证页面仍保留领域专用实现；
