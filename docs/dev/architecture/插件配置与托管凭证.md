...
服务单元配置只允许 `plugins`、`managed_credentials`、`environment_allowlist`、`kernel_service_grants` 和调度器使用的 `partition_keys`。控制面在发布前验证所有插件 key 属于已安装插件；Node Agent 再验证一次。

`kernel_service_grants.<pluginId>` 是 Platform Profile 或可信部署解析结果给该运行单元的**精确授权**，不能使用通配符。Node Agent 将“已验签 Manifest 的 `capabilities.kernelServices` 申请、精确服务授权、内核运维方配置的发布者上限”求交，并确认服务已在候选宿主注册。Manifest 申请中的每一项当前都按必需依赖处理，因此缺项、额外授权、未知发布者或宿主没有实现都会在插件进入执行驱动前 fail-closed；插件不能通过自身配置扩大权限。

Backend Runtime 使用无 fallback 的 `NewPluginMapConfig` 冻结快照。`kernel.config.get` 忽略 payload 中任何身份暗示，只使用握手认证后写入 `CallContext.caller.id` 的插件 ID。未安装、未配置、跨插件 key 默认返回 not found；调用方对声明为可选的字段可传 `optional: true`，缺失时宿主返回成功状态与 JSON `null`，Provider/传输/授权故障仍保持错误。这样插件只在协议边界判断一次“是否配置”，后续工作流统一消费解析后的配置对象，不得在各层自行吞掉读取错误。
...
- 动态 Profile 的首个纵向闭环已经完成：签名清单、可信 Catalog、`configuration.resource.v1`、精确资源 ConfigurationAuthority、plugin-settings 可恢复 Saga、Node SDK、固定 BFF、Workbench MasterDetail 和 Webhook Delivery 共同实现 `cfgc_* / cfgp_*`、固定秘密槽、create/update/delete CAS、无 handle 查询、异人审批、Material Lease 探测和旧引用退休。Webhook Delivery 0.3.0 已按租户迁移至 fenced Shared State，但仍保持 leader 工作流所有者；共享耐久状态本身不授权 active-active。
