...
- Backend Resolver 已从本次解析的验签制品生成 `ConfigurationCatalog v1`；初始发布与 Deployment Manager 在线发布都会把目录作为同代 Deployment sidecar 写入可信控制面，内核只返回与当前 revision/digest 匹配的目录；plugin-settings 0.3.0 已提供目录读取、非敏感动态表单、Draft CAS、审计和重启恢复，并明确不把 Draft 宣称为 Active。

待实现：

- `ConfigurationAuthority`、托管凭证 delegated stage，以及 Application Deployment、Backend Platform Profile 和 `configuration.v1` hot 三条发布控制器；现有全局设置、数据库和凭证页面仍是领域专用实现；
