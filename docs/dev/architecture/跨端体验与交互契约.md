...
Portal 访问 Broker 经 Node Portal Kernel/BFF，使用 OIDC 密封 Session、CSRF 和 `Principal` 投影；Mobile 访问 Broker 经独立移动认证网关，使用移动端令牌和设备绑定。两端均不得把原始浏览器 cookie、移动令牌或 Runner 长期凭证传给插件。Portal/Mobile Gateway 的粗粒度入口授权由独立的 `interaction-access-policy` 执行，Broker 是任务归属、合格响应主体和终态裁决的唯一对象级授权点（ADR-0055）。
