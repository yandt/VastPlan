...
- 已完成 Portal Edge 的浏览器传输适配、独立交互入口策略和 `@vastplan/ui-primitives` 的 `PortalInteractionClient`；`core/kernels/runner/interaction` 已提供 UI 无关的最小 Runner 协调器与 `ProtocolBroker`，后者可适配本地 Protocol Host 或远程 Addressing Router，并强制 Runner 来源/租户/场景绑定。
