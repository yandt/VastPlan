...
`com.vastplan.platform.interaction.broker` 是平台级基础服务，负责创建、读取、响应、取消和观察交互任务。它依据经验证的 `CallContext` 校验：来源 capability、租户、任务归属、目标用户/角色、请求状态和截止时间。

Portal 访问 Broker 经 Edge/BFF，延续 Cookie、CSRF 和 `portalapi.Principal` 投影；Mobile 访问 Broker 经独立移动认证网关，使用移动端令牌和设备绑定。两端均不得把原始浏览器 cookie、移动令牌或 Runner 长期凭证传给插件。
...
App Profile 在构建期选择 Runner/Mobile 的编译型插件和 UI Adapter；Portal 在运行期从经验证制品装配 Web 模块。能力协商不满足时，composition 必须拒绝该 Profile，或使用插件明确声明的替代表现，不能静默降级为未授权界面。

## 6. 实施顺序与验收

1. 建立 `schemas/ui/v1`、TypeScript/Go 生成模型，并从 Web SDK 中抽出无框架的表单和交互类型。
...
- 已完成第 1 步：`schemas/ui/v1` 与 `@vastplan/ui-contract` 提供无框架 UI/交互模型，`@vastplan/portal-ui` 只作为 Web Adapter 消费它。
- 已完成第 2 步的基础声明：清单已支持 `runner.interactions` 与 `mobile.uiAdapters`，并对交互类型、体验面、契约范围和 Native Adapter 的语义能力做封闭校验。
- 已开始第 3 步：`com.vastplan.platform.interaction.broker` 已实现持久化 `open/list/get/present/respond/cancel`，来源绑定、租户隔离、响应主体筛选、过期 fail-closed、条件式单一终态和 `secretRef` 明文拒绝；其 API 不依赖 Portal 或 Mobile Runtime。
- 尚未接入的边界：Runner 的长连接 `watch`/断线恢复、Portal Edge 的浏览器传输适配，以及 Mobile Gateway/Native Adapter。这些必须在 Broker 现有授权与状态机之上实现，不能绕过它直接互调。
