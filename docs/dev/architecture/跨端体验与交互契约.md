...
Runner App Profile 使用独立的 `schemas/app/v1` 契约，在构建期选择编译型插件；`deployment/v2.app_profiles` 只通过 `id + revision + SHA-256 digest` 固定引用其不可变制品，不把它放入后端 `units` 服务调度。Mobile Profile 与 UI Adapter 的具体契约仍待后续版本扩展；Portal 在运行期从经验证制品装配 Web 模块。能力协商不满足时，composition 必须拒绝该 Profile，或使用插件明确声明的替代表现，不能静默降级为未授权界面。
...
- 已完成 Portal Edge 的浏览器传输适配、独立交互入口策略和 `@vastplan/portal-ui` 的 `PortalInteractionClient`；`kernels/runner/interaction` 已提供 UI 无关的最小 Runner 协调器与 `ProtocolBroker`，后者可适配本地 Protocol Host 或远程 Addressing Router，并强制 Runner 来源/租户/场景绑定。
- 已建立 `schemas/app/v1` Runner Profile 契约、受信任 Catalog 校验和认证身份领取边界；`deployment/v2` 通过不可变引用纳入 Profile 期望态，但服务调度仍只处理 `units`。Profile 制品构建/签名/发布和实际 Runner 装配，以及 Mobile Gateway/Native Adapter 仍待实现。这些必须在 Broker 现有授权与状态机之上实现，不能绕过它直接互调。
