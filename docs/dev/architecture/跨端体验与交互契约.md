...
Web Adapter 可以把契约映射为 Arco/MUI，Mobile Adapter 可以映射为 SwiftUI/Compose。二者可以拥有完全不同的布局和导航，只要声明实现相同的语义能力。Portal 的现有 `ui.render.adapter` 仍要求完整 Web 能力集；Mobile 应声明独立的 `mobile.ui-adapter`，并按 App Profile 在编译期选定。
