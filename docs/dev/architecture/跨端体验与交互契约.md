...
- 已完成 Portal Edge 的浏览器传输适配、独立交互入口策略和 `@vastplan/portal-ui` 的 `PortalInteractionClient`；Broker 已提供 Runner/后端来源可恢复的 `watch` 原语。实际 Runner 内核接入，以及 Mobile Gateway/Native Adapter 仍待实现。这些必须在 Broker 现有授权与状态机之上实现，不能绕过它直接互调。
