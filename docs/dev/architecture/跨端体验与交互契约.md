...
- 已开始第 3 步：`com.vastplan.platform.interaction.broker` 已实现持久化 `open/list/get/watch/present/respond/cancel`，来源绑定、租户隔离、响应主体筛选、过期 fail-closed、条件式单一终态、断线后按游标恢复观察和凭证引用字段明文拒绝；其 API 不依赖 Portal 或 Mobile Runtime。
