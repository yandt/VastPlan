...
- 已完成 Portal Edge 的浏览器传输适配、独立交互入口策略和 `@vastplan/portal-ui` 的 `PortalInteractionClient`；`kernels/runner/interaction` 已提供 UI 无关的最小 Runner 协调器，只依赖 Broker `open/watch/cancel` 端口并强制来源/租户绑定。实际 Runner 的远程寻址、Profile 装配，以及 Mobile Gateway/Native Adapter 仍待实现。这些必须在 Broker 现有授权与状态机之上实现，不能绕过它直接互调。
