...
服务配置入口接收两份 config-as-code、版本化文档：平台管理员发布 `Platform Profile`，应用配置人员发布 `Application Composition`。Composition Resolver 校验已验证 Manifest 的固有分类、目标、依赖和来源后生成完整 `Deployment v2`；应用不能直接提交 Deployment 或配置基础插件。详细规则见《[插件分级与组合解析](插件分级与组合解析.md)》和 ADR-0057。
