...
同一插件的 webui/backend/rc 部分之间的通信走本章的内核间平面：前端部分→后端部分走 ①；后端部分→RC 部分走 ② 到 rs 再走 ③。插件不获得私有跨面通道，一律复用统一通信平面，保证鉴权、计量、可观测一致。
...
**通信**
- [ ] 内部服务寻址层接口形态与服务发现/路由/熔断（进程内、网络两路径等价实现）
- [ ] 各平面传输与序列化选型（控制面主选 NATS，强类型/大流式保留 gRPC，见 0008）
