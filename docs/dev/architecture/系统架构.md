...
- **生产信任边界**：NATS 客户端使用 TLS 1.3 双向证书 + 每实例 NKey；gRPC 流式端点使用 TLS 1.3 双向证书。bootstrap/controller/node/manager-node/catalog-publisher/runtime 按具体 KV Stream、JetStream consumer API 与 RPC/Event Subject 分权；Backend Platform Catalog 候选写入使用独立第二连接，不能借用 Manager Node 身份。明文连接只允许显式本地开发，见 ADR-0027/0029/0116。
