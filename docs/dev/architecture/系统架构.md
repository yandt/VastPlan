...
正式机器契约分两层：`schemas/deployment/v2/vastplan.deployment.schema.json` 是全局多节点调度输入，支持 `service`、整数基线副本、自动伸缩区间、节点标签亲和/反亲和与 CPU/内存/GPU 资源请求；`schemas/deployment/v1/vastplan.desired-state.schema.json` 是每节点执行快照，仍固定 `replicas: 1` 并携带已占用资源请求。控制器负责 v2→v1，Node Agent 只执行 v1。Runner App 使用独立 `schemas/app/v1` Profile 契约，deployment/v2 仅以 `app_profiles` 的 revision+digest 引用它，不进入服务调度；`portal` 仍须后续 Schema 扩展。
...
| `portal` | frontend | `route`/`domain`；`audience`（面向哪些租户/角色）；`branding`；精确选择一个已签名 `ui.design-system`；前端插件走 Module Federation 运行时装配 |
...
> `app` 单元是**预构建产物的声明**：插件服务据此 per-profile 预编译签名二进制（ADR-0012），Runner 经自更新、Mobile 经商店/MDM 分发（ADR-0013）。在 `app` 正式进入 deployment Schema 前，Runner 已有独立 Profile 领域模型和领取边界：Profile 必须绑定 `tenant`、Runner 目标与已验证插件目录；领取只接受认证后的 Runner identity，并同时校验 tenant 与 `assignedTo`，不信任客户端声明的 Runner ID。
...
单节点 v1 示例见 [`deploy/local.desired-state.json`](../../../deploy/local.desired-state.json)；多节点 v2 示例见 [`deploy/cluster.deployment.json`](../../../deploy/cluster.deployment.json)。上面的 YAML 同时展示当前可执行的 service 高级调度字段和仍属长期概念的 `portal/app`；后两种 unit 不能交给当前控制器执行。Portal 的实施边界、UI 设计系统插件和在线发布治理由《[前端门户内核](前端门户内核.md)》定义。
