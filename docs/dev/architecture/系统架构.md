...
Backend Node Agent 的实际态采用“当前稳定实例 + candidate”双视图，并在长操作前后持久化检查点；实际态 v3 还保存 Assignment 制品引用的 tenant/owner、单调 generation、期望 revision、心跳时间和 outbox。精确生命周期状态、旧版本读取迁移和失败语义见 [ADR-0032](../decisions/ADR-0032-Backend插件生命周期实际态v2.md)。
