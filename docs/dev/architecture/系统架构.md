...
当前有两条共享同一 reconcile 事务语义的可执行路径：本地开发用 `deploy/local.desired-state.json`；集群用 Platform Profile + Application Composition → Resolver → Deployment v2 KV → 控制器 → 节点 assignment v1 KV → Node Agent。`deploy/cluster.deployment.json` 只是 Resolver 结果样例，不是人工配置入口。两条路径都从不可变制品仓库读取、按 SHA-256 内容寻址安装，并以真实插件进程与能力租约上报运行事实。
...
正式机器契约分层：`schemas/composition/common/v1` 定义四内核共用的输入身份、目标、来源与解析引用，`schemas/composition/backend/v1` 定义 Backend 载荷，`schemas/composition/frontend/v1` 定义 Portal Platform Profile 与 Application Composition；`schemas/deployment/v2/vastplan.deployment.schema.json` 是 Backend Resolver 生成的全局多节点调度锁；`schemas/deployment/v1/vastplan.desired-state.schema.json` 是每节点执行快照。控制器负责 v2→v1，Node Agent 只执行 v1。Runner App 使用独立 `schemas/app/v1` 完整锁定 Profile，deployment/v2 仅以 revision+digest 引用它，不进入服务调度。Portal Composer 在服务端把环境绑定 Profile 与应用输入解析成带 digest/origin 锁的发布 revision，可信 Catalog 复核精确制品，浏览器只执行锁定结果（ADR-0059）。跨内核拆分依据 ADR-0058。
...
平台与应用输入示例分别见 `deploy/platform-profile.json` 和 `deploy/application-composition.json`。`deploy/cluster.deployment.json` 是 Resolver 生成结果，只用于调度诊断和测试，不是服务配置入口。单节点 `deploy/local.desired-state.json` 仅用于 Node Agent 本地开发。Portal、Runner 与 Mobile 使用各自解析产物，不把它们伪装成 Backend service unit。
