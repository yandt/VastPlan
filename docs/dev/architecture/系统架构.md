...
各通信面的落地传输如下；后端 request-reply 与非持久事件已经按 [ADR-0025](../decisions/ADR-0025-NATS控制面寻址与多节点调度.md) 实现：
...
- **wire 版本**：远端调用使用 `proto/addressing/v1`，subject 为 `vp.rpc.v1.<capability-token>`；取消为 `vp.rpc.cancel.v1`，非持久事件为 `vp.event.v1.<event-token>`。
- **生产信任边界**：NATS 客户端使用 TLS 1.3 双向证书 + 每实例 NKey；bootstrap/controller/node/runtime 按具体 KV Stream 与 RPC/Event Subject 分权，明文匿名连接只允许显式本地开发，见 ADR-0027。
...
- **插件服务 Plugin Service**（Backend 内核组合出的核心服务，内部两职分层）：
  - **插件制品仓库**（借鉴 testa release-service）：当前提供本地不可变文件后端和 HTTPS 远端服务；远端发布使用 Ed25519 证明把 publisher/keyId 与完整元数据绑定，服务端和 Node Agent 双重验证，读写令牌分离，见 ADR-0026。声明式可插拔 Kind 注册表（可见性/变体/channel/edition/版本序/是否多平台打包/是否 Manifest 型）仍是后续市场维度。
...
- **部署控制器**（Plugin Service 的期望态职责层）：读取全局 Deployment v2 与节点租约，用标签过滤 + rendezvous hashing 生成每节点 DesiredState v1 assignment。副本不足时不发布半份计划，Node Agent 不自行仲裁全局副本。
- **集群化**：同 capability 的 N 个实例加入同一 NATS queue group；伸缩 = 修改 v2 `replicas` → 控制器推进 assignment generation → 各节点 reconcile 增减。
...
正式机器契约分两层：`schemas/deployment/v2/vastplan.deployment.schema.json` 是全局多节点调度输入，支持 `service`、整数 `replicas >= 1` 与精确标签 `nodeSelector`；`schemas/deployment/v1/vastplan.desired-state.schema.json` 是每节点执行快照，仍固定 `replicas: 1`。控制器负责 v2→v1，Node Agent 只执行 v1。`portal/app`、自动伸缩区间、亲和与资源约束仍须后续 Schema 扩展。
...
| kind | 内核 | 特有字段 |
|---|---|---|
| `service` | backend | `service_role: backend|workspace|rs`；当前 v2 `replicas` 为正整数；`placement.nodeSelector`（见 3.6.4） |
...
- `affinity` / `antiAffinity`：规划中，当前 v2 未开放。
- `resources`：规划中，当前 v2 未开放。
- **集群化**：`replicas` 决定副本数，多副本加入同一 NATS queue group 负载均衡（§3.1）；改 `replicas` → KV 推送 → reconcile 增减实例。
- **当前放置规则**：精确标签全匹配、rendezvous hashing、同一 unit 每节点最多一个副本；匹配节点不足则 fail-closed。
...
单节点 v1 示例见 [`deploy/local.desired-state.json`](../../../deploy/local.desired-state.json)；多节点 v2 示例见 [`deploy/cluster.deployment.json`](../../../deploy/cluster.deployment.json)。上面的 YAML 仅展示长期概念，其中 `portal/app`、自动伸缩区间及高级放置字段不能交给当前控制器执行。

#### 3.6.8 待决

- [x] v1 节点执行快照 + v2 全局多副本正式 JSON Schema 与校验
- [x] `placement.nodeSelector` 精确匹配与确定性副本放置
- [ ] `affinity` / `antiAffinity` / `resources` 语法与计分
...
- [x] 节点代理 reconcile 基础失败语义（幂等、候选失败保留旧实例、显式回滚）→ ADR-0024；在途 drain 仍待设计
