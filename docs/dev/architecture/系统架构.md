...
> 插件当前全部由本方开发（第一方、可信），暂不开放第三方；插件市场用于分发本方插件。第三方清单与运行驱动扩展位已预留，未知发布者至少要求 `process-sandbox`，隔离驱动落地前拒绝运行（ADR-0047）。
...
| 多语言与隔离边界     | native/python 运行驱动；发布者策略强制隔离下限，第三方不得降级到可信进程                                | [0047](../decisions/ADR-0047-多语言运行驱动与第三方隔离边界.md) |
...
- **第一方可信、第三方预留**：第一方 native/python 采用独立进程故障隔离与协议/资源边界；未知发布者必须使用至少 `process-sandbox` 的驱动，当前无可用隔离驱动时拒绝启动。
