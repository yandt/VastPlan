...
本节定义目标契约，尚未表示所有字段和状态都已进入当前 v1/v2 schema。实现顺序、测试要求和未承诺的全局原子性见 ADR-0044；插件实例策略、能力可见性和服务集群边界见 [ADR-0045](../decisions/ADR-0045-插件实例化策略与服务集群边界.md) 及《[插件服务集群化设计](插件服务集群化设计.md)》。代表性平台插件完成后，再进行长时间 soak 与级联恢复测试。
