...
平台与已编译应用提案示例分别见 `engineering/deploy/platform-profile.json` 和 `engineering/deploy/application-composition.json`；在 P1/P2 完成前，后者仍是开发期人工维护的过渡文件，不代表目标在线表单。`engineering/deploy/cluster.deployment.json` 是 Resolver 生成结果，只用于调度诊断和测试，不是服务配置入口。单节点 `engineering/deploy/local.desired-state.json` 仅用于 Node Agent 本地开发。Portal、Runner 与 Mobile 使用各自解析产物，不把它们伪装成 Backend service unit。
