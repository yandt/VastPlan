...
| 插件运行形态        | Backend 默认独立进程；精确首方版本可由部署方选择静态或 dynamic-go 内嵌，安全管道保持统一                 | [0004](../decisions/ADR-0004-插件运行形态.md) / [0051](../decisions/ADR-0051-Backend混合插件运行与受控内嵌边界.md) |
...
│  ── 默认独立进程；受控静态/dynamic-go 内嵌共用安全管道 ──   │
└─────────────────────────────────────────────────────────┘
```
...
- **默认独立进程 + 受控内嵌**（0004/0051）：Backend 插件默认经稳定协议在独立进程运行；只有首方硬身份、精确匹配验签清单且被部署方显式选择的版本才能静态或 dynamic-go 内嵌。三种承载共用 Registry、权限、钩子、资源限制和生命周期。
...
| **协议总线 Protocol Bus** | 宿主与插件实例之间的统一调用和生命周期边界；进程实例以 gRPC 承载，静态/dynamic-go 内嵌实例复用同一语义。详见《[插件契约与协议](插件契约与协议.md)》。 |
...
- **装配方式按内核而异**：Backend 服务端运行时热装，默认独立进程；首方插件可受控静态或 dynamic-go 内嵌（0003/0004/0051）。**Runner 内核**（客户端执行器）编译型插件**预编译进单一签名二进制、进程内、整体原子热升级**（0012），内容型脚本/工作流仍运行时下载；**Mobile 内核** gomobile bind 进原生外壳、商店/MDM 分发（0013/0014）。
...
Backend 插件默认以独立进程运行，经**协议总线**与宿主握手、注册贡献并接入扩展点。受控静态/dynamic-go 内嵌实例省略进程握手和 gRPC 最后一跳，但启动时必须将代码声明与验签清单精确核对；dynamic-go 还要校验首方身份、ABI、共同构建指纹和平台参数。三种承载继续经过同一 Registry、公开调用管道、宿主回调鉴权和生命周期。消息集、握手与实例形态详见《[插件契约与协议 · 插件-宿主协议](插件契约与协议.md)》。**跨面插件**各部分（前端↔后端↔RC）的协作走第二章的内核间通信平面，不占用宿主-插件协议总线。
...
| 默认进程与受控静态/dynamic-go 内嵌 | 0004 独立进程 + 协议总线；0051 混合运行边界 |
