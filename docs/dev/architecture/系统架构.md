...
正式机器契约分层：`schemas/composition/common/v1` 定义四内核共用的输入身份、目标、来源与解析引用，`schemas/composition/backend/v1` 定义 Backend 的 Platform Profile 与 Application Composition 载荷；`schemas/deployment/v2/vastplan.deployment.schema.json` 是 Backend Resolver 生成的全局多节点调度锁；`schemas/deployment/v1/vastplan.desired-state.schema.json` 是每节点执行快照。控制器负责 v2→v1，Node Agent 只执行 v1。Runner App 使用独立 `schemas/app/v1` 完整锁定 Profile，deployment/v2 仅以 revision+digest 引用它，不进入服务调度；`portal` 由前端自己的双输入 Resolver 生成发布 revision。跨内核拆分依据 ADR-0058。
