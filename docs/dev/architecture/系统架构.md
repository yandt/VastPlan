...
插件 runtime 策略及 `deployment` v1/v2 的实例策略字段已进入当前 schema，并由 Node Agent 在激活前校验；本阶段只完整支持 `per-kernel` 与 `active-active`，`leader`/`partitioned` 仍 fail-closed。跨服务 readiness、全局依赖传播、路由域隔离和有状态数据协议仍按 ADR-0044、ADR-0045 及《[插件服务集群化设计](插件服务集群化设计.md)》继续落地。代表性平台插件完成后，再进行长时间 soak 与级联恢复测试。
