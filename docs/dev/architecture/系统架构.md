...
- **独立进程 + 托管 Runtime Provider**（0004/0088/0089）：native/Python 进程、Node Worker、Python 子解释器及隔离 Provider 都经稳定协议运行。托管语言默认只在同一服务、ABI、安全与发布者信任域内共享 Host，部署方可显式 dedicated；每个插件仍有独立 session、权限、贡献和生命周期。dynamic-go 正从 Backend 进程内历史承载迁入 Go Runtime Provider。
