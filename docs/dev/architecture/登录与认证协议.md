...
当前已实施 `cn.vastplan.platform.security.authentication.method.otp`，支持 `enterprise-email-code` 和 `enterprise-sms-code`。它生成无偏数字码，只保存 transaction 绑定 HMAC，并对尝试数、重发冷却、重发次数、TTL 和单次消费做有界限制。重发先通过 CAS 换代挑战，旧码立即失效；并发正确提交只能有一个成功。
