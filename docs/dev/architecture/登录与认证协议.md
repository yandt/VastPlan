...
- `accessTemplate`：选择会话前受治理视觉模板，初始为 `access`；合法可选值使用稳定 ID 形式，如 `access-arco`、`access-mui`；
...
会话前不加载完整 Arco/MUI Runtime，避免放大公网未认证面的代码量和攻击面。Portal Foundation 在同一语义 AuthenticationFlow 上提供最小视觉适配面：`access-arco` 和 `access-mui` 只投影经治理的颜色、圆角、字体、间距和触控尺寸令牌，不执行 Provider 或任意框架代码。`access` 等同默认 Arco Foundation 外观，未知模板也 fail closed 到该默认值；用户级偏好只有建立 Session 后才能读取。
...
| L2 | Foundation `AuthenticationFlow` 语义渲染与 Provider 自动化测试；Arco/MUI 最小视觉适配面 | 已实施 |
...
2026-07-23 已完成上述门禁：Broker 测试以两个 Portal Node 身份竞争同一 Assertion，只允许首个消费；Node 验签测试覆盖 5 秒时钟偏差、过期和 audience/tenant/Portal/transaction 绑定；OTP 测试覆盖重发换代、TTL、尝试锁定、未知主体、Delivery 故障与并发单次成功；数据库 Provider 以可观测验证函数证明已知/未知账号都执行一次同参数 Argon2id 且返回同形错误；Node HTTP 测试覆盖 pre-auth CSRF/Origin 拒绝和新 Session 换代攻击者预置 Cookie。真实 Chromium 验收覆盖键盘 Tab 顺序、ARIA 语义树、RTL、中英文回退、375px 移动端、200% 等效 CSS 视口和 Arco/MUI 外观，修复后无横向溢出且控制台无错误。
