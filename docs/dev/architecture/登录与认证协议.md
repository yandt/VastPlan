...
> 状态：方案 C 已采纳，L0 公共契约已实施｜最后更新：2026-07-23
>
> 本文是会话前登录页面、Access Profile、认证 Broker、登录方法 Provider 和浏览器 Session 签发的单一真相源。架构决策见 [ADR-0108](../decisions/ADR-0108-会话前Access-Profile与认证方法协议.md)，登录后的角色和权限见《[在线角色与权限治理](在线角色与权限治理.md)》与 [ADR-0107](../decisions/ADR-0107-插件权限目录与系统管理授权治理.md)。

## 1. 结论与现状

VastPlan 采用“独立会话前 Access Generation + 统一 AuthenticationFlow + 协议化 Method Provider + Node Session 签发”的方案。密码、临时验证码、OIDC、Passkey 等方法不能自行提供任意前端页面，也不能直接创建浏览器 Cookie。

当前 Node Portal Kernel 已实现 OIDC Authorization Code + PKCE 和开发文件会话，但未登录访问会直接跳转 `/auth/login`，尚无可组合登录页。正常 Portal Generation 依赖已验证 Principal/tenant，不能反向用来加载登录页。新增 Access Profile 以域名和路径在会话前选择最小可信前端基线，解决“先登录才能加载登录页”的循环。
...
Access Profile Catalog 是服务端控制面输入，不直接下发内部 tenant、插件引用或摘要。浏览器只取得裁剪后的品牌、locale、方法描述和当前事务步骤。会话前使用 Platform Profile 默认 Renderer/Theme；用户级偏好只有建立 Session 后才能读取。
...
- Provider 使用 Argon2id、独立 salt，并通过 Credential Material Lease 取得部署级 pepper；数据库不保存明文、可逆密文或 pepper；
...
- `cn.vastplan.platform.security.authentication.method.password`：密码校验 Provider；
...
Broker 和首批密码/OTP Provider 推荐 Go：低延迟、确定状态机、密码学和现有 Backend 契约复用更合适。前端继续 TypeScript/React。短信、邮件等 Delivery Provider 独立按供应商 SDK 选择 Node、Java、Python 或其他语言。运行方式与语言分开：首方 Broker 可进入可信共享 Go Runtime；本地密码校验在高安全生产 Profile 中建议 `dedicated`，外部 Delivery Provider 的隔离由发布者策略决定。

## 6. HTTP、集群和失败语义

计划中的 Node 同源 BFF 路径为：

- `GET /auth/v1/bootstrap`
...
| L1 | Access Profile Catalog 控制面、会话前 Generation、公共 bootstrap 裁剪 | 待实施 |
...
| L6 | 将现有 OIDC 接入 redirect Method，增加在线方法/策略管理 | 待实施 |
