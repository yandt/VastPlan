...
种子 Catalog 只提供首个 PortalVersion 的默认平台配置和服务绑定，仍遵守不可变配置规则：基础插件引用变化必须提升 Catalog 与种子配置 revision、重新计算 digest，并同步 Access Profile 精确引用。种子 Profile 不再拥有在线草稿、审批或发布流程；创建后完整配置复制进 PortalVersion，后续变化只产生该 Portal 的新版本。
...
在线治理以完整 PortalVersion 为唯一审批单位。发布只冻结版本；管理员必须另行创建 PortalRelease，经当前信任校验、物化和 CAS 后才改变线上 Portal。Release 在 CAS 前先保护旧活动与新候选引用并集，成功后先写回滚历史、再收敛活动精确引用；`referencePending` outbox 让仓库瞬时失败与 Composer 重启只造成多保留、不产生漏保护。回滚只能选择同一 Portal 的历史 Release，并创建新的 Release；`system` break-glass 也不能恢复已撤销制品。
