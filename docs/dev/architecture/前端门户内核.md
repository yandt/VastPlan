...
> 状态：v4 已实现，v5 分级热升级实施中｜最后更新：2026-07-21
>
...
生产入口为 `backend portal-edge`。它只接受仓库中的 Composer、Interaction Broker 及各自访问策略的 `id/version/channel` 引用：读取未信任 Envelope 后由内核验证器校验证明、内容和清单绑定，再经内容寻址安装器取得入口与冻结的 runtime contract，最后按该 contract 启动插件。入口必须配置 TLS 证书/私钥、session 文件、Composer/Broker 状态文件、`-portal-platform-catalog`、可信 `-frontend-delivery-origin`、本机 `-frontend-delivery-cache`、集群仓库路由和发布者 trust store；无签名仅可由显式 `--allow-unsigned-local` 用于本地开发，且不能同时配置生产 trust store。Portal 组合可以使用本地 Seed 与可选托管 HTTPS 后备源；仅在 Seed 精确 ref 不存在时回退，任何读取、证明或内容验证异常均 fail-closed。只有 Portal Edge 启动当前 Activation 预取器，普通 Backend 服务不拉取前端快照。启动期间注册 Catalog validate/materialize 与 artifact-reference publish 三个窄能力，再把经验证 session、可信 Catalog 与协议总线组成 BFF；不接受裸二进制路径或裸 Manifest。未配置集群路由的生产 Activation 会 fail-closed；只有显式 `allow-unsigned-local` 可使用不参与 GC 的内存引用校验器。
...
当前实现已完成 UI Contract 4.0、三级导航、统一 Shell Catalog 与标准侧栏/顶部导航独立 Shell Library、Library 延迟下载与候选 Generation 无刷新切换、Arco/MUI 同契约适配、插件内多语种资源、Application/Profile/Binding 分域治理、不可变 Activation、当前值 CAS、精确输入回滚、服务端 Resolver、输入 digest/origin 锁、Catalog 分类复核、类型化控制面客户端、Frontend Test Release 的 Application 与专用测试 Profile 闭环、ESM 制品注入、中央快照与 Edge 本机预取/冷填充、`RuntimeSpec`/模块端点、浏览器字节摘要校验、可部署 Portal Shell、共享依赖 import map、CSP nonce、Shadow DOM/CSS 隔离、内核原生恢复页、通用 JSON Schema 表单、Arco 按需构建，以及认证 Activation SSE、候选 Generation 自动提交和 Host Epoch `pending/active/lastKnownGood/failed` 回退。当前文件状态适配器和进程内 CAS 用于开发及单 leader 部署；ADR-0081 规定的 JetStream KV 聚合 CAS、leader fencing、Origin orphan 容量治理和多 Edge 预热确认仍属于生产封板项。
