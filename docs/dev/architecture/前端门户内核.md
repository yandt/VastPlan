...
当前 `@vastplan/ui-primitives` 已实现上表的语义组件面。Arco 与 MUI 插件都集成 RJSF 6，并共享 `@vastplan/rjsf-csp-validator` 的 Draft 7 解释式校验器；该校验器不使用 `Function/eval`，可在 Portal CSP 下工作。每个 Renderer 只实现自己的 widgets、字段/对象/数组模板、数组操作与错误展示，RJSF 类型不进入公共 UI SDK。同步校验和默认值遵循 JSON Schema，嵌套错误路径使用 `object.field` 与 `array[0].field`。异步校验通过 FormRenderer 参数注入，默认去抖并使用 AbortSignal 取消过期请求。凭证字段必须同时声明 `format: vastplan-credential-ref` 和 `writeOnly: true`，`ui:widget: secretRef` 只控制呈现，不能作为安全依据。
