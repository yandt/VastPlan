...
方案 C 增加会话前 Access Profile，但不复制 Frontend Platform Profile。Node 按 hostname/route 选择精确引用既有基础层的 Access Profile，生成不可变 Access Generation，并通过裁剪后的 `/auth/v1/bootstrap` 公开品牌、语言和方法策略；tenant 与 Platform Profile 锁只留在可信宿主。`/auth/access` 已以 Portal Foundation 固定样式运行统一语义 `AuthenticationFlow`，Method Provider 只实现 `authentication.method.v1` 固定步骤。Broker、OIDC Provider、通用关系数据库密码 Provider、Assertion 消费与 Session 授权绑定均已实施；OTP/Delivery Provider 作为后续可选方法，不影响企业插件式认证主链。
