...
> v4 已将基础单例收敛为 `ui.render.adapter`、`ui.structure.shell`、`ui.workflow.workbench`。Shell 在一个可信插件中固定 Slot/导航语义，并提供 `standard` 与 `top-navigation` 模板；Render Adapter 在一个可信插件中固定语义组件契约，并提供 `arco`、`mui` 等 Renderer。Profile 管理两类模板的默认值、允许范围和用户选择权限。
...
Renderer 选择的默认值、允许范围和用户权限属于 Profile。用户偏好只保存在以 tenant/portal/adapter 隔离的本地数据中；切换先验证允许范围，随后重新装配完整 Portal Generation 并刷新页面，绝不在同一个 React/DOM 树混用两个框架。当前 Renderer 随可信 Adapter 制品一并交付；其签名子制品按需传输需扩展 RuntimeSpec 的多资产校验协议，未实现前不宣称按需加载。
