...
本地开发采用独立传输面：`platformdev` 监听首方前端源码，构建新的单文件 ESM，以 SHA-256 标识模块并通过本机 SSE 发布“候选已就绪”事件；浏览器取得开发 Runtime 覆盖后仍复算实际字节摘要，再交给同一事务管理器。生产构建不连接该事件源，生产 `parsePortalRuntimeSpec` 也不接受开发 URL。开发态不能覆盖不可变仓库中的同版本制品，也不能把开发模块写入 Portal Edge 的正式交付快照。
