...
> 状态：v3 实施中｜最后更新：2026-07-19
>
> 本文是 Frontend Portal Kernel、UI 三层基础插件、在线组合与浏览器安全边界的单一真相源。术语和命名取舍见 [ADR-0083](../decisions/ADR-0083-前端UI分层术语与插件命名空间.md)，其余架构取舍见 [ADR-0052](../decisions/ADR-0052-前端门户内核与多UI设计系统插件.md)、[ADR-0062](../decisions/ADR-0062-Frontend可信ESM制品与运行描述.md)、[ADR-0063](../decisions/ADR-0063-Portal静态宿主与样式隔离.md)、[ADR-0064](../decisions/ADR-0064-Portal语义组件契约与动态表单运行时.md)、[ADR-0065](../decisions/ADR-0065-通用JSON-Schema表单与Arco主题适配.md)、[ADR-0066](../decisions/ADR-0066-Arco按需构建与单文件制品边界.md)、[ADR-0067](../decisions/ADR-0067-Portal控制面闭环安全恢复与第二适配器验收.md)、[ADR-0073](../decisions/ADR-0073-Portal内容寻址交付快照.md)、[ADR-0074](../decisions/ADR-0074-Portal组合Slot与纯布局插件分层.md)、[ADR-0075](../decisions/ADR-0075-Portal管理绑定与多平台基线.md)、[ADR-0076](../decisions/ADR-0076-Portal-Edge分布式快照交付.md)、[ADR-0078](../decisions/ADR-0078-Frontend事务式热替换与插件生命周期.md)、[ADR-0079](../decisions/ADR-0079-Frontend框架无关多语种契约.md)、[ADR-0080](../decisions/ADR-0080-Portal三级导航与可切换布局.md)、[ADR-0081](../decisions/ADR-0081-Portal治理与不可变Activation.md)、[ADR-0082](../decisions/ADR-0082-前端工作台组合框架.md) 和 [ADR-0086](../decisions/ADR-0086-单Shell插件与可切换布局模板.md)；视觉与交互规范见《[Portal 设计系统](../design/DESIGN.md)》；组合模式见《[UI 工作台组合框架](UI工作台组合框架.md)》；Shell 收敛的目标模型与迁移阶段见《[Portal 单 Shell 多模板设计](Portal单Shell多模板.md)》；Portal 与 Mobile/Runner 的跨端体验协作见《[跨端体验与交互契约](跨端体验与交互契约.md)》。

> v3 已将基础单例收敛为 `ui.render.adapter`、`ui.structure.shell`、`ui.workflow.workbench`。Shell 在一个可信插件中固定 Slot/导航语义，并提供 `standard` 与 `top-navigation` 模板；Profile 管理默认值和允许范围，浏览器只接受允许范围内的用户模板偏好。
...
flowchart TB
  ...
  PK --> SC["ui.structure.composition\n页面与 Slot 统一管理"]
  PK --> SL["ui.structure.layout\n纯视觉布局"]
  SC --> FP["前端功能插件\n页面声明与 Slot 内容"]
  DS --> UI["@vastplan/ui-primitives\n稳定组件语义"]
  FP --> UI
```

内核负责：可信 Runtime 装载、四个平台基础插件（渲染适配、结构组合、结构布局、Workbench）的单例选择、Portal 路由、权限可见性过滤、插件生命周期、错误隔离、最小恢复界面与 BFF 客户端。

渲染适配器插件负责：主题 token、**主题模板目录**、语义组件、导航控件、弹窗/抽屉/通知、动态表单、数据展示、空态/错误态和图标。主题模板是 `id`、可本地化标签与 `light`/`dark`/`high-contrast` 语义方案组成的通用契约；Platform Profile 只能以 `renderAdapter.config.themeTemplate` 选择适配器已声明的模板，未知或畸形目录在装配期拒绝。Arco、MUI 和未来框架各自把该模板映射到其原生主题 API 或 CSS 机制；Profile 不得传 CSS、颜色值、URL 或框架私有主题对象。Workbench 负责集合、表单与 Overlay 的展示档位，例如紧凑/标准/宽松密度；Shell 组合插件负责 Slot 拓扑与内容归并；布局插件只负责 LOGO、菜单、页头、正文和侧栏的视觉位置与样式。

功能插件负责：以 UI 契约组合自己的视图、命令、编辑器和表单 Schema，通过 `addPage` 声明导航语义区并填充标准 Slot；不得控制 Page Shell、顶级 HTML、全局样式、会话令牌或跨插件内部调用。

## 2. 设计系统与多框架

`ui.render.adapter`、`ui.structure.composition` 与 `ui.structure.layout` 是 Frontend 的三个独立 `single` 扩展点。Portal Platform Profile 分别精确固定插件 ID、制品版本和 `uiContract` 兼容范围；装配前必须验证贡献存在、来自三个不同的已签名第一方制品且满足范围。Application Composition 不包含这三种选择，一个 Portal 同时只能激活各一个实现。

当前有两个第一方 render adapter：`cn.vastplan.foundation.frontend.render.adapter.arco` 以 Arco Design 提供完整主题和高级 JSON Schema widgets，`cn.vastplan.foundation.frontend.render.adapter.mui` 以 Material UI 提供同一 3.x 语义组件面与标准 RJSF 表单基线。二者分别用于不同 Portal，功能插件不声明对其中任一框架的依赖。框架切换是 Platform Profile 新 revision 与 PortalActivation，不是普通 Application Composition 编辑；候选插件必须先通过契约、来源和制品校验。
...
Portal Platform Profile 包含设计系统、Shell 组合、Shell 布局、Workbench、安全加载与恢复基线；Portal Application Composition 包含 `route`、`domains`、`audience`、`branding`、功能插件精确 refs 和非敏感 `config`；PortalBinding 固定管理服务与 operation grants。三者分别形成可审批、可发布的不可变 revision，但 Published 只表示可被引用。只有 `PortalActivation` 精确锁定三份 `id + revision + digest` 与物化快照并成为线上事实。同一路径/域名在一个租户内唯一；平台基线必须恰有四个相互独立的 UI 基础插件；功能插件必须满足其声明的 `uiContract`。
...
当前实现已完成 UI Contract 3.0、三级导航、标准侧栏与顶部导航独立布局插件、Arco/MUI 同契约适配、插件内多语种资源、Application/Profile/Binding 分域治理、不可变 Activation、当前值 CAS、精确输入回滚、服务端 Resolver、输入 digest/origin 锁、Catalog 分类复核、类型化控制面客户端、ESM 制品注入、中央快照与 Edge 本机预取/冷填充、`RuntimeSpec`/模块端点、浏览器字节摘要校验、可部署 Portal Shell、共享依赖 import map、CSP nonce、Shadow DOM/CSS 隔离、内核原生恢复页、通用 JSON Schema 表单与 Arco 按需构建。当前文件状态适配器和进程内 CAS 用于开发及单 leader 部署；ADR-0081 规定的 JetStream KV 聚合 CAS、leader fencing、Origin orphan 容量治理和多 Edge 预热确认仍属于生产封板项，不能用当前单机闭环替代其集群证明。
...
热替换按依赖边界分为两级。功能、结构布局、结构组合和 render adapter 插件源码变化时，浏览器继续在当前文档内事务式切换整个 Portal Generation。Portal Kernel、静态宿主、`ui-primitives`、`ui-contract`、vendor 构建入口或依赖锁变化时，`platformdev` 必须先完整构建并验证宿主与插件候选，再原子切换开发网关持有的内存静态资源和模块集合，最后通过独立 `reload` 事件刷新页面。旧页面不能先加载新插件；这避免新插件链接到缺少命名导出的旧共享 vendor。Portal Edge 的生产静态快照仍为启动时不可变内存对象，不参与开发态动态切换。
