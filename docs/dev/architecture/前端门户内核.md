...
Renderer 与 Shell Library 的默认值、允许范围和用户权限属于 Profile。服务端 PortalPreference 已作为租户/Portal/用户隔离的跨设备真源，localStorage 只作相同 Catalog scope 下的验证缓存；浏览器不能提交身份、scope、模块引用或样式。Shell Library 在同 Renderer family 与兼容 UI Contract 下通过候选 Portal Generation 无刷新切换；Renderer family 使用 `pending/active/lastKnownGood` Host Epoch 受控刷新，成功启动后才提交服务端偏好，绝不在同一 React/DOM 树混用两个框架。签名子模块按需传输必须由 RuntimeSpec 锁定摘要和来源，未锁定模块不下载、不执行。
...
> P2.3 目标模型已由 [ADR-0174](../decisions/ADR-0174-Portal可选版本控制与发布快照分离.md) 修订，实施设计见 [Portal 可选版本控制接入](Portal可选版本控制接入.md)。当前代码仍使用下述 PortalVersion 工作流；P2.3 将其拆为 WorkingCopy、Publication、Release 与可选 VersionControlBinding。未配置 Workspace 时不再产生通用版本历史，但冻结审批候选和 Release 安全语义保持不变。

`Portal` 是唯一在线治理聚合。每个 `PortalVersion` 同时保存 Runtime Engine、Render Adapter、Shell、Workbench、安全策略、`route`、`domains`、`audience`、`branding`、功能插件、非敏感 `config` 以及管理服务 grants。版本整体执行审批和发布，Published 只表示内容已冻结；只有 `PortalRelease` 精确引用一个 Published PortalVersion 并完成物化后才成为线上事实。同一路径/域名在一个租户内唯一；Foundation 子模块只能由可信种子 Catalog 初始化，功能插件必须满足声明的 `uiContract`。

种子 Catalog 只提供首个 PortalVersion 的默认平台配置和服务绑定，Composer 将它作为可信 `creationTemplate` 投影给管理界面，因此空租户不依赖既有 Portal 也能创建第一条记录。该模板仍遵守不可变配置规则：基础插件引用变化必须提升 Catalog 与种子配置 revision、重新计算 digest，并同步 Access Profile 精确引用。种子 Profile 不再拥有在线草稿、审批或发布流程；创建后完整配置复制进 PortalVersion，后续变化只产生该 Portal 的新版本。

机器契约位于 `contracts/schemas/composition/frontend/v1` 与中立 `portalapi`。浏览器提交完整 PortalVersion 配置，但不能提交 tenant、Principal 或可信解析结果。Composer 在锁外验证并物化候选，随后以治理聚合 CAS 确认 PortalVersion、当前 Release 和全局不变量未变化，再提交 PortalRelease 与审计。内核 Catalog 对精确制品、来源、当前撤销状态与管理绑定执行二次校验，浏览器 Portal Runtime 只消费 Release 锁定结果。
...
Node BFF 调用 Composer 的能力名固定为 `tool.package/platform.portal-composer`。管理操作收敛为 `portalGovernance`、Portal 创建、PortalVersion 创建/更新/删除/状态迁移、`releasePortalVersion`、`rollbackPortalRelease` 与 `audit`；运行时只读取 `listPortalReleases`。BFF 使用窄 `PortalComposerPort`，再由 Addressing 适配器执行集群寻址。Composer 只从宿主 `CallContext` 投影 Principal/tenant。
...
在线治理以完整 PortalVersion 为唯一审批单位。发布只冻结版本；管理员必须另行创建 PortalRelease，经当前信任校验、物化和 CAS 后才改变线上 Portal。普通 Release 只接受最新且从未上线的 Published 版本，旧版本恢复必须走同一 Portal 的显式 rollback 并创建新的 Release，避免两条入口形成不同审计语义。Release 在 CAS 前先保护旧活动与新候选引用并集，成功后先写回滚历史、再收敛活动精确引用；`referencePending` outbox 让仓库瞬时失败与 Composer 重启只造成多保留、不产生漏保护。所有资源身份以可信 URL 和服务端状态为准；`system` break-glass 必须说明原因、记录高优先级审计，且不能恢复已撤销制品。
