...
方案 C 增加会话前 Access Profile，但不复制 Frontend Platform Profile。Node 将按 hostname/route 选择 Access Profile 精确引用的 Runtime Engine、Render Adapter、Shell 与 Workbench，在 `access` 模板中运行统一 `AuthenticationFlow`；Method Provider 只实现 `authentication.method.v1` 固定步骤。该合同和 ADR 已完成，Access Generation、Broker、密码/验证码 Provider 尚待后续阶段实现；在其通过真实验收前，现有 OIDC 仍是生产登录入口。
