...
标准布局插件 `com.vastplan.foundation.frontend.layout.standard` 只把该组合模型映射为视觉结构。桌面默认使用 64px 图标主轨和右侧常驻 240px 二级导航栏，不使用二级弹出菜单；主轨和二级栏分别管理溢出滚动。Page Header 位于 Page Body 主滚动容器之外，正文滚动时保持可见；正文使用由语义 surface/text 派生的中性 canvas，让导航、页头与内容面板形成稳定层级。手机隐藏桌面双栏并使用设计系统 Drawer。`layout.config` 只保存正文宽度等视觉策略，不能新增 Slot、分组或注入 CSS。设计系统适配器必须把公共响应式断点按较小断点向上继承，并由 Table 组件自身管理窄视口横向溢出，不能把这些框架差异泄漏给功能插件。
