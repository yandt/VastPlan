...
当前 `@vastplan/portal-ui` 已实现上表的语义组件面。Arco 插件在内部集成 RJSF 6 + AJV 8，并提供完整 Arco widgets、字段/对象/数组模板、数组操作与错误展示；RJSF 类型不进入公共 SDK。同步校验和默认值遵循 JSON Schema，嵌套错误路径使用 `object.field` 与 `array[0].field`。异步校验通过 FormRenderer 参数注入，默认去抖并使用 AbortSignal 取消过期请求。凭证字段必须同时声明 `format: vastplan-credential-ref` 和 `writeOnly: true`，`ui:widget: secretRef` 只控制呈现，不能作为安全依据。

V1 禁止远程 `$ref` 并限制 Schema 大小、深度与节点数；后端在接受交互请求时编译内嵌 Schema，浏览器 AJV 不配置网络 loader。Mobile 与其他 Renderer 消费同一数据 Schema，可忽略 Web `uiSchema` 并采用自己的布局；升级 JSON Schema 方言必须显式版本化。
...
└── modules[]
    ├── id / version / channel
    ├── entry
    ├── url: /v1/portal-modules/{revision}/{pluginID}.js
    ├── sha256: 入口 JavaScript 实际字节摘要
    └── packageSha256: 已验证插件包摘要
```

Edge 生成描述和读取模块都重新经过 `ArtifactVerifier`，并从包内读取 Manifest 锁定的入口；模块 URL 不允许浏览器选择版本、channel 或任意包内路径。只有当前激活 revision 的模块可读。Portal Loader 使用同源会话获取字节，以 Web Crypto 复算 `sha256`，完全匹配后才创建 Blob URL 执行。模块自行导出的签名、发布者或 integrity 声明一律不可信，provenance 只能由宿主赋值。

加载后的插件只得到宿主创建的窄 `FrontendPluginContext`。宿主绑定真实插件 ID；路由必须唯一且提供组件；菜单只能指向本插件已注册路由。设计系统仍须通过单一贡献、完整 capability 与 UI contract 检查，普通模块不能注册第二设计系统。
