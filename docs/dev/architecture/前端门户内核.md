...
> 状态：v5 已实现，v6 Node Portal Kernel 与可信模块图迁移实施中｜最后更新：2026-07-22
>
...
内核负责：可信 Runtime 装载、四个平台基础插件（Runtime Engine、渲染适配、Shell、Workbench）的单例选择、Portal 路由、权限可见性过滤、插件生命周期、错误隔离、最小恢复界面与 BFF 客户端。Node Portal Kernel 逐步接管原 Go Portal Edge 的 HTTP、会话、BFF、静态宿主、模块交付与 Generation 协调；制品信任、权限裁决、集群寻址和插件治理仍由 Go Backend 通过窄端口提供。
...
Node Portal Kernel 必须用显式静态资产目录启动，并在接管流量前验证 index、nonce 占位符、文件类型、数量、总大小和符号链接边界。未知 `/v1/*` 始终返回 404，不能被 SPA fallback 掩盖；普通页面路径返回带每请求随机 nonce 的 shell。CSP 只允许同源脚本、nonce import map 和 Loader 所需的 `blob:` 模块，并禁止 frame ancestor、object、非受控 worker 与跨域连接。迁移期间 Go Edge 与 Node Host 必须通过同一组黑盒安全测试，Node 达到等价后才允许删除 Go 入口。
...
迁移中的 Node 入口以 `--frontend-delivery-cache` 配置本机私有缓存，以可选 `--frontend-delivery-origin` 配置可信中央快照目录；origin 不能脱离 cache 单独启用。Node 与 Go 共享 `snapshots/{tenant+portal digest}/{revision}.json` 和 `objects/{digest prefix}/{digest}.blob[.gz]` 格式，但 Node 不盲信文件：先以当前 Activation 的 Go 序列化兼容摘要绑定 snapshot，再校验每个 URL 的 revision/digest/media type 和对象实际 SHA-256，gzip 还需解压后复核。冷填充写完并验证全部对象后才原子提交 snapshot。该兼容读路径不等于 Catalog 所有权已迁移；在 Node 接管 validate/materialize 和平台预取器之前，Go Edge 仍负责可信物化并保持生产后备。
...
当前实现已完成 UI Contract 4.0、三级导航、统一 Shell Catalog、Arco/MUI 同契约适配、Workbench、插件内多语种资源、分域治理、Activation、服务端 Resolver、Catalog 复核、CSP/Shadow DOM、Generation 热替换与 Host Epoch 回退。本轮已落地 `ui.runtime.engine` 机器契约、React Engine 首个实现和四基础单例校验；Node Portal Kernel 已建立按配置、静态资产、HTTP 安全与服务生命周期分层的基础宿主，并以黑盒测试守护 CSP、nonce、ETag、HEAD、SPA fallback 和 `/v1` fail-closed。Module Graph 已从签名 Manifest 贯通 Go Artifact Trust、Catalog、内容寻址交付快照、RuntimeSpec 和 Browser Loader。Node Addressing 已直接复用 NATS + Protobuf v1、签名 Capability Directory、NKey 身份/重放保护与精确 capability allowlist，并接入 Portal Host 生命周期；Portal Composer 的全部现有 Web BFF 和 Interaction Web BFF 已按资源工作流拆分。平台管理已完成当前 Activation、Audience、Binding 结构/摘要与服务目标的可信解析，并实施内核 allowlist + Binding grant 双重授权；Settings、Credentials、Database、Artifacts 与 Deployment HTTP 工作流已按领域独立迁移，其中 Artifacts 拆为查询、生命周期、GC、迁移，Deployment 拆为节点引导、服务发布、测试发布。RuntimeSpec、Recovery、内容对象和按需共享 SSE 更新也已由 Node 提供；运行契约、Activation 选择、交付存储、路径解析和 HTTP 响应分别实现，未集中到单一路由文件。通用 Addressing/CallContext 投影与各领域 capability 白名单分别实现，HTTP 输入、CSRF、路径身份投影和能力响应映射保持独立。其余 Catalog validate/materialize/平台预取所有权、服务端 Generation/SSR Worker 与多端统一权限仍在按 ADR-0103～0106 实施；在等价测试通过前，Go Edge 保持生产后备，不提前删除。
