...
>
> 本文是 Frontend Portal Kernel、设计系统插件、在线组合与浏览器安全边界的单一真相源。取舍见 [ADR-0052](../decisions/ADR-0052-前端门户内核与多UI设计系统插件.md)；Portal 与 Mobile/Runner 的跨端体验协作见《[跨端体验与交互契约](跨端体验与交互契约.md)》；统一插件清单与扩展点见《[插件契约与协议](插件契约与协议.md)》。
...
生产入口为 `backend portal-edge`。它只接受仓库中的 Composer `id/version/channel` 引用：读取未信任 Envelope 后由内核验证器校验证明、内容和清单绑定，再经内容寻址安装器取得入口与冻结的 runtime contract，最后按该 contract 启动插件。入口必须配置 TLS 证书/私钥、session 文件、Composer 状态文件和发布者 trust store；无签名仅可由显式 `--allow-unsigned-local` 用于本地开发，且不能同时配置 trust store。启动期间注册 `kernel.portal.catalog.validate`，再把经验证 session、可信 Catalog 与协议总线组成 BFF；不接受裸二进制路径或裸 Manifest。

在线组合 API 的资源为版本化 Draft：创建或编辑产生 draft revision；提交后进行制品、依赖、路由冲突、UI 契约、权限和 Schema 校验；不同 Principal 审批后才可发布。发布保留旧版本；回滚只能选择同一 Portal 的**非当前**已发布历史 revision，并创建新的 active 发布 revision，不能把当前 live revision 复制为伪回滚。`system` break-glass 发布必须携带原因、强制审计并产生高优先级事件。
