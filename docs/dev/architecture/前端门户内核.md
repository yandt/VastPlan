...
当前实现已完成 Frontend v1 双输入 Schema、服务端 Resolver、输入 digest/origin 锁、Catalog 分类复核、发布/回滚重新解析、ESM 制品注入、Edge `RuntimeSpec`/模块端点、浏览器字节摘要校验、宿主 provenance 赋值、可部署 Portal Shell、共享依赖 import map、CSP nonce、Shadow DOM/CSS 隔离、完整 Portal UI 语义组件面、RJSF/AJV 通用表单与 Arco 主题适配、Arco 组件与样式按需构建，以及 Portal Runtime 的解析锁、路由和菜单边界复核。Platform Profile 在单个 Composer 进程内不可热切换；平台升级通过候选实例预检和进程切换完成。

尚未完成的 Portal v1 工程项是：组合管理全流程页面、设计系统故障回退，以及真实浏览器验收。它们不得因 UI 契约、Edge、Loader 与静态 Shell 已实现而标记完成。
