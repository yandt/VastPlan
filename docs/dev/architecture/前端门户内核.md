...
>
> 本文是 Frontend Portal Kernel、设计系统插件、在线组合与浏览器安全边界的单一真相源。取舍见 [ADR-0052](../decisions/ADR-0052-前端门户内核与多UI设计系统插件.md) 和 [ADR-0062](../decisions/ADR-0062-Frontend可信ESM制品与运行描述.md)；插件分级与双输入组合见 [ADR-0057](../decisions/ADR-0057-插件分级管理与双输入组合解析.md) 和 [ADR-0059](../decisions/ADR-0059-Frontend双输入服务端权威解析.md)；Portal 与 Mobile/Runner 的跨端体验协作见《[跨端体验与交互契约](跨端体验与交互契约.md)》。
...
所有远程模块共享单例 `react`、`react-dom`、`@vastplan/portal-ui` 和 `@vastplan/ui-contract`。V1 由 Portal Shell 的 import map 提供共享依赖；设计系统 CSS 必须作为受治理资产在 Portal 根容器内作用域化，功能插件不得携带全局 reset 或框架私有样式。UI 契约的 major version 不兼容即拒绝装配。
...
当前实现已完成 Frontend v1 双输入 Schema、服务端 Resolver、输入 digest/origin 锁、Catalog 分类复核、发布/回滚重新解析、ESM 制品注入、Edge `RuntimeSpec`/模块端点、浏览器字节摘要校验、宿主 provenance 赋值，以及 Portal Runtime 的解析锁、路由和菜单边界复核。Platform Profile 在单个 Composer 进程内不可热切换；平台升级通过候选实例预检和进程切换完成。

尚未完成的 Portal v1 工程项是：可部署 Portal Shell 静态产物与共享依赖 import map、设计系统 CSS 资产治理、完整 Arco UI 契约与动态表单字段、组合管理全流程页面、设计系统故障回退，以及真实浏览器验收。它们不得因 Edge 与 Loader 已实现而标记完成。
