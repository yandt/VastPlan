...
个人中心不属于 Portal 内核业务实现。独立基础插件 `cn.vastplan.foundation.frontend.identity.account-center` 以标准 `addPage` 注册用户信息和外观页面；它因使用可信 Host 本地个性化窄端口而归入 `foundation.frontend`，不作为普通应用插件暴露。Shell 只把受治理 `account` 根分组渲染为专用头像入口。页面、二级/三级菜单、活动路径、深链接和 Generation 换代均继续通过同一 `PortalRegisteredPage → ShellCompositionModel` 数据链，布局插件不得维护自己的账户功能清单。可信 Host 仅用 `PortalPersonalizationProvider` 向插件暴露账户摘要和本地外观窄端口，不暴露会话密封材料或服务端偏好写入能力。
