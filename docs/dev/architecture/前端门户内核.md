...
当前实现已完成 UI Contract 4.0、三级导航、统一 Shell Catalog、Arco/MUI 同契约适配、Workbench、插件内多语种资源、分域治理、Activation、服务端 Resolver、Catalog 复核、CSP/Shadow DOM、Generation 热替换与 Host Epoch 回退。Node Portal Kernel 已按配置、静态资产、HTTP 安全、领域 BFF、交付存储和服务生命周期拆分，未形成单一路由大文件。Module Graph 已从签名 Manifest 贯通 Go Artifact Trust、可信宿主 Catalog、内容寻址交付快照、RuntimeSpec 和 Browser Loader。开发流量现由 Node Portal Kernel 承接；Catalog validate/materialize 与引用保护已通过受调度 Composer 回调 Go 可信宿主完成。剩余封板项是服务端 Generation/SSR Worker、生产 OIDC/NATS mTLS 权限对照和删除 Go Edge。
