...
方案 C 增加会话前 Access Profile，但不复制 Frontend Platform Profile。Node 已能按 hostname/route 选择精确引用既有基础层的 Access Profile，生成不可变 Access Generation，并通过裁剪后的 `/auth/v1/bootstrap` 公开品牌、语言和方法策略；tenant 与 Platform Profile 锁只留在可信宿主。下一阶段在 `access` 模板中运行统一 `AuthenticationFlow`；Method Provider 只实现 `authentication.method.v1` 固定步骤。Broker、密码/验证码 Provider 尚待后续阶段实现；在其通过真实验收前，现有 OIDC 仍是生产登录入口。
