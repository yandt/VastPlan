...
设计系统插件负责：主题 token、命名主题目录、语义组件、导航控件、弹窗/抽屉/通知、动态表单、数据展示、空态/错误态和图标。Platform Profile 只能选择适配器已声明的 `renderAdapter.config.theme`，未知主题在装配期拒绝；它不能传 CSS 或框架私有主题对象。Workbench 负责集合、表单与 Overlay 的展示档位，例如紧凑/标准/宽松密度；Shell 组合插件负责 Slot 拓扑与内容归并；布局插件只负责 LOGO、菜单、页头、正文和侧栏的视觉位置与样式。
