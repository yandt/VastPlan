...
>
> 本文是 Frontend Portal Kernel、设计系统插件、在线组合与浏览器安全边界的单一真相源。取舍见 [ADR-0052](../decisions/ADR-0052-前端门户内核与多UI设计系统插件.md)、[ADR-0062](../decisions/ADR-0062-Frontend可信ESM制品与运行描述.md)、[ADR-0063](../decisions/ADR-0063-Portal静态宿主与样式隔离.md) 和 [ADR-0064](../decisions/ADR-0064-Portal语义组件契约与动态表单运行时.md)；插件分级与双输入组合见 [ADR-0057](../decisions/ADR-0057-插件分级管理与双输入组合解析.md) 和 [ADR-0059](../decisions/ADR-0059-Frontend双输入服务端权威解析.md)；Portal 与 Mobile/Runner 的跨端体验协作见《[跨端体验与交互契约](跨端体验与交互契约.md)》。
...
| 表单 | `FormRenderer(schema, value, context)`、十类内置字段、同步/可取消异步校验、条件显示、只读/禁用、错误摘要与提交状态 |
...
当前 `@vastplan/portal-ui` 已实现上表的语义组件面，Arco 插件完成对应适配。默认值、条件可见性和同步校验由无 UI 依赖的 `@vastplan/ui-contract` 运行时统一执行；嵌套错误路径使用 `object.field` 与 `array[0].field`。异步校验通过 FormRenderer 参数注入，默认去抖并使用 AbortSignal 取消过期请求，网络函数不进入 Schema。V1 不开放框架私有自定义字段注册，新增通用字段必须同时评估 Web 与 Mobile 适配。
...
当前实现已完成 Frontend v1 双输入 Schema、服务端 Resolver、输入 digest/origin 锁、Catalog 分类复核、发布/回滚重新解析、ESM 制品注入、Edge `RuntimeSpec`/模块端点、浏览器字节摘要校验、宿主 provenance 赋值、可部署 Portal Shell、共享依赖 import map、CSP nonce、Shadow DOM/CSS 隔离、完整 Portal UI 语义组件面、Arco 适配和十类动态表单字段，以及 Portal Runtime 的解析锁、路由和菜单边界复核。Platform Profile 在单个 Composer 进程内不可热切换；平台升级通过候选实例预检和进程切换完成。
