...
当前实现已完成 UI Contract 4.0、三级导航、统一 Shell Catalog、Arco/MUI 同契约适配、Workbench、插件内多语种资源、分域治理、Activation、服务端 Resolver、Catalog 复核、单文件 ESM 交付、CSP/Shadow DOM、Generation 热替换与 Host Epoch 回退。本轮已落地 `ui.runtime.engine` 机器契约、React Engine 首个实现和四基础单例校验；Node Portal Kernel 已建立按配置、静态资产、HTTP 安全与服务生命周期分层的基础宿主，并以黑盒测试守护 CSP、nonce、ETag、HEAD、SPA fallback 和 `/v1` fail-closed；Module Graph 已具备签名 Manifest Schema、Go 强类型、入口绑定、依赖闭包、受控共享 externals、大小/重复映射和规范化 digest 校验，前端构建器从 esbuild metafile 生成逐文件图，打包器复核节点并注入签名 Manifest，`artifacttrust` 在仓库接收/读取时再次绑定 tar 实际字节，Go/Node 共用 digest golden。BFF Addressing 客户端、Catalog/RuntimeSpec/Loader 图交付、服务端 Generation/SSR Worker 与多端统一权限仍在按 ADR-0103～0106 实施；在等价测试通过前，Go Edge 保持生产后备，不提前删除。
