...
Renderer 与 Shell Library 的默认值、允许范围和用户权限属于 Profile。目标形态以服务端 PortalPreference 作为跨设备真源，localStorage 只作启动缓存；当前服务端真源尚未实施。Shell Library 在同 Renderer family 与兼容 UI Contract 下通过候选 Portal Generation 无刷新切换；Renderer family 使用 `pending/active/lastKnownGood` Host Epoch 受控刷新，绝不在同一 React/DOM 树混用两个框架。签名子模块按需传输必须由 RuntimeSpec 锁定摘要和来源，未锁定模块不下载、不执行。
