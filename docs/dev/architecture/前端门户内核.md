...
当前 React Engine 的远程模块共享单例 `react`、`react-dom`、`@vastplan/ui-primitives`、`@vastplan/ui-contract` 和两级 `@vastplan/icon-catalog` 入口，由 Portal Shell 的 import map 提供。语义入口只包含 23 个同步图标；完整 846 图标目录按稳定摘要分为 27 个延迟分片，只有 Foundation 图标管理能力显式请求时才加载。Portal Kernel 挂载在独立 Shadow DOM；设计系统 CSS 作为模块图中的已验签资源交付，文档根选择器及 `body[theme]` 等主题宿主选择器映射到 `:host`，普通选择器由 Shadow DOM 隔离。Ant Design/Arco/MUI Renderer 必须按组件或子路径入口构建，禁止整包框架组件和完整主题 CSS；CSS-in-JS Renderer 必须将样式容器绑定到 Shadow Root。功能插件不得导入原始图标目录、携带全局 reset 或框架私有样式。Runtime Engine、Renderer、Shell、Workbench 与 UI 契约的 major version 不兼容即拒绝装配。
