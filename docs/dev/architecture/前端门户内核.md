...
当前实现已完成 Frontend v1 双输入 Schema、多 Profile 平台 Catalog、Portal 管理绑定、服务端 Resolver、输入 digest/origin 锁、Catalog 分类复核、Application 草稿/审批/发布、类型化控制面客户端、ESM 制品注入、中央快照与 Edge 本机预取/冷填充、`RuntimeSpec`/模块端点、浏览器字节摘要校验、可部署 Portal Shell、共享依赖 import map、CSP nonce、Shadow DOM/CSS 隔离、内核原生恢复页、完整 Portal UI 语义组件面、RJSF 通用表单、Arco 按需构建和 MUI 适配器。v2 正在补齐 UI Contract 2.0、三级导航、顶部布局、Profile/Binding 分域治理、不可变 Activation、CAS 状态端口与当前信任恢复；完成前不得把 v1 的 Application 发布语义描述为完整在线基线切换能力。
