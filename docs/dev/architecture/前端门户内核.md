...
机器契约位于 `schemas/composition/frontend/v1`。普通 Draft API 只接收 Application Composition；Portal Edge 以 `-portal-platform-profile` 绑定平台输入，经可信 `kernel.config.get` 向 Composer 注入。Composer 合并后生成带两份输入摘要和逐插件来源锁的 `PortalSpec`，内核 Catalog 对精确制品和来源执行二次校验；浏览器 Portal Runtime 只消费该锁定结果，不接收或合并原始双输入。
