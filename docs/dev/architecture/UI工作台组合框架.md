...
| `record.row` / `card.footer` | 图标 + Tooltip 居中呈现；`record.row` 使用 16×16 的紧凑图标按钮和 8px 间距，行操作最多两个直接可见，其余进入图标化 overflow。行操作不使用实心 primary 背景，危险操作保留危险色；overflow 触发器以 flex 对齐。页面头部等其他区域仍使用常规 44×44 按钮 |
