...
| `record.row` / `card.footer` | 图标 + Tooltip 居中呈现；`record.row` 按 Collection density 使用《[Portal 设计系统](../design/DESIGN.md)》对应的 `sm/md/lg` recipe，行操作最多两个直接可见，其余动作进入每行一个“图标 + 标签”的 action overflow 菜单。行操作不使用实心 primary 背景，危险操作保留危险色；页面头部动作固定使用 `lg` |
