...
默认 `auto-single-row`：不足两行时文本 Enter 提交，选择/布尔/范围完成即提交且不显示操作；达到两行时保留草稿并显示查询与清除。`explicit` 始终要求显式提交。操作单元跨越当前行剩余列并右对齐，因此在每个响应式断点都落于末行末列。FilterPanel 通过 `FormPresentation.labelPlacement=inside-inline` 使用持久 Label，而不是输入后消失的 placeholder：Label 与控件共享当前列宽，保持单行并在 40%/45% 上限内省略，框架 Tooltip 和可访问名称保留全文；功能插件不能覆盖该视觉策略。
...
新增、导入、发布属于页面功能动作，必须放在 Page Header；刷新、列设置属于当前集合的展示控制，保留在集合工具区并同样使用图标与 Tooltip。列设置只有在 Collection 明确声明 `preferences` 时显示，不能把未声明的列暴露给用户；它以触发按钮附近的紧凑 Popover 呈现，无额外“确定”步骤。显示密度使用 24px 高的按钮式单选组；列顺序通过拖拽句柄调整，并为键盘提供 ArrowUp/ArrowDown 等价入口；显隐只使用 `visibility/visibilityOff` 语义图标即时切换，隐藏列整行使用 muted 灰色但仍允许排序和恢复。所有变化即时应用并保存。
