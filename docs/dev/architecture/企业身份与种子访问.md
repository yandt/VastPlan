...
> 状态：企业 Provider、Broker 会话、授权绑定与 Seed 交接可信链已实施｜最后更新：2026-08-05
>
...
```mermaid
flowchart LR
  Client["Portal / Mobile / Desktop"] --> Broker["Authentication Broker"]
...
Provider Profile、Lifecycle、审批测试证据与已发布 Catalog 的长期状态由 Authentication Broker 0.6.0 通过 service-scope fenced Shared State CAS 保存；平台控制数据库就绪后由 SQL Provider 承载，领域 generation 必须与 Store revision 同步推进。Seed 首次配置保留只读 Bootstrap Catalog，但仅在 Shared State 明确尚未发布目录时使用；数据库不可用、损坏或认证失败时禁止回退本机 JSON。短时认证 transaction 与一次性 Assertion 仍是 Leader 有界内存状态，不与长期配置混存。
...
Seed 管理员不是企业用户，也不能套用“忘记密码”流程。其处置按状态区分：

| 场景 | 允许入口 | 禁止项 |
|---|---|---|
| `Uninitialized` 首次安装 | 可信宿主显式启用的一次性 `enrollment` Step | 默认密码、命令行明文密码、公开重置链接 |
| `SeedActive` 至 `HandoffReady` 丢失 Seed 凭据 | 本机或等价硬件证明开启的受审计 Reset Ceremony | 浏览器自助重置、单一远程账号直接覆盖 verifier |
| `EnterpriseActive` 企业身份正常 | 不重置 Seed；企业 Provider 自己管理日常凭据 | 把 Seed 重新变成普通登录入口 |
| `EnterpriseActive` 企业身份不可用 | 本机证明开启短时 Recovery Lease，仅用于恢复企业身份与授权 | 以 Recovery Lease 永久恢复 Seed 登录或跳过企业交接 |

未来的 **Seed Reset Ceremony** 必须是独立的、可替换的受信工作流，而不是 Seed Provider 的一个网页字段。最低要求是：

1. 可信本机/硬件/KMS 平台证明与明确 `reason`；生产环境还需要与发起人不同的第二责任主体批准，不能自己审批自己。
2. 创建带过期时间、单次 nonce、目标 Seed generation 和审计摘要的 Reset Intent；任何状态变化均用 CAS 提交，旧 Intent、过期 Intent 或并发重放一律拒绝。
3. 提交前撤销旧 Seed 会话、Assertion、恢复租约及相关授权投影；写入新的 Argon2id verifier 后旋转 Seed credential epoch，再签发一次性、短时的恢复会话。
4. 不传输、记录或回显明文密码；密码只从受控表单或受保护本机输入读取，统一裁剪首尾 Unicode 空白。成功、失败和撤销均追加不可篡改审计并发送企业告警。
5. Reset 结束后恢复 `EnterpriseActive` 的交接约束：Seed 不重新成为日常 Provider，企业身份、Session、Policy 和 Enforcer 的边界不变。

本地开发的 `platform-dev.sh clean` 只用于清空 `.vastplan/dev-platform` 测试状态并重新验证首次安装，绝不构成生产重置机制。
...
| E3 | Provider Catalog SQL Shared State、Broker 唯一路由与 leader-owned 短时 transaction 边界 | 已实施 |
