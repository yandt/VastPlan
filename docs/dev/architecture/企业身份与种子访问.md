...
本地种子组合已经启用 Seed Access、Authentication Broker 与 Authorization Session 三个基础插件。Seed Access 与 Broker 同属一个认证服务单元，使 Method Provider HostCall 保持在同一可信服务边界；Authorization Session 仍是可独立扩缩的无状态服务。Seed Operator 经固定 Provider Profile `local-seed-access` 转换为非 PII 稳定主体，再由签名 Policy Snapshot 绑定开发平台 Owner 权限；Portal Session 不直接携带 Seed 文件中的账号、密码 verifier 或任意明文凭据。首次管理员必须通过本机 owner-only 文件初始化，不存在源码内置或网关自动生成的密码。
