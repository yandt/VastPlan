...
> 状态：Backend 1.0 已实现；应用侧依赖改由 Planner 派生的边界已采纳、待实施｜最后更新：2026-08-02
...
选主获取期限与已提交 unit 的租约续期生命周期必须分离：前者只限制一次 reconcile 等待选主的时间，后者归 Runtime 所有，直到 unit 被替换、停止或 Runtime 关闭。不得把 reconcile 的临时 `context` 继续作为续租 owner，否则 reconcile 返回会在服务仍标记 Ready 时静默撤销 fencing。`leader` 与 `partitioned` 的 Ready 判定必须持续核对全部预期 Leadership 仍为 Current；续租生命周期意外结束或 fencing 失效时立即转为非 Ready 并停止该 unit，等待控制面重新收敛。

实例策略与状态位置正交：`leader-owned` 表示状态随 owner 管理；`external-shared` 表示服务连续性不绑定当前副本独占的本地耐久真相，既可以由插件直接访问可信共享 Store，也可以经 `strong + remote` 能力委托给明确的耐久真相拥有者。后者不会自动把服务变成 active-active，也不会替外部 SSH、云 API 或数据库迁移提供副作用 fencing；这些执行端仍必须校验 leader epoch/operation token。第一方插件的直接拥有和委托关系分别记录在状态归属清单的 `durableTruth` 与 `delegatedDurableTruth` 中。
...
插件签名清单声明能力边界和实例策略；Platform Profile 声明共享平台服务和运维上限，应用用户通过插件层 Application Intent 选择根插件、Feature、配置及受限副本/放置参数。Composition Planner 生成 Application Composition 提案，既有 Kernel Resolver 将其与 Platform Profile 锁定为 Deployment v2。任何组合输入都不能突破清单的实例策略；应用输入不能选择或覆盖平台管理插件，也不能手写服务依赖、路由或状态模型。

服务配置来源与执行机制正交。首次平台建立从受信 Seed 文件读取，后续在线服务从 Deployment Manager 共享账本读取；两种来源都必须适配为同一组 Platform Catalog/Profile 与 Application Composition 输入，并进入同一个 `deploymentpublisher → Resolver → Deployment v2 → Controller → Node Agent` 链路。不得因为服务来自 Seed 就增加特殊 ServiceUnit、调度器或 Node Agent 分支，见 [ADR-0147](../decisions/ADR-0147-种子与在线服务配置统一发布机制.md)。
...
以下仍是解析后的内部服务规格字段。Platform Profile 可以直接声明共享平台服务；应用 unit 的这些字段由 Planner 从签名 Manifest 和受限运维意图生成，不是用户表单：
...
- 选主 acquisition context 与 Runtime-owned renewal context 已分离；Ready 状态持续校验 fencing，续租意外结束会停止 unit，而不是保留假 Ready；
