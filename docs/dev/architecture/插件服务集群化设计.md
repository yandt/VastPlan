...
- 有状态插件的数据复制或共识协议；
- 按 `logical_service` 和路由域隔离 queue group，当前仍以 capability 作为请求分发组；
...
1. 完成按 `logical_service`/路由域的能力目录查询与 queue group 隔离；
2. 补充跨服务 readiness、依赖传播和本地/远端失效处理；
3. 先实现 leader（数据库 migration、凭证轮换）运行时协议；
4. 最后设计 partitioned，并补充网络分区、租约过期、重复调用和滚动升级 E2E；
5. 代表性插件完成后再执行 soak 和级联恢复测试。
