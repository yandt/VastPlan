...
> 状态：核心已在 UI Contract 4.x 实施，服务端跨设备偏好待实施｜最后更新：2026-07-23
>
...
用户偏好以 `tenantId/portalId/catalogId/contractMajor` 隔离。目标形态以服务端 `PortalPreference` 作为跨设备真源，localStorage 只作启动缓存；选择顺序为有效服务端/本地偏好、Profile 默认、Catalog 默认，均无效则候选装配失败并保留上一 Generation。

当前 4.x 实现已经完成允许范围校验、localStorage 偏好、按需加载和候选 Generation 切换，满足同一浏览器内的用户即时切换；服务端 `PortalPreference` 尚未实施，因此当前选择不能跨设备同步。这个缺口不影响 Catalog/Library 拆分，也不能通过把两个 Library 合回一个模块解决。
...
4. 已将 `standard`、`top-navigation` 转为独立签名、内容寻址、按需下载的 Library；唯一 Shell Catalog 继续拥有 Slot 与组合语义。
5. 已完成受允许范围约束的浏览器本地用户选择、候选提交后偏好保存和失败回退；服务端跨设备 `PortalPreference` 仍待实施。
