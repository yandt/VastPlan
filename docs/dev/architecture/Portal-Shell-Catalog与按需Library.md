...
> 状态：目标架构已采纳，UI Contract 5.0 待实施｜最后更新：2026-07-20
>
...
`standard` 与 `top-navigation` 是首批两个 Library。当前作为 Shell 主模块内部源码的旧目录在 UI Contract 5.0 迁移时转为真实 deferred 模块。
...
```json
{
  "shell": {
    ...
    "defaultLibrary": "standard",
    "allowedLibraries": ["standard", "top-navigation"],
    "userSelectable": true,
    "libraryOptions": {
      "standard": {},
      "top-navigation": {}
...
用户偏好以 `tenantId/portalId/catalogId/contractMajor` 隔离。服务端 `PortalPreference` 是跨设备真源，localStorage 只作启动缓存。选择顺序为：有效服务端/本地偏好、Profile 默认、Catalog 默认；均无效则候选装配失败并保留上一 Generation。
...
1. UI Contract 5.0：新增 Catalog/Library/LayoutPlan/PageOutlet 类型；
2. 服务端 Profile、Resolver、Catalog 和 RuntimeSpec 支持 deferred Library；
3. Portal Kernel 建立稳定 ShellSurface/PageOutlet 与候选选择事务；
4. 把 standard/top-navigation 转为独立 Library；
5. 增加 PortalPreference 服务端真源和界面选择入口；
6. 增量 HMR、真实制品 Activation 和 Host Epoch 恢复闭环。
