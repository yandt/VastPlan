...
`standard` 与 `top-navigation` 是首批两个 Library，现已作为独立签名制品进入 Catalog，并在 RuntimeSpec 中作为 deferred 模块锁定。它们是可按需下载的 Shell Library，不是两套拥有 Slot、页面归并和导航语义的完整 Shell 基础插件。两者的二、三级菜单必须共同使用 `PortalNavigationMenu`：标准布局选择常驻 `inline` 呈现并保存本会话展开态，顶部布局选择 `popup` 呈现；两种呈现都以原标准侧栏的 44px 行高、12px 内边距、8px 圆角、悬停和选中态为视觉基准，菜单行、层级、图标、账户退出和无障碍行为由同一 Renderer `Menu` 契约实现。
