...
> 状态：B1 权限目录与 B2 Authorization IR/Provider Protocol 已实施｜最后更新：2026-07-22
>
...
B2 定义 `PolicySnapshot` 与 Ed25519 签名信封的 wire shape，但不负责签发、密钥托管或校验运行时；这些属于 B3 Policy 和 B4 Enforcer。当前也没有任何 Provider 实现被默认信任或自动装载。
...
- high/critical 只在策略要求的在线复核或极短决定 TTL 内继续，否则 fail-closed；
...
OIDC Session 只保存经验证主体、租户和外部角色/组引用，不把全部权限固化进浏览器 Token。Node Portal Kernel 向 Enforcer 请求当前 Portal/服务所需的裁剪决定或短期签名体验投影，用于导航和按钮；每次 Backend 调用仍重新判定。
...
| B3 | `authorization-policy`：Catalog、角色、绑定、审批、撤权、Provider Profile 与签名快照 | 待实施 |
| B4 | `authorization-enforcer`：每内核本地快照、Engine Provider、Decision/Proof、缓存和 fail-closed | 待实施 |
| B5 | `role-management` Workbench：权限汇总、角色 revision、主体绑定和审计 | 待实施 |
| B6 | OIDC 外部组映射、Portal 体验投影；移除硬编码角色表和通用管理员旁路 | 待实施 |
