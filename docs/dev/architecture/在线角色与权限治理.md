...
> 状态：B1—B6 已实施，B7 随 Mobile/Desktop 推进｜最后更新：2026-07-22
>
...
`approval` 声明 `none/different-subject/two-person/policy` 语义。`policy` 表示领域服务调用组合根选择的 `approval.policy.v2` Provider；它只是治理元数据，不是授权结论。最终主体、冻结摘要、职责分离和审批状态始终由领域服务以真实对象强制执行。Portal 首个 Provider 接入见 [ADR-0189](../decisions/ADR-0189-审批策略Provider与声明式规则.md)。
...
| 服务与节点 | `platform.deployment.read/write/bootstrap/compose/approve/publish/test-target`、`platform.deployment.plugin.preview/request/approve/activate` | 服务组合与插件安装分别授权；插件预览、申请、审批、激活/回滚分离，审批要求不同主体 |
| 全局设置 | `platform.settings.read/write` | 写权限不能由通用管理员隐式获得；Bootstrap Policy 继续保护自举键 |
| 凭证 | `platform.credentials.read/audit/write/rotate/revoke` | 元数据读取、脱敏生命周期审计、写入、轮换、撤销分离；永不授予明文读取 |
| 数据库连接 | `platform.database.read/write/probe` | 连接定义管理不等于 SQL 数据面授权 |
| Portal 治理 | `platform.portal.read/compose/approve/publish` | Profile、Binding、Application、审批、Activation 和回滚分离；精确 operation 由 Composer Manifest 绑定 |
...
- `RoleRevision`：角色 ID、作用域、权限选择器、Catalog digest、已展开精确权限、可选资源选择器和委托上限；
...
角色采用 Draft → PendingApproval → Approved → Published。高风险角色修改和安全管理员绑定要求不同主体审批。治理输入支持精确权限和受限 Glob：`*` 匹配一个点分段，`**` 匹配一个或多个点分段，首段必须是字面命名空间。Policy 服务在当前签名 Catalog 与 Domain ceiling 内展开并冻结精确权限；未来 Catalog 新增权限不会自动进入旧 Role revision。角色发布不能包含 Catalog 中未知、不可分配、已失效或作用域不相容的权限；插件卸载后 code 保留为 retired/orphaned，禁止被其他插件重新解释。
...
开发环境中，Seed 管理员可能晚于授权 Shared State 初始化。只有显式 `bootstrap` 可由可信组合根注入 `seed-owned` 协调策略，把 Bootstrap State 中 `seed-authority` 所有的 revision-1 Published Role/Binding 原子同步到 Shared State；普通 `up/restart` 始终注入禁用策略。协调不得改写用户角色，保留 ID 冲突、异常 revision 和悬空 Binding 均 fail closed。
...
```mermaid
flowchart LR
  UI["authorization-policy frontend\n在线管理与审批"] --> AP["authorization-policy backend\n统一模型与签名快照"]
...
- `cn.vastplan.platform.security.authorization-policy`：全栈授权治理插件；Backend 是唯一授权真相源，Frontend 提供 Workbench 管理面并只调用同一插件的管理协议；
- `cn.vastplan.foundation.security.authorization-enforcer`：每个内核服务本地运行，拥有 fail-closed PEP 和快照切换；
- Provider 插件：实现存储、判定引擎、主体目录或受控导入导出，不取得上面两个职责。
...
Authorization IR v1 固定包含 Provider Profile、Policy Domain、已编译精确 Role、精确 Subject Binding 和单调 Revocation。Glob 只属于 Policy 治理输入，不进入 IR、Engine、Session 或多端投影。Statement 只支持显式 allow/deny、有限资源选择器，以及 `eq/in/prefix` 属性约束；同一 Statement 内为 AND，OR 通过多个 Statement 表达。Provider 不得把 Rego、Cedar、Casbin model 或其他私有表达式塞入 IR 未知字段。IR 在签名/摘要前按稳定 ID、revision 和集合字段规范排序，时间统一 UTC。
...
B2 定义 `PolicySnapshot` 与 Ed25519 签名信封的 wire shape；B3 已实现 leader Policy 与 Ed25519 签发。ADR-0156 将 Role、Binding、撤权和审计账本从 leader 本地文件迁移到 service-scope fenced Shared State：业务 generation 与 Store revision 分离，leader 漂移可接管同一真相源，存储不可用时 fail-closed。B4 已实现每内核 Enforcer 和独立的默认 Native `authorization.engine.v1` Provider 插件。其他 Store/Engine/Directory/Exchange Provider 仍需显式进入受信 Provider Profile，不会被自动信任或装载。

## 7. 集群、故障和多门户

`authorization-policy` 是平台真相源，以 `leader + external-shared` 运行；每个内核服务运行本地 `authorization-enforcer`，消费签名快照并原子切换。策略服务故障时：
...
Mobile 使用自己的 OAuth/设备载体，Desktop 使用设备身份和 Execution Lease，但三端消费同一 permission code、Role revision 和 Policy revision。高风险系统管理权限永远不能进入 Desktop 离线租约。
...
| B2 | Authorization IR、Policy Domain 与 store/engine/directory/exchange Provider Protocol 契约 | 已实施；Store/Engine 默认运行接线待去直连 |
| B3 | `authorization-policy`：Catalog、角色、绑定、审批、撤权、Provider Profile 与签名快照 | 已实施 |
| B4 | `authorization-enforcer`：每内核本地快照、Decision/Proof、缓存和 fail-closed | 默认本地判定已实施；Engine Provider 运行接线待完成 |
| B5 | `authorization-policy` Frontend Workbench：权限汇总、角色 revision、主体绑定和审计 | 已实施；ADR-0152 起与 Backend 共用全栈制品 |
| B6 | 外部组可信目录投影、Portal 体验投影；移除硬编码用户角色表和通用管理员旁路 | 已实施 |
| B7 | Mobile/Desktop 载体与 Execution Lease 集成 | 随对应内核实施 |

当前跨节点边界仍需保持诚实：Shared State 已解决治理账本随 leader 本机丢失的问题，但签名 Snapshot、Trust 和 LKG 仍由本地文件 Adapter 物化。Authorization Policy 现以两阶段后台控制器协调：ACTIVATE 只注册控制器，宿主确认 Active 并授予 Leader Fence 后，控制器立即从权威 Store 协调 `SnapshotLeasePolicy`、受管 Binding 和本地 Projection，随后按精确续签边界设置定时器；`platformdev` 普通启动不再形成第二个续签权威。代码链不判断开发/生产，差异只由组合根 Profile 注入。首次协调前消费者继续使用签名材料的 fail-closed/LKG 语义，不允许控制器伪造身份或绕过 Fence。

跨节点 `authorization.snapshot.v1` 发布/订阅协议仍待实施。完成后可替换本地文件 Adapter，但不得让 Enforcer 跨插件读取 Policy 私有 Shared State，也不得把签名私钥下发给 Enforcer。类似地，`authorization.store.v1` 和 `authorization.engine.v1` 虽已有公共契约，当前默认 Policy Store 与 Enforcer 求值仍分别直连 Shared State Go Adapter和本地 Go Engine；必须在后续阶段改为真实 Provider Port 后，才能宣称授权存储与判定均已完全多协议化。
...
- `authorization-enforcer` 以优先级 2000 部署在平台服务内。它只治理 Catalog 已声明的用户操作；系统、插件、Agent 和 Desktop 的 workload grant 继续由窄策略处理，人的角色不会传播给它们。
- `platform-admin-access-policy` 逻辑规则已删除用户 operation-role 表与 `platform.admin` 放行，只保留精确系统/插件回调和未知平台 workload 的 fail-closed 防线；ADR-0152 起它作为 Authorization Enforcer 内部 Policy Bundle 运行，不再拥有独立插件进程。Settings、Credentials、Database、Artifacts、Deployment 与 API Exposure 均由各自 Manifest 拥有权限声明。
- Portal Composer 1.8.0 已完成同样迁移：签名 Manifest 用统一 Capability Contract 拥有 `platform.portal.*`，Portal Policy Bundle 对用户操作弃权。Composer 不再二次读取 Token 中的 `portal.*` 角色决定权限，只校验可信主体、租户、对象状态、CAS 与异人审批，避免在线 Role/Binding 与旧角色表发生冲突。
