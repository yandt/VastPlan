...
当前跨节点边界仍需保持诚实：Shared State 已解决治理账本随 leader 本机丢失的问题，但签名 Snapshot、Trust 和 LKG 仍由本地文件物化。生产多节点需要后续签名发布/订阅协议把已提交 Snapshot 分发给各 Enforcer；不得让 Enforcer 跨插件读取 Policy 私有 Shared State，也不得把签名私钥下发给 Enforcer。

回环开发环境额外实施有界启动续签：`platform-dev up/restart` 只在已发布 Permission Catalog、Bootstrap Policy、签名身份和 Trust 完全一致，且 Snapshot 或 `seed-authority` Binding 距到期不足六小时时，续签既有开发租约并写入独立审计。它不会吸收 Catalog 漂移、创建新授权对象或触发 Deployment/Portal 发布；检测到运行期 Snapshot 超出本地 Bootstrap 状态时拒绝覆盖。该适配器不能替代上述生产签名发布/订阅控制器。
