...
`approval` 声明 `none/different-subject/two-person/policy` 语义。`policy` 表示领域服务消费组合根注入的 `approval.policy.v1`，由协议决定异人审批或单人复验；它只是治理元数据，不是授权结论。最终主体、冻结摘要、职责分离和审批状态始终由领域服务以真实对象强制执行。Portal 首个接入见 [ADR-0188](../decisions/ADR-0188-授权与业务审批策略解耦.md)。
