...
Gateway 固定执行：规范化 Host 与路径 → 解析当前 Catalog generation → 建立可信身份 → 校验 tenant/Portal/认证 Profile → 权限与速率预检 → 请求大小与 JSON Schema 校验 → 生成受限 `GatewayInvocation` → 调用内部 capability → Backend 最终 PEP 再校验 → 映射允许公开的错误 → 响应 Schema 校验。
