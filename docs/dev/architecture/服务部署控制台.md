...
> 状态：节点引导与发布 v1、Application Intent 与 Composition Planner P1—P4 已实施；真实插件 E2E 待实施｜最后更新：2026-07-25
>
...
7. **P1—P4 协议、Planner、后端工作流与 Portal 已实现**：Portal 只提交封闭 Application Intent，表单仅包含部署目标、根插件、Feature、非敏感配置及副本/扩缩容/资源/节点选择。依赖、运行策略、Provider Binding、配置缺口、Artifact Lock 和诊断均来自只读 Resolution Report；提交、审批和发布前复验 stale，可信 Configuration Snapshot 只携带 CredentialRef。旧 Composition 创建/更新在线入口已删除，历史 revision 仅可查看。
