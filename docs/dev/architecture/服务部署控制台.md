...
> 状态：节点引导与发布 v1、Application Intent 与 Composition Planner P1—P2 已实施；在线工作流接入待实施｜最后更新：2026-07-25
>
...
7. **P1—P2 协议与 Planner 已实现、在线入口仍待切换**：Backend 已有封闭 Application Intent、Resolution Report、Configuration Plan、Feature、TypeScript SDK 和独立 Go Planner；仓库提供精确 Manifest 规划投影，可信 Configuration Snapshot 只携带 CredentialRef。Deployment Manager 现有页面仍允许编辑 `depends_on`、实例策略、状态模型和路由；P3—P4 必须接入 Intent/Planner 工作流，并把依赖、运行策略及 Provider Binding 改为只读结果。
