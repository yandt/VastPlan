...
> 状态：节点引导与发布 v1、Application Intent 与 Composition Planner P1—P3 已实施；Portal 切换待实施｜最后更新：2026-07-25
>
...
7. **P1—P3 协议、Planner 与后端工作流已实现，Portal 待切换**：Backend 已有封闭 Application Intent、Resolution Report、Configuration Plan、Feature、TypeScript SDK 和独立 Go Planner；Deployment Manager 已持久化计划快照，并在提交、审批、发布前复验 stale。仓库提供精确 Manifest 规划投影，可信 Configuration Snapshot 只携带 CredentialRef。现有 Portal 页面仍允许编辑 `depends_on`、实例策略、状态模型和路由；P4 必须切换 Intent 表单，并把依赖、运行策略及 Provider Binding 改为只读结果。
