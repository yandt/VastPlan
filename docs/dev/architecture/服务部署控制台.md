...
> 状态：节点定义、引导审批与可信执行桥已实施，服务发布与 UI 待接入｜最后更新：2026-07-18
>
...
4. **部分实现**：Node Agent 可用 `-credential-root` 注入企业 secret mount 目录 Broker；集中式 Vault/KMS provider 与 Node Lease 观察器把 `SystemdActive` 推进到 `Ready` 仍待实现。
5. **待实现**：服务 Draft 解析为 Backend Platform Profile 与 Application Composition，经现有 Resolver 产生 Deployment v2，并以单调 revision/CAS 发布。
6. **待实现**：UI 提供节点、服务、插件、replicas、放置约束、依赖图、预检差异、审批、发布、滚动进度和回滚入口。
