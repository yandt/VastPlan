...
> 状态：节点引导与发布 v1 已实施；Application Intent 协议 P1 已实施、Composition Planner 待实施｜最后更新：2026-07-25
>
...
7. **P1 协议已实现、UI 仍待切换**：Backend 已有封闭 Application Intent、Resolution Report、Configuration Plan、Feature 和 TypeScript SDK；Deployment Manager 现有页面仍允许编辑 `depends_on`、实例策略、状态模型和路由。P2—P4 必须接入 Planner 和 Intent 工作流，并把依赖、运行策略及 Provider Binding 改为只读结果。
