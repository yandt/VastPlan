...
| `designSystems` | `ui.design-system` | Portal 唯一设计系统：主题与框架无关语义 UI 组件实现 |
| `shellCompositions` | `ui.shell-composition` | Portal 唯一组合层：页面、导航语义区、标准 Slot、排序与冲突规则 |
| `shellLayouts` | `ui.shell-layout` | Portal 唯一视觉布局：LOGO、菜单方向、页头、正文和侧栏样式 |
...
- [x] Frontend 入口使用签名包内单文件 `.js/.mjs` ESM bundle，Edge 生成带模块字节摘要的 RuntimeSpec（ADR-0062）
...
- [x] Frontend 使用签名 bundle、`ui.design-system` 与 Portal Runtime 组件装配；不复用 Backend gRPC 进程模型（ADR-0052、《[前端门户内核](前端门户内核.md)》）
