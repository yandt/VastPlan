...
- [x] Frontend v1 已使用签名包内单文件 ESM bundle；v6 Module Graph Schema、Go 强类型、入口绑定、依赖闭包、受控 externals、大小/重复映射和规范化 digest 校验已落地；构建器生成图，打包器逐节点复核并注入签名 Manifest，`artifacttrust` 在仓库接收/读取时再次绑定 tar 实际字节；Catalog/RuntimeSpec/Loader 迁移继续实施
