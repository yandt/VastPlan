...
`ui.design-system` 由 `foundation` 层的已签名第一方前端插件提供。普通前端插件只依赖 `@vastplan/portal-ui`，不得在 descriptor 中要求具体 UI 框架；框架选择属于 Portal Platform Profile，普通应用组合不能直接选择或替换。完整的加载、切换与故障恢复语义见《[前端门户内核](前端门户内核.md)》。
