...
运行时 descriptor、处理器注册集合和可信 Host 的 operation 白名单应由该归一化契约机械投影。需要不同权限的状态动作必须拆成不同 operation，例如 `submitProfile/approveProfile/publishProfile`，不得让 Enforcer 根据业务 payload 猜测。用户授权只在 Kernel Enforcer 判定一次；业务插件保留主体、租户、对象状态、CAS 和职责分离校验，不再重复检查 Token 角色名。完整决策见 [ADR-0170](../decisions/ADR-0170-统一Capability-Contract与可信ArtifactIdentity.md)。
