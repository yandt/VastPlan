...
Feature 是插件开发者随签名 Manifest 发布的可选组合开关。每项使用稳定 `id/title`，并至少增加一项包 `dependencies`、`runtimeRequires` 或 `configurationSchema` 条件。Feature 不能包含脚本、`when`、任意 JSON Pointer 或配置值表达式；配置条件只能约束顶层 `configuration.schema` 已声明的字段，必须保持 `type=object + additionalProperties=false`，远端 `$ref`、新增字段、自依赖和重复 Feature ID 均在仓库摄取前拒绝。应用用户只能启用存在于精确制品 Manifest 的 Feature；Planner 展开后仍由 Controller 从完整 Manifest 二次复验。
