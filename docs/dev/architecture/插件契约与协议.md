...
> 本文是**插件如何被定义、如何通信、携带什么数据**的单一真相源，整合了统一插件定义（清单）、插件-宿主协议、不可变契约字段。系统整体架构见《[系统架构](系统架构.md)》；决策依据见《[ADR](../decisions/README.md)》。字段/消息为语义定义，栈无关（[0005](../decisions/ADR-0005-骨架与技术栈解耦.md)）；候选传输 gRPC/Protobuf（[0008](../decisions/ADR-0008-骨架技术选型对比.md)/[0009](../decisions/ADR-0009-内核技术栈选型.md)）。
...
```mermaid
flowchart TB
  M["统一插件定义（一份清单）<br/>contributes: webui / backend / rc / mobile"]
  M -->|取 webui 贡献| WK["前端内核 WebUI Kernel"]
  M -->|取 backend 贡献（按 service_role 分派到 backend/workspace/rs）| BK["后端内核 Backend Kernel"]
  M -->|取 rc 贡献| RK["Runner 内核（桌面执行器，原 APP/RC）"]
  M -->|取 mobile 贡献| MK["移动内核 Companion（手机）"]
```
...
| `version` | ✓ | 语义化版本 |
| `publisher` | ✓ | 发布者标识 |
| `engines` | ✓ | 各内核兼容版本范围，如 `{ backend: "^1.0", webui: "^1.0", rc: "^1.0" }` |
| `capabilities` | | 装配元数据（内核能力/资源/凭证句柄），见 §1.5 |
| `activation` | ✓ | 惰性激活事件，见 §1.6 |
| `dependencies` | | 依赖的其他插件 `{id: versionRange}` |
| `entry` | ✓ | 各面运行时入口 `{ backend, webui, rc }` |
| `contributes` | ✓ | 三面贡献集合，见 §1.3 |

### 1.3 三面贡献 `contributes`

四个可选键 `webui / backend / rc / mobile`，任意组合（对应四内核，0014）。每条贡献有稳定逻辑 `id`（对应契约 `CallTarget.capability`）。
...
**客户端 `contributes.rc`**（Runner 内核面；`rc` 保留为客户端/runner 命名空间，见 [ADR-0011](../decisions/ADR-0011-组合是通用内核能力.md)。编译型客户端插件预编译进二进制、进程内运行，`scripts/workflows` 为运行时下载的内容，见 [ADR-0012](../decisions/ADR-0012-APP内核运行模型.md)）

| 贡献点 | 扩展点 | 说明 |
|---|---|---|
| `scripts` | `rc.script` | 可下发执行的脚本 |
| `workflows` | `rc.workflow` | 多步骤工作流 |
| `runtimeRequirements` | — | 客户端需具备的运行时（如 `python>=3.10`） |

**移动 `contributes.mobile`**（移动内核 Companion，手机；无后台常驻/本地执行，见 [ADR-0014](../decisions/ADR-0014-四内核结构.md)/[0013](../decisions/ADR-0013-APP多档能力与手机Companion.md)）
...
插件默认**惰性**，仅在声明事件发生时被唤醒；三面可独立激活。事件如 `onStartup` / `onView:<id>` / `onCommand:<id>` / `onAgentTool:<package>` / `onRcScript:<id>`。
...
| `scene` | string | 场景名（三元组之 scene），如 `agent.tool_call` / `webui.api` / `rc.exec` |
...
**Caller** `{ kind, id }`，`kind` ∈ `{ user, agent, plugin, system, rc }`（CallerKind 登记）。**三元组 `(caller, scene, target)`** 是权限/计量/统计的统一判定依据（校验器/事件汇为扩展点）。
...
> 由认证边界（WebUI 边缘入口 / RC 接入 / 服务内网信任）构造后注入 CallContext，此后只读透传。
...
- [ ] 各面运行时入口打包格式（backend 可执行/模块、webui bundle、rc 目录）
- [ ] 扩展点(Registry)完整清单与各贡献 descriptor 契约（分面专篇）
