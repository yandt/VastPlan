...
| 字段 | 必填 | 说明 |
|---|---|---|
| `id` | ✓ | 全局唯一，反向域名式；新首方生产插件使用 `com.vastplan.<layer>.<category...>.<component>`（ADR-0050） |
...
首方 `layer` 为 `foundation/platform/product/integration/example`；`category` 至少一级，可用 `data.relational` 这样的多级路径表示功能子域；最后一段是具体组件。`com.vastplan.*` 与 `publisher=vastplan` 强制绑定；命名空间可用于分类，但不能替代签名和授权。历史 demo ID 只作兼容，不再作为新插件命名样例。
...
| `designSystems` | `ui.design-system` | Portal 唯一设计系统：主题、布局与框架无关 UI 契约的实现 |
...
**范围是内核内**：一套内核宿主与它在本节点管辖的插件实例。Backend 默认使用独立进程（[0004](../decisions/ADR-0004-插件运行形态.md)），受控 dynamic-go 内嵌实例复用相同的调用、授权与生命周期语义（[0051](../decisions/ADR-0051-Backend混合插件运行与受控内嵌边界.md)）。跨服务/跨机器不归本协议（走《[系统架构 · 第二章](系统架构.md)》的 mesh）。承载物是第三章的不可变契约。
...
dynamic-go 内嵌实例没有 gRPC 握手与心跳消息。从 Manifest `execution.backend.dynamicGo` 指向的已签名 `.so` 读取 `vastplan.dynamic-go.v1` 模块，并在 `plugin.Open` 前验证签名清单中的共同构建指纹，再复核首方命名空间、Go 工具链/平台/关键参数和共享依赖。它把代码贡献与验签 LaunchPolicy 逐项核对，并与进程实例共享 Registry、`Host.Invoke`、宿主回调身份重签、资源限制、Drain 和迁移。放置策略仍是节点运维策略；`dynamicGo.required` 可禁止进程回退但不能自行请求进入内核进程。
...
- **心跳/崩溃**：进程实例双向心跳，失联即从所有 Registry 摘除贡献；内嵌实例的普通 handler `panic` 转为稳定错误并摘除贡献。内嵌实例无法隔离 OOM、`fatal`、死循环或内存破坏，因此只允许首方插件并由部署策略显式启用；dynamic-go 不能卸载或进程内热升级（[0051](../decisions/ADR-0051-Backend混合插件运行与受控内嵌边界.md)）。
...
| `ui.design-system` | single | `uiContract`（SemVer 范围）、`framework`、`capabilities`（layout/menu/overlay/form/data/feedback/theme）；每个 Portal 只能选择一个 |

`ui.design-system` 由 `foundation` 层的已签名第一方前端插件提供。普通前端插件只依赖 `@vastplan/portal-ui`，不得在 descriptor 中要求具体 UI 框架；框架选择属于 Portal 组合而非功能插件。完整的加载、切换与故障恢复语义见《[前端门户内核](前端门户内核.md)》。
...
- [x] Go/Python SDK 共享同一 proto 真源并由真实跨语言 E2E 守护
