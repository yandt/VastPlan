...
Node/Python Runtime Host 属于内核发布物和信任计算基，默认采用共享宿主。Pool key 包含内核服务、Provider、隔离等级、发布者信任域、平台与执行要求摘要；不同硬边界不得合池。每个 Worker/子解释器必须拥有独立 session、死亡信号、允许环境和停止能力；一个执行单元退出不能伪造或关闭其他插件的会话。热替换始终创建候选执行单元并原子切换，不依赖 ESM/importlib 缓存失效。详见 [ADR-0089](../decisions/ADR-0089-Runtime-Provider与共享Host池.md)。
