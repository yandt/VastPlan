...
| `renderAdapters` | `ui.render.adapter` | Portal 唯一设计系统：主题与框架无关语义 UI 组件实现 |
...
`dynamic-go` 从 ADR-0088 起只保留为首方历史承载，不再作为新插件的推荐热装方案；它不能卸载，升级仍须滚动重启 Backend。
...
首版 Node Runtime Host 采用“每个插件一个受管 Worker”：Host 进程是内核发布物，Worker 才是可替换执行单元。Node SDK 从 Runtime Host 注入的契约目录加载同一份 Protobuf，插件不能自带修改版协议。该结构允许后续把多个 Worker 池化到一个 Host，但池化不得改变 `PluginInstance`、一次性启动票据和逐插件生命周期语义。

首版 Python Runtime Host 同样采用每插件 Host 进程，但把协议面和业务面分开：主解释器持有 gRPC 会话，子解释器只加载纯 Python 插件桥接与业务依赖。这样原生 gRPC 扩展不进入子解释器；插件自身的原生依赖仍必须满足 `subinterpreterSafe` 承诺。桥接只宣告实际支持的协议 feature，当前静态贡献与 Invoke 之外的能力不得伪装可用。
...
dynamic-go 内嵌实例没有 gRPC 握手与心跳消息。从 Manifest `execution.backend.dynamicGo` 指向的已签名 `.so` 读取 `vastplan.dynamic-go.v1` 模块，并在 `plugin.Open` 前验证签名清单中的共同构建指纹，再复核首方命名空间、Go 工具链/平台/关键参数和共享依赖。它把代码贡献与验签 LaunchPolicy 逐项核对，并与其他 `PluginInstance` 共享 Registry、`Host.Invoke`、宿主回调身份重签、资源限制、Drain 和迁移。放置策略仍是节点运维策略；`dynamicGo.required` 可禁止进程回退但不能自行请求进入内核进程。

Node/Python Runtime Host 属于内核发布物和信任计算基。它可以采用每插件宿主或共享宿主，但每个 Worker/子解释器必须拥有独立 session、死亡信号和停止能力；一个执行单元退出不能伪造其他插件的会话身份。热替换始终创建候选执行单元并原子切换，不依赖 ESM/importlib 缓存失效。
...
- **心跳/崩溃**：进程和 Runtime Host 执行单元双向心跳，失联即从所有 Registry 摘除贡献；内嵌实例的普通 handler `panic` 转为稳定错误并摘除贡献。Worker/子解释器可被 Runtime Host 整体释放，但原生 Addon/扩展、全局 OOM 或 Runtime Host 崩溃仍会影响同宿主实例，因此只属于 `trusted-runtime`。dynamic-go 不能卸载或进程内热升级（[0051](../decisions/ADR-0051-Backend混合插件运行与受控内嵌边界.md)）。
...
| `ui.render.adapter` | single | `uiContract`（SemVer 范围）、`framework`、`capabilities`（layout/menu/overlay/form/data/feedback/theme）；每个 Portal 只能选择一个 |

`ui.render.adapter` 由 `foundation` 层的已签名第一方前端插件提供。普通前端插件只依赖 `@vastplan/ui-primitives`，不得在 descriptor 中要求具体 UI 框架；框架选择属于 Portal Platform Profile，普通应用组合不能直接选择或替换。完整的加载、切换与故障恢复语义见《[前端门户内核](前端门户内核.md)》。
...
- [x] Frontend 使用签名 bundle、`ui.render.adapter` 与 Portal Runtime 组件装配；不复用 Backend gRPC 进程模型（ADR-0052、《[前端门户内核](前端门户内核.md)》）
