...
**范围是内核内**：一套内核宿主与它在本节点管辖的插件实例。Backend 默认使用独立进程（[0004](../decisions/ADR-0004-插件运行形态.md)），受控静态/dynamic-go 内嵌实例复用相同的调用、授权与生命周期语义（[0051](../decisions/ADR-0051-Backend混合插件运行与受控内嵌边界.md)）。跨服务/跨机器不归本协议（走《[系统架构 · 第二章](系统架构.md)》的 mesh）。承载物是第三章的不可变契约。
...
内嵌实例没有 gRPC 握手与心跳消息。静态实例以 `ID + version` 精确命中编译时目录；dynamic-go 从 Manifest `execution.backend.dynamicGo` 指向的已签名 `.so` 读取 `vastplan.dynamic-go.v1` 模块，并在 `plugin.Open` 前验证签名清单中的共同构建指纹，再复核首方命名空间、Go 工具链/平台/关键参数和共享依赖。二者都会把代码贡献与验签 LaunchPolicy 逐项核对，并与进程实例共享 Registry、`Host.Invoke`、宿主回调身份重签、资源限制、Drain 和迁移。放置策略仍是节点运维策略；Manifest 只声明制品内容，不能自行请求进入内核进程。
