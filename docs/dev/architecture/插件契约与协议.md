...
- [x] backend 已实现扩展点的运行态 descriptor Schema（`tool.package` / `permission.checker` / `event.sink` / `hook`）；其余扩展点随实现补齐
...
**清单/贡献点**
- [x] 清单 / 已实现 backend descriptor / 制品元数据的 JSON Schema 与 Go 校验工具链（v1；其他扩展点随实现补齐）
