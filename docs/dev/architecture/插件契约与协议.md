...
清单是插件的**唯一声明入口**：四套内核只读清单就能知道"这个插件是谁、需要哪些内核能力/资源、在前端/后端/Desktop/移动各面各贡献什么、何时激活、依赖谁"。声明与实现分离；一份清单贯通各面（[0006](../decisions/ADR-0006-内核分区与后端组合.md)/[0014](../decisions/ADR-0014-四内核结构.md)）。

```mermaid
flowchart TB
  M["统一插件定义（一份清单）<br/>contributes: backend / frontend / desktop / mobile"]
  M -->|取 frontend 贡献| WK["Frontend 内核（浏览器）"]
  M -->|取 backend 贡献（按 service_role 分派到 backend/workspace/rs）| BK["Backend 内核"]
  M -->|取 desktop 贡献| RK["Desktop 内核（桌面执行器，原 APP/RC）"]
...
4. **capabilities 是申请而非授权**：声明需要什么内核能力/资源，供装配与排依赖；其中 `kernelServices` 还会进入可信宿主 Grant 编译，任何发布者都不能只凭清单取得内核服务。
...
7. **权限由能力所有者声明，角色独立治理**：Manifest 可以声明 operation 所需权限，但不能给主体授权或创建角色；在线角色只消费可信 Permission Catalog（ADR-0107）。
...
| `version` | ✓ | 语义化版本 |
| `publisher` | ✓ | 发布者标识 |
| `engines` | ✓ | 各内核兼容版本范围，如 `{ backend: "^1.0", frontend: "^1.0", desktop: "^1.0", mobile: "^1.0" }` |
| `capabilities` | | 运行需求申请（内核能力/资源/凭证句柄）；`kernelServices` 同时进入运行时 Grant 编译，见 §1.6 |
| `contextAccess` | | Backend 对 CallContext 语义字段的必需/可选申请与 baggage 前缀白名单；不是自行授权，见 §1.5.3 |
| `authorization` | | 插件拥有的权限 namespace、可选 capability 所有权、权限定义及精确 operation guard；角色和主体绑定不在 Manifest，见 §1.5.5 |
| `composition` | | 签名、可选的组合 Feature；只允许增加预声明依赖和配置条件，见 §1.8.1 |
| `configuration` | | 非敏感 Schema、作用域、生效方式、托管凭证字段及可选语言中立热配置控制器 |
...
| `extensionPoints` | | 本插件拥有的版本化扩展点；包含 surface、contract、kind、dispatch、目标与闭合 descriptor Schema |
| `extensions` | | 对其他插件扩展点的 JSON-only 贡献；必须同时在 `dependencies` 中依赖所有者 |
| `dependencies` | | 依赖的其他插件 `{id: versionRange}` |
| `entry` | ✓ | 各面运行时入口 `{ backend, frontend, frontendServer, desktop, mobile }`；`frontendServer` 只用于已签名服务端模块图 |
| `frontendModuleGraphs` | | Frontend v6 可信模块图：必选 browser、可选 server；逐节点锁定 path/SHA-256/size/mediaType/purpose/dependencies，并以规范化 digest 绑定 |
...
内核公共 `contributes` 表达“向系统骨架提供能力”；`extensionPoints/extensions` 表达“一个插件公开稳定扩展面，另一个插件按契约扩展它”。两者不能互相冒充。

- 扩展点 ID 必须位于所有者插件命名空间，贡献 ID 必须位于贡献者命名空间；插件不能替其他插件声明扩展点或贡献身份。
- 扩展点使用精确 SemVer，贡献使用 SemVer 范围；贡献者还必须用插件 `dependencies` 锁定所有者兼容范围。
- descriptor 只能是符合所有者闭合 JSON Schema 的数据，不能传递组件、回调、进程内对象、凭证或任意代码。
- 可信 Catalog 在插件代码运行前解析整张扩展图；所有者缺失、版本不兼容、Schema 不匹配、single 多贡献或治理目标冲突都会拒绝候选。
- 各内核只接收本 surface 的已验证投影。Frontend 将扩展图与模块图放入同一 RuntimeSpec/Generation；Backend 后续把已验证绑定投影到现有 Registry 和 Protocol Bus，不另建调用旁路。

详见 [ADR-0178](../decisions/ADR-0178-插件专属扩展点与可信扩展图.md)。

### 1.3 四面贡献 `contributes`

四个可选键 `backend / frontend / desktop / mobile`，任意组合（对应四内核，规范 ID 见 [0015](../decisions/ADR-0015-内核与贡献面命名规范.md)）。每条贡献有稳定逻辑 `id`（对应契约 `CallTarget.capability`）。

**后端 `contributes.backend`**（每条贡献必填 `service_role`：backend/workspace/rs；`desktopCapabilities` 固定为 rs）

| 贡献点 | 扩展点(Registry) | 说明 |
|---|---|---|
| `tools` | `tool.package` | Agent 工具包（package + 子命令） |
| `agents` | `agent` | 预置 Agent 定义 |
| `apiContracts` | 控制面元数据 | 由平台治理后暴露的 HTTP/JSON 契约，公开地址不含插件 ID |
| `dataPlaneServices` | 控制面元数据 | 独立 HTTPS 数据面声明，运行端点由短时 Endpoint Lease 提供 |
| `apiRoutes` | `api.route`（deprecated） | 仅保留 Backend v1 清单与兼容矩阵，不供新插件或 Node Gateway 使用 |
| `permissionCheckers` | `permission.checker` | 基于 `(caller,scene,target)` 的权限校验器 |
| `eventSinks` | `event.sink` | 事件汇（审计/可观测消费 CallEvent） |
| `hooks` | `hook` | 关键节点前后钩子 |
| `desktopCapabilities` | `desktop.capability` | RS 侧调度能力/执行模式（service_role=rs） |
| `configuration.controller`（顶层配置契约派生） | `configuration.controller` | `service + hot` 的 prepare/commit/abort/status；capability 由签名插件 ID 派生，不由发布者自报 |
| `configuration.resource-controller`（资源集合契约派生） | `configuration.resource-controller` | 独立配置资源的 list/get/prepare/commit/abort/status；capability 与 `cfgc_*` 集合 ID 均由签名身份派生 |
| `configurationScopedResolvers` | `configuration.scoped-resolver` | tenant/user Hot 的 resolve/watchRevision；请求不携带 tenant、subject、插件或配置身份，统一 capability 为 `configuration.scoped` |
...
**Desktop `contributes.desktop`**（Desktop 内核面，规范 ID 统一 `rc`→`desktop`，见 [ADR-0015](../decisions/ADR-0015-内核与贡献面命名规范.md)。编译型客户端插件预编译进二进制、进程内运行，`scripts/workflows` 为运行时下载的内容，见 [ADR-0012](../decisions/ADR-0012-APP内核运行模型.md)）

| 贡献点 | 扩展点 | 说明 |
|---|---|---|
| `scripts` | `desktop.script` | 可下发执行的脚本 |
| `workflows` | `desktop.workflow` | 多步骤工作流 |
| `interactions` | `desktop.interaction` | Desktop 可发起的确认、表单与审批交互，以及允许投递的体验面 |
...
`stateModel` 必须与实例策略匹配：`local-ephemeral`、`external-shared`、`leader-owned`、`partition-owned`。其中 `external-shared` 表示服务连续性不绑定当前副本独占的本地耐久真相，不等价于插件直接申请 Shared State；有状态操作也可以通过强远程依赖委托给其他真相拥有者。`visibility` 可取 `local/service/cluster/global`；`routing` 可取 `direct/queue/leader/shard`。
...
`backgroundService=true` 表示插件在没有入站 `Invoke` 时也需要自主 `HostCall`。它不是免鉴权标记：首版只允许 `leader + leader-owned + leader`，必须具有 `service + restart` 配置，并在配置 Schema 中要求字符串 `tenantId`。安装器冻结该声明，Node Agent 从已验证的插件隔离配置绑定唯一租户；宿主只在 ACTIVATE 成功后开放，DRAIN/SHUTDOWN/断连前关闭。自主调用只取得宿主构造的 tenant、plugin caller 与 `system.background` scene，不继承插件自报身份、凭证或 metadata。完整边界见 [ADR-0140](../decisions/ADR-0140-后台服务自主HostCall与宿主绑定上下文.md)。

跨 Deployment 的 `remote + strong/data` 必须同时声明 `logicalService` 与 `routingDomain`。发布器只为同一 Deployment 中可见的 provider 建立 unit DAG；外部目标由 Node Agent 使用全局 capability readiness 和版本租约在激活前复核。缺少完整路由身份时仍在发布期 fail-closed，不能用 capability-only 依赖碰运气。
...
Python 插件依赖按 [ADR-0137](../decisions/ADR-0137-Python标准依赖锁与离线Overlay.md) 使用 Manifest 绑定的 PEP 751 标准锁和包内 wheel。Node Agent 在内容寻址安装目录内离线生成插件专属 overlay；独立进程通过 `PYTHONPATH` 挂载，共享 Host 只把该路径加入对应子解释器，并复核路径未越出插件根。锁定与依赖可见性不改变进程池边界，也不把共享解释器提升为第三方沙箱。
...
需要接受用户/服务主体授权的 Backend capability，在签名 Manifest 中声明 `authorization.namespace`、`permissions[]` 与 `operationGuards[]`。每个权限固定 code、scope、risk、assignable 和 offlineAllowed；每个 guard 精确绑定本插件已经声明的 `tool.package/capability/operation`，并给出必需权限、read/write/approve/execute 类型和职责分离提示。

Backend Tool 可在 subcommand 上声明 `audience=user|workload|system`，并由宿主归一化为 `ToolCapabilityContract`。一旦同一 Tool 有一个 operation 声明 audience，该 Tool 全部 operation 都必须分类：`user` 必须绑定唯一的 operation guard，`workload/system` 不得挂用户权限。权限 namespace 与逻辑 capability 不同的首方插件，使用 `authorization.capabilities[]` 显式绑定；这只是所有权声明，不授予调用者权限。

运行时 descriptor、处理器注册集合和可信 Host 的 operation 白名单应由该归一化契约机械投影。需要不同权限的状态动作必须拆成不同 operation，例如 `submitPortalPublication/approvePortalPublication/publishPortalPublication`，不得让 Enforcer 根据业务 payload 猜测。用户授权只在 Kernel Enforcer 判定一次；业务插件保留主体、租户、对象状态、CAS 和职责分离校验，不再重复检查 Token 角色名。完整决策见 [ADR-0170](../decisions/ADR-0170-统一Capability-Contract与可信ArtifactIdentity.md)。

Schema 与 Go 语义校验共同拒绝外部发布者占用 `platform.*`、权限越出 namespace、未知或重复 operation、未声明/未使用权限，以及平台级或高风险权限允许离线。Permission Catalog 只能从 Artifact Trust 已验证的 Manifest 和制品 SHA-256 构建，跨插件 code/operation 所有权冲突继续 fail-closed。

Manifest 不包含角色、人员、组织、授权结果或管理员标志。安装插件不自动授权；在线 Role revision、Subject Binding、Policy Domain、Provider Protocol 和每内核 Enforcer 见《[在线角色与权限治理](在线角色与权限治理.md)》。

跨语言权限公共契约位于 `contracts/schemas/authorization/v1`。Authorization IR、Policy Snapshot、Provider Profile 以及 `authorization.store/engine/directory/exchange.v1` 均由拆分 JSON Schema 与同义 Go DTO 共同冻结；未知字段、未登记 operation、Domain 委托扩大和 Provider 越权语义必须在进入具体插件前拒绝。具体 Policy/Enforcer 插件只能消费这些契约，不能复制同义 wire 类型。

### 1.6 装配元数据 `capabilities`

声明插件**需要什么**，供装配注入与排依赖。声明本身不授予权限；`kernelServices` 必须继续通过服务级精确授权、发布者上限和宿主注册校验。

| 键 | 语义 |
|---|---|
| `kernelServices` | 申请的内核服务（如 `kernel.info` / `kernel.config.get` / `kernel.state.shared.get`）；当前均为必需项，未获得完整 Grant 时拒绝激活 |
| `credentials` | 需申领的凭证名（值由宿主按**句柄**注入，明文不过插件——见 §3.9） |
| `resources` | 资源约束（GPU、特定运行时），供节点选择/放置 |

### 1.7 激活事件 `activation`

插件默认**惰性**，仅在声明事件发生时被唤醒；各面可独立激活。事件如 `onStartup` / `onView:<id>` / `onCommand:<id>` / `onAgentTool:<package>` / `onDesktopScript:<id>`。

### 1.8 依赖 `dependencies`

`{ "com.acme.crm-core": "^2.0.0" }`。生命周期管理器解析依赖树、定序激活、检测环状依赖。

#### 1.8.1 组合 Feature `composition.features`

Feature 是插件开发者随签名 Manifest 发布的可选组合开关。每项使用稳定 `id/title`，并至少增加一项包 `dependencies`、`runtimeRequires` 或 `configurationSchema` 条件。Feature 不能包含脚本、`when`、任意 JSON Pointer 或配置值表达式；配置条件只能约束顶层 `configuration.schema` 已声明的字段，必须保持 `type=object + additionalProperties=false`，远端 `$ref`、新增字段、自依赖和重复 Feature ID 均在仓库摄取前拒绝。应用用户只能启用候选制品 Manifest 中存在的 Feature；Feature 依赖随候选参与仓库回溯，Planner 展开后仍由 Controller 从精确完整 Manifest 二次复验。应用层共享 `ArtifactRequirement` 使用 `pluginId/constraint/channel/features`，运行层继续使用精确 `ArtifactRef`，详见 [ADR-0158](../decisions/ADR-0158-应用插件兼容范围与精确运行锁.md)。
...
```jsonc
{
  ...
  "engines": { "backend": "^1.0", "frontend": "^1.0", "desktop": "^1.0" },
  "runtime": {
    "instancePolicy": "active-active",
    "stateModel": "external-shared",
    "visibility": "cluster",
  ...
  "capabilities": {
    "kernelServices": ["kernel.config.get"],
  ...
  "entry": { "backend": "dist/backend/main", "frontend": "dist/frontend.js", "desktop": "desktop/" },
  "contributes": {
    "backend": {
      "tools": [
    ...
    "desktop": {
      "scripts": [ { "id": "acme.collectLogs", "runtime": "python", "entry": "desktop/collect_logs.py", "paramsSchema": { } } ],
...
- **插件回调宿主**（`HostCall`）：用同样的 `target + context` 回调宿主；本地命中即内核服务，否则转交寻址层到别的服务。普通调用必须携带宿主为当前 Invoke 签发的短期 delegation；显式 `backgroundService` 只能取得 ADR-0140 的宿主绑定最小上下文。凭证类回调由宿主注入、明文不过插件。
...
Host、第一方 SDK 与 addressing 共用 `contracts/runtime/go/protocollimit.Limits`，并在每一跳执行同一份限制。默认一元 payload 4 MiB、流帧 1 MiB、`CallContext`/传输 metadata 16 KiB、并发 256、pending 512、能力调用深度 16、调用 deadline 30 秒、drain 30 秒；零值表示采用安全默认而不是禁用限制。完整决策与拒绝语义见 [ADR-0034](../decisions/ADR-0034-Backend协议资源边界.md)。
...
| `scene` | string | 场景名（三元组之 scene），如 `agent.tool_call` / `frontend.api` / `desktop.exec` |
...
Backend Kernel 1.0 的稳定错误码由 `contracts/runtime/go/errorcode` 单一登记：
...
**Caller** `{ kind, id }`，`kind` ∈ `{ user, agent, plugin, system, desktop }`（CallerKind 登记）。wire 值中 Desktop 使用 `CALLER_KIND_DESKTOP = 6`；读取端在迁移期兼容弃用的 `CALLER_KIND_RUNNER = 5`，但新写端不得再发送 5，详见 [ADR-0201](../decisions/ADR-0201-Desktop-CallerKind加法式演进.md)。**三元组 `(caller, scene, target)`** 是权限/计量/统计的统一判定依据（校验器/事件汇为扩展点）。
...
> 由认证边界（Frontend 边缘入口 / Desktop 接入 / 服务内网信任）构造并校验后注入可信上下文。`CallContext.tenant_id` 是规范租户值；兼容保留的 `Principal.tenant_id` 必须与其一致。对下游只发放受众投影快照，不把原始可变指针交给插件。
...
| 语义 | 含义 | 典型扩展点 |
|---|---|---|
| **single**（单一提供者） | 一个 `id` 由唯一贡献提供，调用直接路由 | `tool.package` / `agent` / `editor.provider` |
...
| `api.route`（deprecated） | single | 仅兼容旧 Backend v1 descriptor；不得作为新产品 API 入口 |
...
| `desktop.capability` | single | `service_role: rs`、`kind`（执行模式/调度能力）、`params?` |
| `configuration.controller` | single | `protocol: configuration.v1`；仅由 `configuration.controller` 顶层声明合成，响应只含摘要和生命周期事实 |
| `configuration.resource-controller` | single | `protocol: configuration.resource.v1`；仅由 `resourceController/resourceCollections` 合成，查询只含非敏感值与凭证状态，控制响应只含摘要事实 |
| `configuration.scoped-resolver` | single | `protocol: configuration.scoped.v1`；resolve 返回签名 Schema 下的非敏感值，watchRevision 只返回 revision/digest 事实 |
...
> `capabilities.kernelServices` 只声明申请。Node Agent 以 `Manifest 申请 ∩ ServiceUnit 精确 Grant ∩ 发布者上限` 编译不可变 Grant Plan，并确认候选宿主已经注册每项服务；任一环节缺失都在插件启动前 fail-closed。插件经 HostCall 调用，注册同名 contribution 同样会 fail-closed。
...
`ui.runtime.engine` 与 `ui.render.adapter` 由 `foundation` 层的已签名第一方前端插件提供，Shell 与 Workbench 也必须声明相同 `engineFamily`。普通前端插件只依赖 `@vastplan/workbench-sdk`，不得要求 React、Vue 或具体 UI 框架；框架选择属于 Portal Platform Profile，普通应用组合不能直接选择或替换。当前只发布 React Engine 与 Ant Design Renderer，其他实现必须有真实插件和契约测试后才能进入 Catalog。完整语义见《[前端门户内核](前端门户内核.md)》。

### 4.5 Desktop 面扩展点

| 扩展点 | 分发 | descriptor 特有字段 |
|---|---|---|
| `desktop.script` | single | `runtime`（如 `python>=3.10`）、`entry`、`paramsSchema`、`resultSchema?` |
| `desktop.workflow` | single | `definition`（步骤 DAG 引用）、`paramsSchema` |
| `desktop.interaction` | mount | `interactionContract`、`kinds`（confirm/form/approval/notification/progress）、`allowedSurfaces`（frontend/mobile/desktop.local） |

> Desktop 面贡献为**内容型**（脚本/工作流定义），运行时下载执行（ADR-0012）；编译型 Desktop 插件的能力经 `backend` 面 `desktop.capability`（service_role=rs）或预编译进 Desktop 二进制暴露。
...
> Mobile 面**无执行类扩展点**（无后台常驻/本地执行，ADR-0013/0014）；只呈现、审批、触发，重活经内核间通信落到后端/Desktop。`mobile.ui-adapter` 仅适配语义 UI 契约，不让插件绑定具体原生框架；跨端交互的完整边界见《[跨端体验与交互契约](跨端体验与交互契约.md)》。
...
- [x] backend 8 个公开插件扩展点均有封闭的运行态 descriptor Schema；未知点与内部 `kernel.service` fail-closed（ADR-0031、ADR-0117）
...
- [x] Frontend v1 的单文件 ESM 仅保留为迁移输入；v6 Module Graph 已贯通 Schema、Go 强类型、构建器、打包器、`artifacttrust`、Catalog、内容寻址交付、RuntimeSpec 与 Browser Loader。入口、完整依赖闭包、DAG/64 层深度、64 MiB 总量、受控 externals、逐节点摘要、媒体类型和 Generation 资源释放均由测试覆盖
- [ ] Desktop 目录与 Mobile 原生外壳的运行时入口打包格式
- [x] 扩展点(Registry)完整清单与各贡献 descriptor 契约 → 第四章（v0.1）
- [ ] `service_role` 取值集与期望态组合的精确衔接
- [x] 新 API 使用治理式 `apiContracts` + `ApiExposure`；公开路径、鉴权与独立数据面边界见《[API 暴露与数据面服务](API暴露与数据面服务.md)》。`apiRoutes` 仅保留兼容
