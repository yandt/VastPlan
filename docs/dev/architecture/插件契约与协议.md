...
Host、第一方 SDK 与 addressing 共用 `shared/go/protocollimit.Limits`，并在每一跳执行同一份限制。默认一元 payload 4 MiB、流帧 1 MiB、`CallContext`/传输 metadata 16 KiB、并发 256、pending 512、调用 deadline 30 秒、drain 30 秒；零值表示采用安全默认而不是禁用限制。完整决策与拒绝语义见 [ADR-0034](../decisions/ADR-0034-Backend协议资源边界.md)。
...
| 层级 | 稳定错误码 |
|---|---|
| 应用层 `CallResult.Error` | `capability.not_found`、`hook.aborted`、`hostcall.failed`、`kernel.service_error`、`permission.denied`、`plugin.handler_error`、`plugin.inactive` |
