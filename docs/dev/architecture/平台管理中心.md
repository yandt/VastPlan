...
v1 已实现四个既有领域页面、TypeScript 客户端、Edge BFF、角色/CSRF 防护、NATS addressing 适配和权限插件；节点部署后端、SDK 与签名 Node Lease Ready 判定已接入，页面仍待实现。制品领域当前只显示服务状态；目录检索、发布审批和供应链证明浏览需要先定义独立的制品管理契约，不复用仓库上传 API。
