...
v1 已实现五个领域页面、按 Portal/服务寻址的 TypeScript 客户端、管理绑定、Edge 强类型 BFF、活动受众/operation grant/角色/CSRF 纵深防护、NATS addressing 适配和权限插件。v2 正在把 Portal 治理拆成 `Platform Profiles` 与 `Portals` 两个工作区，并加入 Application/Profile/Binding 分域发布、不可变 Activation、语义差异、阶段进度与安全回滚。节点引导与 Backend 服务组合仍由 Deployment Manager 管理，见 [ADR-0077](../decisions/ADR-0077-Backend在线组合与可信发布边界.md)。制品领域当前只显示服务状态；目录检索、发布审批和供应链证明浏览需要先定义独立的制品管理契约，不复用仓库上传 API。
