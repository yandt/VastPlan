...
最小 Bootstrap 页面与 Portal Edge 通过 `X-VastPlan-Bootstrap-Page-Contract` 执行轻量契约握手。页面契约是规范模板（HTML、CSS、脚本及握手头名称，以 nonce 和契约值占位符归一化）的 SHA-256；页面内容变化会机械产生新的 64 位小写十六进制值，不再维护人工数字。Frontend CI 必须校验响应头、页面内嵌契约和摘要格式一致。测试或初始化前发现已打开页面落后于当前宿主时，页面必须自动刷新，禁止把旧表单结构送入新后端后再显示泛化 Schema 错误。响应继续使用 `Cache-Control: no-store`，但不能依赖缓存策略替换已经运行在浏览器中的旧 JavaScript。
...
- Database Runtime `0.17.0` 同时贡献公开数据面、`foundation.data.record-store@1.2.0`，以及仅宿主可调用的 `foundation.state.shared.sql.bootstrap@3.1.0` 和 `foundation.state.shared.sql@1.0.0`；Bootstrap 3.1 使用与普通连接相同的 `DatabaseConnectionCandidate`，并新增首次显式建库操作，但仍由可信宿主管理 secret、Profile 和 Store 绑定；Controller 从 Deployment 锁定的已验证制品投影 DataModel Inventory，只向该 Runtime 注入宿主保留配置，Node Agent 在候选能力进入公开路由前使用 Host 固定的 SYSTEM 身份同步，并在收到与候选目录精确绑定的 Schema Activation 授权后执行迁移，再从普通插件配置中摘除目录；其用户配置 `clusterMaxOpen` 与 Scheduler 可信派生的 `clusterMaxReplicas` 在组合根生成每实例连接硬预算，覆盖 active-active 与双代轮换的最坏连接占用；
...
- Deployment/Assignment 增加 `startup_tier=bootstrap|full`，默认 `full`。Node Agent 可以完成 Full 单元的下载、验签和安装，但在 Shared State Ready 前拒绝激活；激活门禁检查的是当前可用性（`BindingStore.Live()`），而非仅仅曾经绑定过（`Snapshot()`）。两个信号语义不同：Snapshot 是单调不可逆的历史记录，Live 反映最近一次调用结果——`ErrUnavailable` 清除 Live，任何应用级错误（NotFound/Conflict/Invalid）保留 Live。使用 Live 确保 Provider 实例消失时门禁能推迟新单元激活，而不是把一次可恢复的停机放大成 Full 层激活失败风暴，见 [ADR-0198](../decisions/ADR-0198-激活门禁活跃性与引导重试.md)。
- 恢复由已验证能力目录的 topology change 触发。Topology 订阅关闭（如 Router 重建）会结束旧订阅循环，但引导协调器会带退避重新订阅，确保 Router 恢复后 Open 仍能被尝试。已提交 Profile 但尚未绑定时同样启用有界退避重试（初始 500ms、上限 30s），稳态下零开销、Context 取消即停，见 [ADR-0198](../decisions/ADR-0198-激活门禁活跃性与引导重试.md)；Runtime 重建并重新登记 Capability 后，Controller 按已提交 Profile 幂等执行 Open；
...
- Bootstrap 单元换版不再经过依赖 Shared State 的 Deployment Manager。普通 `application` 发布 lane 在 desired CAS 前拒绝任何 Bootstrap 单元变化；可信宿主 `controlplane -bootstrap-unit-release` 只能改变现有 Bootstrap 插件的精确制品引用，不能同时改变配置、拓扑、副本、插件身份、Full 单元或应用组合。首次 Seed 仍使用独立 `seed` lane，三条 lane 的校验与 NATS KV CAS 共用同一次 current 读取，见 [ADR-0200](../decisions/ADR-0200-Bootstrap单元可信换版与双代Open屏障.md)。
- 热替换 Bootstrap 单元时，Node Agent 先激活候选路由并保留旧代，再把精确 Runtime Instance ID 交给 Platform Control Open 屏障。只有候选确实贡献 Bootstrap capability，且进入 `openReplicas` 最近一次全部成功的副本集合后，Node Agent 才提交新 generation 并退役旧代；超时或 Open 失败时回滚候选、保留旧代。冷启动、Full 单元和无关 Bootstrap 单元不等待该屏障。
