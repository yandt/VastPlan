...
6. 在线 Backend 配置同样只提交 Application Composition；`BackendPlatformCatalog` 由平台运维把 tenant/deployment 绑定到精确 Profile，可信内核重新解析并发布，不向浏览器开放 raw v2 或 Profile 写入（ADR-0077）。
...
- `engineering/deploy/platform-profile.json` 与 `engineering/deploy/application-composition.json` 是可编辑输入，`engineering/deploy/cluster.deployment.json` 由测试保证与 Resolver 输出完全一致。
