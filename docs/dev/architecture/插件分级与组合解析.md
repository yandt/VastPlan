...
> 状态：Backend v1 实施设计｜最后更新：2026-07-18
>
> 本文是插件分级、平台基线、应用组合和解析锁的单一真相源。决策依据见 [ADR-0057](../decisions/ADR-0057-插件分级管理与双输入组合解析.md)；插件身份见《[插件契约与协议](插件契约与协议.md)》，解析后的副本与放置见《[插件服务集群化设计](插件服务集群化设计.md)》。
...
字段沿用 Deployment v2 `ServiceUnit` 结构以避免重复一套调度 DTO，但其中 `plugins` 在此资源中语义固定为“应用候选”。Resolver 必须逐个读取已验证 Manifest 并执行分类，不能只靠字段名或 UI 筛选。
...
- 已实现 `schemas/composition/v1`、Backend Composition Resolver、解析锁、Controller 二次来源校验和单调 Deployment revision；生产 `controlplane` 已移除 raw v2 发布入口。
...
- Portal Platform Profile、Runner 构建 Resolver 和 Mobile Profile 是后续各内核实现项；本轮先固定跨内核契约原则。
