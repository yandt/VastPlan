...
- `FormDialog` 是 Dialog 的受治理 `form` 视觉变体。Dialog 框架、标题和操作按钮保持请求尺寸，内部表单控件统一降低一档：`xs → xs`、`sm → xs`、`md → sm`、`lg → md`；独立页面表单不继承该规则。其内容几何继续按 Dialog 请求尺寸计算：`xs/sm/md/lg` 的 body padding 为 `8/10/12/16px`、说明与字段区 gap 为 `4/8/8/16px`、行内 Label 最小宽度为 `80/88/96/112px`。FormDialog 使用 Renderer 原生垂直居中，最大高度为 `95vh`，并始终保留 24px 视口侧边距；Header、可滚动 Body 与 Footer 分别拥有布局空间。普通 Dialog 仍使用默认定位和 `90vh` 上限。Renderer 用同一张表单中最长的非 Boolean Label 推导唯一列宽，所有字段共享该宽度；宽度还统一预留必填星号与 12px 的 Label—输入框间距，桌面最多占字段行 48% 并保持单行，小于 480px 时切为上 Label、下控件。表单行距使用 FormDialog `contentGap`，顺序分区间距使用其两倍，避免与字段 margin 重复累加。说明文本自身 `margin: 0`，所有垂直节奏只由组合 Stack 的 gap 控制。
...
- Renderer 的 Form 根容器必须始终占满上级已分配的字段宽度；顺序分区 `sections` 属于结构容器，同样必须保持全宽。分区标题与唯一右侧分隔线同行；分区直接拥有对象字段时，该对象内部字段继承分区的 `columns/columnWidths`，对象自身通过字段 `span` 占据分区宽度。`controlAlignment` 只能对齐字段内部控件，不能令 JSON Schema 根对象、分区容器或筛选字段按内容宽度收缩。
- JSON Schema 数组统一使用紧凑 List 外壳：外层复用普通字段的 Label/控件列宽，不绘制卡片或列表外框；新增按钮固定在列表底部、占满字段列宽，并使用当前 Renderer 的系统按钮配色。标量项每行只保留输入、语义化删除按钮与可拖拽排序手柄；对象项使用无卡片的紧凑字段组，并保留对象内部真实字段 Label。所有项都禁止显示框架生成的“字段名-序号”标题；排序同时支持拖拽手柄、拖拽浮层与键盘替代操作。
- JSON Schema 字段声明 `description` 时，Renderer 必须在独立 Label 后显示系统帮助图标，并在鼠标悬停或键盘聚焦时通过 Tooltip 展示完整说明；说明不得同时重复显示在字段下方。`ui:help` 继续作为字段下方的显式帮助内容，Boolean 字段因不绘制独立 Form Label，可继续在字段下方呈现 `description`。
...
- 两种 Shell 通过共享 `PortalBrand` 输出 Logo、短名称与完整 Portal 名称；紧凑桌面 Logo 在鼠标悬停或键盘聚焦时显示完整名称。展开层覆盖其后紧邻的布局上下文而不改变导航或页面的几何尺寸，具体覆盖范围由各 Layout 的 CSS 依据自己的导航结构定义。
...
- 顶部布局的根组与“更多”触发器仅显示语义图标，并使用与 Page Header 操作一致的 `md` 图标规格；可访问名称与 Renderer 原生 Tooltip 使用完整本地化菜单名称。账户区保持圆形头像，二、三级页面名称只在展开菜单中显示。
