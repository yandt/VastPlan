...
排除：业务 Agent/Workflow/用户系统/LLM 实现，Frontend/Desktop/Mobile 内核，以及具体权限、审计和可观测后端插件。
...
| 性能 | 核心 benchmark 基线与 CI 回归阈值 | 已完成：注册/协议/本地寻址/调度/persistence 基准，PR 在同一 desktop 比较 base/head；耗时 >50% 且 >100ns 或分配 >25% 阻断 |
| 安全与供应链 | mTLS/NKey/ACL、漏洞/许可证、签名制品、SBOM | 内核基座已完成、制品准入增强中：运行链路、发布者签名、不可信 ArtifactSource → 内核 Verifier → VerifiedArtifact 强制链、签名种子源、Backend 源码漏洞/许可证 CI、逐目标 CycloneDX SBOM 与 OIDC 来源/SBOM 证明已有机器门禁；插件制品级漏洞/许可证准入与持续复扫按 ADR-0138 分阶段接入 |
