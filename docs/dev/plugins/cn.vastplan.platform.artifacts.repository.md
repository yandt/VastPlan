...
该第一方基础插件运行 HTTPS 制品发布与读取服务，负责 HTTP 传输、分离操作令牌、可重建 Catalog、单调 Publish Journal、确定性依赖解析、`deprecated/yanked/revoked` 生命周期、消费者引用快照、离线 Bundle 和可回滚 File Volume 迁移。对象存储与 OCI 通过供给 Provider 增加；控制器引用发布适配器、GC、审批和市场 API 仍在仓库领域扩展。
