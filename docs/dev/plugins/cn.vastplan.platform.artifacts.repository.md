...
stable 发布新增 `listPublications/submitPublication/approvePublication/getSupplyChainEvidence`。申请只能引用仓库中已验签、active 的 testing 制品，并绑定目标 stable ref、SHA-256、publisher、key ID 与源证明摘要；批准人与提交人必须不同。最终 HTTPS stable 上传在物理写入前再次验签并要求精确匹配批准记录，成功后记录最终证明摘要。只有发布 token、只有 Portal 权限或只有批准记录都不能单独完成 stable 发布。崩溃后从已验签 Catalog 收敛 `Approved -> Published`，迁移观察期两卷审批状态不一致会冻结发布。
