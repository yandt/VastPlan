...
- 动态表单把受治理的 `ui:options.autocomplete` 原样传至原生文本和密码输入；数据库等外部系统凭证可用 `new-password` 阻止浏览器将本站登录账号自动回填；
...
- 动态表单的 `duration` widget 使用 Ant `InputNumber + Select` 紧凑组合，按调用方白名单显示毫秒、秒、分、小时、天、周、月；切换单位只改变显示，提交值仍按契约的存储单位输出；
