...
本协议是**信封**，`CallContext / CallTarget / CallResult / CallEvent` 是**信件**——字段定义在[契约字段](契约字段.md)（下一篇）。同一套契约既走本协议（内核内），也走 02（内核间），保证端到端上下文与可观测一致。
...
- [ ] `CallContext/CallTarget/CallResult/CallEvent` 字段定稿（下一篇，本协议的承载物）
