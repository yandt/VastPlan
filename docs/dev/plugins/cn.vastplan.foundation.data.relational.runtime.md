...
宿主内部 Capability：`foundation.state.shared.sql.bootstrap@3.1.0`、`foundation.state.shared.sql@1.0.0`。两者只服务 Platform Control 两阶段启动，不作为普通插件可选择的数据库连接暴露。
...
`0.17.0` 把 Bootstrap 契约提升到 `3.1.0` 并增加 `close`。候选连接池活在 Runtime 进程内，宿主本地丢弃 Store 句柄不足以释放它；`close` 让宿主按 generation 显式归还被放弃的候选池。Runtime 只在请求 generation 与当前实际管理的 generation 一致时关闭，过期 close 不会误杀正在服务的池，见 [ADR-0199](../decisions/ADR-0199-跨进程候选连接池释放.md)。
