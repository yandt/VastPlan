...
Trivy binary、数据库内容摘要、离线参数和报告 Schema 均由插件内部 `assessmentruntime` 强制。`trivySnapshotDirectory` 必须精确指向 `snapshots/<databaseRevision>`，由 File Database Snapshot 基础插件先行原子物化；每次扫描仍验证 binary 版本和 `db/metadata.json + db/trivy.db` 内容摘要，固定启用 filesystem `vuln,license`、JSON、`--offline-scan`、`--skip-db-update`。未知 Schema、空扫描、报告超限、数据库漂移、未知许可证超过策略阈值均 fail-closed。
