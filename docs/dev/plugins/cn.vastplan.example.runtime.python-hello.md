...
源码入口为 `examples/plugins/cn.vastplan.example.runtime.python-hello/backend/main.py`。本地开发需先安装 `extensions/sdk/python/requirements.txt`，或执行 `python3 -m pip install -e extensions/sdk/python`。
...
go run ./engineering/tools/pluginpackage \
  -source examples/plugins/cn.vastplan.example.runtime.python-hello \
  -license-file LICENSE \
  -notice-file NOTICE \
  -out dist/cn.vastplan.example.runtime.python-hello.tar.gz
```
