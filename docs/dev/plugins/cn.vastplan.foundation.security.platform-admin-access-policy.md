...
该 foundation 插件以 `per-kernel + local-ephemeral + local + direct` 运行，在设置、凭证、数据库、制品、API Exposure 和部署能力所在 unit 内执行最终授权。它不读取 settings，不依赖远端服务，也不向浏览器暴露能力。业务插件只可调用 `stageManaged/activateManaged/abortManaged/retireManaged` 管理由凭证插件强制绑定给自己的句柄，不能继承 `put/rotate/revoke` 等管理员权限。API Exposure 另行收窄管理角色、插件自有 Endpoint Lease 和控制面向精确数据面安装 Ticket 的边界。
...
测试目标绑定已从通用 `platform.admin` 收窄为 `platform.deployment.test-target`。本插件仍包含 B1→B6 迁移期的硬编码 operation-role 表和 `platform.admin` 兼容；后续由签名 Permission Catalog、Policy Snapshot 和 `authorization-enforcer` 取代，不能继续向该兼容表增加新权限。角色、Provider Protocol、Node BFF 前置检查、settings 自举策略顺序和部署要求见《[在线角色与权限治理](../architecture/在线角色与权限治理.md)》、《[平台管理中心](../architecture/平台管理中心.md)》与 [ADR-0107](../decisions/ADR-0107-插件权限目录与系统管理授权治理.md)。
