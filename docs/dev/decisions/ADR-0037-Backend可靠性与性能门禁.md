...
3. 核心 benchmark 覆盖 Registry lookup/fanout、协议 context/关联表、本地 addressing、调度筛选和 scoped persistence。PR 在同一 desktop、同一次 job 中分别采样 base/head 各五次并比较中位数。
...
- 代价：PR benchmark 增加 CI 时间；24h soak 需要单独 desktop 配额。
