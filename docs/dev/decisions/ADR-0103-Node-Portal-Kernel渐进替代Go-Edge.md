...
- Portal Host 已接入可选 Addressing 生命周期和严格启动配置，首批 `GET /v1/portal-drafts`、`GET /v1/portal-governance` 只读 BFF 通过窄 Composer 端口调用；未知能力和路由继续 fail-closed。
- 后续仍需迁移 Composer 写操作、Interaction/平台管理、Activation/RuntimeSpec/内容对象端点，并完成真实 NATS+mTLS+权限对照 E2E 后切换流量。
