...
- 2026-07-22（身份边界修订）：消费 Bootstrap Inventory 与发布 Seed/LKG 引用拆成独立职责。普通 Node Agent 只验签并消费；唯一发布节点必须显式启用 `publish-bootstrap-references`，自动推进 LKG 也要求该职责。跨 NATS 时，稳定 `SYSTEM bootstrap-inventory/<repositoryId>` 只有同时满足“可信宿主创建”和 transport trust 的 `allowedSystemCallers` 精确授权才保留；否则仍重建为签名节点身份。该修订补齐原决策第 2、3 条在远程仓库场景下缺少的身份绑定，不允许通配符。
