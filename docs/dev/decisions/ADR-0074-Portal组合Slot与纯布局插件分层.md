...
导航固定为有界的 `zone → group → page`。页面只声明 `groupID`；分组的 `id / label / zone / icon / order` 由 `ui.shell-composition` 通过 Platform Profile 的 `composition.config.navigationGroups` 治理。未显式分组的页面进入所属 zone 的内建同名组；引用未知分组或跨 zone 分组必须拒绝组合。布局插件只能消费标准化后的非空分组，不能自行猜测业务分组或开放任意二级菜单 Slot。
