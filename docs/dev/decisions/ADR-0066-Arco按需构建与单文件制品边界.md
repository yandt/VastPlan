...
## 2026-07-18 补充：CSP 安全校验器预算

ADR-0072 用解释式 Draft 7 校验替换浏览器 AJV 动态编译。Metafile 确认新增成本主要来自 `json-schema-library` 93,823 字节和 URI 支持 19,759 字节，压缩后单文件为 1,644,870 字节；门禁预算据此显式调整为 1,700,000 字节。组件直连、样式闭包、单文件验签和禁止全量 Arco 入口的决定不变。
