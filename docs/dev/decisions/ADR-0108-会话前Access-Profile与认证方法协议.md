...
- 本地文件只是一种开发/静态部署适配器，在线控制面实现同一窄端口，不改变 HTTP 层。在本 ADR 的首个实施阶段，`access` Shell、AuthenticationFlow、Broker、密码数据库和验证码发送尚未落地；其后续完成情况见下方实施记录。
...
- 上述“roles claim 属于迁移状态”和“OIDC 后续包装”是决策落地过程中的历史状态。封板代码已删除 Portal Host 直连 OIDC 与 claim-to-role 路径；OIDC 现为 `authentication.method.v1` redirect Provider，主体权限只经 Authorization Session/Policy Snapshot 投影。
- 上述“OTP/Delivery 仍是后续”的记录已完成：邮箱/短信 OTP、`authentication.delivery.v1`、HTTPS Webhook Delivery 和 Material Lease 均已落地，并通过重发换代、过期、锁定、未知主体、Delivery 故障与并发单次消费测试。
