...
8. React、React DOM、`@vastplan/portal-ui` 和 `@vastplan/ui-contract` 由 Portal Shell 作为单例共享依赖提供。V1 使用浏览器 import map；后续可以换成 Module Federation 或其他装载器，但不得改变 RuntimeSpec、字节摘要先验校验和宿主注册边界。
