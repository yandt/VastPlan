...
## 实施补充：多语言自动取证（2026-07-24）

第一方工作区采用“统一编排、按语言读取实际构建事实”，不再要求每个插件手工准备 SBOM，也不生成只有插件根组件的空壳文档：

- Go `native` / `dynamic-go` 从每个最终可执行文件或 `.so` 的 build info 读取实际链接模块，并分别绑定 SHA-256；源码 import 或 `go.mod` 不能替代最终链接事实。
- Node Worker 与 Frontend 从各自最终 esbuild metafile 读取实际打包输入，再把 npm 包映射到工作区或已安装依赖的精确版本；构建器必须把 metafile 与模块图放在同一个临时构建目录。
- Python 按 ADR-0137 从签名包内的 PEP 751 `pylock.toml` 读取完整传递依赖；Manifest 只保留解释器范围和精确直接依赖。缺少标准锁或锁、wheel、直接依赖不一致时拒绝自动生成。
- 一个插件同时包含多种贡献面时，共享同一个 CycloneDX 根组件，并合并上述证据。组件和依赖按稳定键排序，serial number 由规范化内容确定性派生。

`pluginpackage -source` 在未指定 `-sbom` 时自动调用这些取证器，并在打包前用内核共享校验器复验结果；显式 `-sbom` 仍是第三方构建系统、工作区外源码和 Rust/Java/C# 等尚未接入自动取证器语言的标准扩展入口。未知原生语言不得把任意入口伪装成 Go 二进制，缺少对应构建事实时必须 fail-closed。

`pluginpackage -package` 仍然只允许原样复用已审批 testing 候选，不能自动生成、补加或替换 SBOM。引入自动 SBOM 后，已经发布过的同版本插件不得用新包覆盖；首次发布带 SBOM 的制品必须按正常版本升级产生新的不可变 ref。
