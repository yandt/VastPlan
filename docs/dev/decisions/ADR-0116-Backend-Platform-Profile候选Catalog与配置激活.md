...
- 2026-08-05：Backend Platform Profile v1 增加 `productCapabilities` 时，Catalog 内容发生了变化，但 NATS envelope 当时已经是 schema v3，因此不能只按外壳版本识别迁移。契约层新增唯一强类型迁移器：先按缺少该字段的历史字段顺序复核 Profile binding 和 Catalog digest，再补齐空能力目录、派生当前精确摘要。Catalog Store 对 v1/v2 envelope 以及无候选的 v3 历史内容均可只读解释，Bootstrap `Seed` 再用原 KV revision CAS 写回当前规范；本地 Seed Runtime LKG 复用同一迁移器。未知字段、候选混入旧内容、旧摘要漂移或非精确 binding 继续失败关闭，不删除整个 NATS 目录，也不以新的启动 Seed 覆盖在线活动 Catalog。
