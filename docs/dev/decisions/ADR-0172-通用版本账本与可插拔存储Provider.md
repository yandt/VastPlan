...
2. `version.ledger.v1` 只负责：不可变 JSON 版本、最多双父的版本 DAG、内容摘要、幂等写入、精确读取、第一父历史遍历、可变命名 Head、不可变 Tag、确定性内容比较和祖先查询。审批、发布、上线、回滚、业务权限、版本可见性和内容合并仍由调用方领域插件负责。
...
9. 每个 Provider 必须通过同一契约套件：create-only 幂等、同 key 不同内容冲突、内容摘要复核、双父 DAG 闭合、Head CAS 单赢家、Head 删除重建防 ABA、Tag 不可变、跨 stream/tenant 拒绝、损坏 fail-closed、分页稳定和重启恢复。
10. Head 删除不是 revision 的删除。Provider 必须保留足以延续同名 Head revision 的墓碑或等价持久状态；不存在状态上的 `expectedRevision=0` 只表示当前没有活动 Head，不得令旧的非零 revision 在重建后重新有效。
