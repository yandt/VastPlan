...
现仅保留不可覆盖的 `vastplan.host/account` 账户头像根锚点。它是 Shell 的唯一固定根菜单，但不是通用插件菜单容器：只有 Platform Profile 选定的个人中心插件可在签名 Manifest 中以该锚点为 required 父级，声明自己的账户二级菜单；页面始终挂到二级菜单，成为最终叶子。个人中心是账户区唯一特例，默认提供“用户信息”和“偏好设置”菜单；其他插件保持普通导航模型，只能声明普通区域的菜单，或经个人中心受治理扩展点向其已有菜单添加页面。`primary`、`secondary`、`settings` 继续作为插件 Manifest 选择的布局区域，分别决定两种 Shell 的视觉承载位置，但不携带业务名称、图标、顺序或菜单树。

`navigationOverrides` 只能操作已安装的插件节点，并明确禁止修改账户锚点或将节点重新移入账户锚点。账户二级菜单只能由 Platform Profile 选定个人中心的签名 Manifest 声明，避免 Profile 以重排方式绕过插件归属、依赖和安装治理；这样账户入口保持认证与恢复所需的最小可信边界，所有其他一级、二级菜单仍保持插件安装即自发现、停用即消失的自治行为。
