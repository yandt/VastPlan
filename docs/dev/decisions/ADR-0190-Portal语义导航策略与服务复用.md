...
现仅保留不可覆盖的 `vastplan.host/account` 账户头像根锚点。它是 Shell 的唯一固定根菜单，同时是受治理的插件菜单容器：任何已启用、签名可信的插件都可在 Manifest 中以该锚点为 required 父级，声明自己的账户二级菜单；页面始终挂到二级菜单，成为最终叶子。个人中心是默认贡献者，提供“用户信息”和“偏好设置”菜单；其他插件可声明平级账户菜单，也可经个人中心受治理扩展点向其已有菜单添加页面。`primary`、`secondary`、`settings` 继续作为插件 Manifest 选择的布局区域，分别决定两种 Shell 的视觉承载位置，但不携带业务名称、图标、顺序或菜单树。

`navigationOverrides` 只能操作已安装的插件节点，并明确禁止修改账户锚点或将节点重新移入账户锚点。账户二级菜单只能由插件自己的签名 Manifest 声明，避免 Profile 以重排方式绕过插件归属、依赖和安装治理；这样账户入口保持认证与恢复所需的最小可信边界，所有其他一级、二级菜单仍保持插件安装即自发现、停用即消失的自治行为。

## 2026-08-03 第一方菜单图标基线

第一方业务菜单不得再以 `settings`、`success`、`menu` 等通用语义图标充当领域标识。菜单优先使用集中维护、跨 UI 引擎一致的扩展语义词表；当前基线覆盖用户、偏好、认证、市场、制品库、资源、插件、门户、数据库、部署、API、安全、凭证、工作台和扩展等领域。相同 Portal 区域的一级菜单应使用不同的领域语义。

自定义 SVG 是补充能力而非强制要求：只有统一词表不存在准确语义时，插件才在 `frontend/icons/navigation/` 作者目录提供受限 SVG，并在制品打包时归一化为 AST。未来新增第一方业务导航时，必须同次提交语义图标引用（或必要的 SVG）、插件 SemVer 和所有精确 Profile 引用，避免稳定制品身份漂移。
