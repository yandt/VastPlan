...
| Portal 前端内核 | 管理闭环可试用 | 已有 UI Contract 3.0、标准/顶部动态布局、Arco/MUI 适配、插件多语种、五个服务管理页面，以及 Profile/Application/Binding 发布与不可变 Activation 上线闭环；不包含上层业务页面，生产集群状态端口仍待封板 |
