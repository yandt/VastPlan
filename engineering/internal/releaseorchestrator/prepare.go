...
func PrepareRelease(repositoryRoot string, spec ReleaseSpec) (ReleasePlan, error) {
	...
	generated := make(map[string]struct{}, len(plan.GeneratedFiles)+len(contractChanges)+len(catalogChanges))
	for _, path := range plan.GeneratedFiles {
		generated[path] = struct{}{}
	}
	for _, change := range append(contractChanges, catalogChanges...) {
