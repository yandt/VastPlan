...
//
// 走真实 contracts/proto/协议/进程，不 mock 契约——否则测不出协议漂移与版本失配。
// 运行：go test -tags=e2e ./engineering/e2e/...
package e2e
...
func repoRoot(t *testing.T) string {
	...
	return filepath.Dir(filepath.Dir(wd)) // engineering/e2e/ 的上两级
}
...
// 兼容性夹具绕过当前 extensions/sdk/go/plugin，直接使用 Plugin-Host v1 消息集。
// 因此 SDK 内部实现即使重构，也不能掩盖宿主对既有 wire 客户端的破坏。
func TestPluginHost_LegacyV1RawClientRemainsCompatible(t *testing.T) {
	bin := buildPlugin(t, "./engineering/e2e/fixtures/plugins/legacy-v1")
	host := newHost(t, "1.0.0")
...
func TestPluginMigrationLifecycle_ThreePhaseAndMissingHandlerFailClosed(t *testing.T) {
	...
	bin := buildPlugin(t, "./engineering/e2e/fixtures/plugins/migrator")
...
func TestProtocolRuntime_StateMigrationCommitAndFailureRollback(t *testing.T) {
	...
	t.Setenv("VASTPLAN_MIGRATION_LOG", logPath)
	v1Bin := buildPlugin(t, "./engineering/e2e/fixtures/plugins/migrator-v1")
	v2Bin := buildPlugin(t, "./engineering/e2e/fixtures/plugins/migrator")
...
func TestPluginLaunch_MissingEnginesRejected(t *testing.T) {
	bin := buildPlugin(t, "./engineering/e2e/fixtures/plugins/no-engines")
	host := newHost(t, "0.1.0")
...
func TestPluginRegistration_InvalidDescriptorRejected(t *testing.T) {
	bin := buildPlugin(t, "./engineering/e2e/fixtures/plugins/invalid-descriptor")
	host := newHost(t, "0.1.0")
