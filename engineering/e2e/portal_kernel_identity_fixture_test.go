...
func (f *portalFileIdentityFixture) write(t *testing.T) {
	t.Helper()
	f.mu.Lock()
	defer f.mu.Unlock()
	f.writeLocked(t)
}
func (f *portalFileIdentityFixture) writeLocked(t *testing.T) {
	raw, err := json.Marshal(map[string]any{"sessions": f.sessions})
	if err != nil {
		t.Fatal(err)
	}
