...
func TestSelect_NoCheckerRegisteredDeniesEverything(t *testing.T) {
	...
	launchAll(t, host, "./examples/plugins/cn.vastplan.example.backend.hello-world/backend") // 故意不装权限插件
...
func TestSelect_HigherPriorityDenyWinsOverLowerPriorityAllow(t *testing.T) {
	host := newHost(t, "0.1.0")
	launchAll(t, host, "./examples/plugins/cn.vastplan.example.backend.permission/backend")
