...
func publishLocalTest(
	...
	packageBytes []byte,
) (artifactrepository.Artifact, artifactrepositoryv1.Receipt, bool, error) {
	token, err := localtest.ReadTokenFile(filepath.Join(runDir, "secrets", "artifact-local-test.token"))
	if err != nil {
		return artifactrepository.Artifact{}, artifactrepositoryv1.Receipt{}, false, err
	}
	...
	if err != nil {
		return artifactrepository.Artifact{}, artifactrepositoryv1.Receipt{}, false, fmt.Errorf("读取 local-test Catalog: %w", err)
	}
	if receipt, found := exactReceipt(snapshot, ref); found {
		if receipt.SHA256 != artifact.SHA256 {
			return artifactrepository.Artifact{}, artifactrepositoryv1.Receipt{}, false, errors.New("local-test 仓库已存在相同 ref 但摘要不同的不可变制品")
		}
...
func verifyLocalEnvelope(trust *artifactrepository.TrustStore, envelope artifacttrust.Envelope) error {
