...
func publishRemote(packageBytes []byte, publisher, channel string, options remotePublishOptions) (artifactrepository.Artifact, error) {
	if options.RepositoryURL == "" || options.PublishToken == "" || options.TrustFile == "" || options.SignKey == "" || options.KeyID == "" {
		return artifactrepository.Artifact{}, errors.New("必须配置远端地址、发布令牌、信任文档、签名私钥和 key ID")
	}
	if channel == "stable" && (options.ReadToken == "" || options.ReadToken == options.PublishToken) {
		return artifactrepository.Artifact{}, errors.New("stable 发布必须配置与发布令牌不同的读取令牌")
	}
	if options.Timeout <= 0 {
		return artifactrepository.Artifact{}, errors.New("远端发布超时必须大于零")
	}
	artifact, err := artifactrepository.Describe(channel, packageBytes)
	if err != nil {
		return artifactrepository.Artifact{}, fmt.Errorf("生成制品元数据: %w", err)
	}
	privateKey, err := artifactrepository.LoadEd25519PrivateKeyPEM(options.SignKey)
	if err != nil {
		return artifactrepository.Artifact{}, fmt.Errorf("加载发布私钥: %w", err)
	}
	trust, err := artifactrepository.LoadTrustStore(options.TrustFile)
	if err != nil {
		return artifactrepository.Artifact{}, fmt.Errorf("加载信任文档: %w", err)
	}
	...
	if channel == "stable" {
		reader := &artifactrepository.RemoteRepository{BaseURL: options.RepositoryURL, Token: options.ReadToken, Trust: trust, Client: client}
		...
		if len(provenanceRaw) == 0 && len(verificationRaw) == 0 {
			...
			return artifactrepository.Artifact{}, errors.New("stable 来源证明与 testing 候选不是同一组不可变 sidecar")
		}
		if len(securityAdmissionRaw) == 0 {
			...
			return artifactrepository.Artifact{}, errors.New("stable 安全准入记录与 testing 候选不是同一不可变 sidecar")
		}
	}
	attestation, err := artifactrepository.SignArtifact(artifact, publisher, options.KeyID, privateKey, time.Now().UTC())
	if err != nil {
		return artifactrepository.Artifact{}, fmt.Errorf("签署制品: %w", err)
	}
	remote := &artifactrepository.RemoteRepository{BaseURL: options.RepositoryURL, Token: options.PublishToken, Trust: trust, Client: client}
	...
	if err != nil {
		return artifactrepository.Artifact{}, fmt.Errorf("提交签名制品: %w", err)
	}
	return published, nil
}

func verifyStableCandidate(ctx context.Context, reader *artifactrepository.RemoteRepository, stable artifactrepository.Artifact, publisher, keyID string) ([]byte, []byte, []byte, error) {
	if reader == nil {
		return nil, nil, nil, errors.New("stable 发布缺少 testing 候选读取器")
	}
	source := artifactrepository.Ref{PluginID: stable.PluginID, Version: stable.Version, Channel: "testing"}
	...
	var proof artifactrepository.Attestation
