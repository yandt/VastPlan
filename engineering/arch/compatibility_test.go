...
func TestCompatibility_BackendV1Matrix(t *testing.T) {
	...
	assertStringSet(t, "Backend 公开扩展点", extpoint.BackendPluginPoints(), []string{
		"agent", "api.route", "authentication.provider", "configuration.controller", "configuration.resource-controller", "configuration.scoped-resolver", "event.sink", "hook", "permission.checker", "desktop.capability", "tool.package",
	})
