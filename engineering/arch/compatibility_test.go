...
func TestCompatibility_BackendV1Matrix(t *testing.T) {
	...
	assertStringSet(t, "Backend 公开扩展点", extpoint.BackendPluginPoints(), []string{
		"agent", "api.route", "authentication.provider", "configuration.controller", "configuration.resource-controller", "event.sink", "hook", "permission.checker", "runner.capability", "tool.package",
	})
